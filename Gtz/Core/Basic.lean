...
/-- **Conjugation of an atom**, rectangular: `(M g)(M g)ᵀ = M (g gᵀ) Mᵀ`. Every
congruence step of the campaign — whitening, compression, deflation, Naimark's four
steps — is this identity at a different shape of `M`. -/
theorem atomMatrix_conj {rows cols : ℕ} (M : Matrix (Fin rows) (Fin cols) ℝ)
    (g : Fin cols → ℝ) :
