...
converging with `Gtz.Certificates.TriangleClosure` from the independent Poncelet round.
...
the tight-direction hypothesis the whole `Gtz.Reduction.RayleighCertificate` chain consumes —
