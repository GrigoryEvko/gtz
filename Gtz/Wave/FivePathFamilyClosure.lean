...
/-- The FivePath family, listed. -/
theorem canonicalFivePathFamily_eq_literal :
    canonicalFivePathFamily
      = ({{0, 1, 2}, {0, 2, 3}, {0, 1, 4}, {0, 3, 5}} :
          Finset (Finset (Fin 6))) := by decide

/-- **THE FIVEPATH KILL.**  No crux selects this family. -/
theorem SixThreeCrux.false_of_family_canonicalFivePathFamily
    (crux : SixThreeCrux)
  ...
  have hblockCases : block = {0, 1, 2} ∨ block = {0, 2, 3} ∨ block = {0, 1, 4} ∨ block = {0, 3, 5} := by
    have hmem := hblockMem
    rw [hfamily, canonicalFivePathFamily_eq_literal] at hmem
  ...
  · -- selected {0, 1, 2}
    have heraseEq : canonicalFivePathFamily.erase {0, 1, 2}
        = ({{0, 2, 3}, {0, 1, 4}, {0, 3, 5}} :
            Finset (Finset (Fin 6))) := by decide
    have hcov1 : (4 : Fin 6) ∈ totalTightSupport tightVec {0, 1, 4} := by
      obtain ⟨coverBlock, hcoverMem, hcoverAtom⟩ := hcoverage 4
      rw [hfamily, canonicalFivePathFamily_eq_literal] at hcoverMem
    ...
    have hcov2 : (5 : Fin 6) ∈ totalTightSupport tightVec {0, 3, 5} := by
      ...
      · exact absurd (totalTightSupport_subset tightVec (by decide) hcoverAtom) (by decide)
      · exact hcoverAtom
    by_cases hmemSplit3 : (3 : Fin 6) ∈ totalTightSupport tightVec {0, 2, 3}
    ·
    ...
    ·
      have hpin10 : totalTightSupport tightVec {0, 2, 3} = {0, 2} :=
  ...
  · -- selected {0, 2, 3}
    have heraseEq : canonicalFivePathFamily.erase {0, 2, 3}
        = ({{0, 1, 2}, {0, 1, 4}, {0, 3, 5}} :
            Finset (Finset (Fin 6))) := by decide
    ...
    ·
      have hpin20 : totalTightSupport tightVec {0, 1, 2} = {0, 2} :=
  ...
  · -- selected {0, 1, 4}
    have heraseEq : canonicalFivePathFamily.erase {0, 1, 4}
        = ({{0, 1, 2}, {0, 2, 3}, {0, 3, 5}} :
            Finset (Finset (Fin 6))) := by decide
    ...
    ·
      by_cases hmemSplit23 : (3 : Fin 6) ∈ totalTightSupport tightVec {0, 2, 3}
      ·
        ...
        have hcancelAt26 := hcancel 2 (fun hmem => absurd (hpairSubset hmem) (by decide))
        rw [hfamily, heraseEq, Finset.sum_insert (by decide),
          Finset.sum_insert (by decide), Finset.sum_singleton]
        ...
        have hcoeff28 : coefficient {0, 2, 3} = 0 :=
      ...
      ·
        have hpin30 : totalTightSupport tightVec {0, 2, 3} = {0, 2} :=
          eq_pair_of_notMem_third (totalTightSupport_subset tightVec (by decide))
          (crux.two_le_card_totalTightSupport tightVec hunit hEigen {0, 2, 3} (by decide)) hmemSplit23 (by decide)
        exact crux.false_of_totalTightSupport_subset_neighbor
          (hostBlock := {0, 2, 3})
          (neighborBlock := {0, 1, 2})
          hfour (by rw [hfamily]; decide) (by rw [hfamily]; decide)
  ...
  · -- selected {0, 3, 5}
    have heraseEq : canonicalFivePathFamily.erase {0, 3, 5}
        = ({{0, 1, 2}, {0, 2, 3}, {0, 1, 4}} :
            Finset (Finset (Fin 6))) := by decide
    have hcov32 : (4 : Fin 6) ∈ totalTightSupport tightVec {0, 1, 4} := by
      obtain ⟨coverBlock, hcoverMem, hcoverAtom⟩ := hcoverage 4
      rw [hfamily, canonicalFivePathFamily_eq_literal] at hcoverMem
      simp only [Finset.mem_insert, Finset.mem_singleton] at hcoverMem
      rcases hcoverMem with rfl | rfl | rfl | rfl
      ...
      · rw [hpairEq] at hcoverAtom
        exact absurd (hpairSubset hcoverAtom) (by decide)
    by_cases hmemSplit33 : (1 : Fin 6) ∈ totalTightSupport tightVec {0, 1, 2}
    ·
      by_cases hmemSplit34 : (2 : Fin 6) ∈ totalTightSupport tightVec {0, 2, 3}
      ·
        have hcancelAt35 := hcancel 4 (fun hmem => absurd (hpairSubset hmem) (by decide))
        have hcancelAt36 := hcancel 1 (fun hmem => absurd (hpairSubset hmem) (by decide))
        have hcancelAt37 := hcancel 2 (fun hmem => absurd (hpairSubset hmem) (by decide))
        rw [hfamily, heraseEq, Finset.sum_insert (by decide),
          Finset.sum_insert (by decide), Finset.sum_singleton]
          at hcancelAt35 hcancelAt36 hcancelAt37
        have hcoeff38 : coefficient {0, 1, 4} = 0 :=
        ...
        have hcoeff40 : coefficient {0, 2, 3} = 0 :=
      ...
      ·
        have hpin41 : totalTightSupport tightVec {0, 2, 3} = {0, 3} :=
          eq_pair_of_notMem_second (totalTightSupport_subset tightVec (by decide))
          (crux.two_le_card_totalTightSupport tightVec hunit hEigen {0, 2, 3} (by decide)) hmemSplit34 (by decide)
        exact crux.false_of_totalTightSupport_subset_neighbor
          (hostBlock := {0, 2, 3})
          (neighborBlock := {0, 3, 5})
          hfour (by rw [hfamily]; decide) (by rw [hfamily]; decide)
    ...
    ·
      have hpin42 : totalTightSupport tightVec {0, 1, 2} = {0, 2} :=
        eq_pair_of_notMem_second (totalTightSupport_subset tightVec (by decide))
