...
/-!
# The rank-five normal form — the interface of the second rank rung

A `(6,3)` crux datum whose assembly has rank FIVE reduces to one package:
a support-minimal multiplier with the same assembly, five positive labels
whose directions are a basis of `range Ξ`, and the coefficient coordinates
`L`, `M`, `H` with every landed law.  The trace of `M` is the captured
rank, and the two rank-five survivors `(5,2,3)` and `(5,3,2)` give the
disjunction `tr M = 2 ∨ tr M = 3` — the rank-five read of the survivor
list.  The per-pattern work of the rank-five rung consumes THIS statement
and nothing upstream of it.

## PROVED here (no `sorry`, no `axiom`, no `native_decide`)

* `Gtz.SixThreeCrux.exists_rankFive_coefficient_normalForm` — **THE NORMAL
  FORM.**  Fourteen simultaneous facts at every rank-five crux datum,
...
/-- **THE RANK-FIVE NORMAL FORM.**  Every rank-five crux datum carries a
support-minimal multiplier with the same assembly, an injective five-label
basis inside its positive support, and coefficient coordinates `L`, `M`, `H`
with

    `L B = 1`,  `P B = B M`,  `M² = M`,  `B H Bᵀ = Ξ`,  `Hᵀ = H`,
    `H ⪰ 0`,  `ker H = 0`,  `M H = H Mᵀ`,  `tr M = 2 ∨ tr M = 3`. -/
theorem SixThreeCrux.exists_rankFive_coefficient_normalForm
    (crux : SixThreeCrux)
    ...
    ∃ (reducedWeight : activeIndex → ℝ)
      ...
      IsChartStationaryData 3
        ...
        ∧ M * H = H * Mᵀ
        ∧ (Matrix.trace M = 2 ∨ Matrix.trace M = 3) := by
  classical
  ...
  have hrankReduced : Module.finrank ℝ (LinearMap.range (Matrix.toLin'
      (chartMultiplierAssembly activeSet reducedWeight tightDir))) = 5 := by
    rw [hassemblyEq]
    exact hrankFive
  ...
  -- transport the index type along the rank-five equation
    ...
    calc (L * assembly * Lᵀ)ᵀ
      ...
      _ = L * assembly * Lᵀ := by
          rw [Matrix.transpose_transpose, Matrix.transpose_mul, hsymmAssembly,
            Matrix.mul_assoc]
  -- the trace at rank five: the captured rank is two or three
  have htrace : Matrix.trace (L * ((chartPointOfDesign crux.design).chart * B)) = 2
      ∨ Matrix.trace (L * ((chartPointOfDesign crux.design).chart * B)) = 3 := by
    ...
    rcases hsurvivors with ⟨htotal, -, -⟩ | ⟨-, hcaptured, -⟩ | ⟨-, hcaptured, -⟩
      | ⟨htotal, -, -⟩
