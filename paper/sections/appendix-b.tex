\section{Worked designs}\label{app:data}

This is the instance catalogue: every design the paper computes with,
recorded once, with the numbers each body statement reads off it.  Each is
given by data from which
...
\]
confirming $\sum_C\pi_C=1$.  Corollary~\ref{cor:minor} reads the gap minors:
$\det W[C]=\bigl(\prod_{c\in C}t_c\bigr)\det(\Gram_C-I_3)=\tfrac1{64}\cdot0=0$
at every triple, while the same reading at the pair $\{0,1\}$ gives
$\tfrac1{16}\cdot3=\tfrac3{16}$, so the $3\times3$ minor vanishes and the
$2\times2$ one does not.  All four characteristic polynomials coincide,
