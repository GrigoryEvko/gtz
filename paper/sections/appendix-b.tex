...
be verified by finite arithmetic, and every constant quoted is exact.  Radicals occur in three places, the icosahedral design of
\S\ref{app:icosa}, the whitener of a graphic design
(\S\ref{app:diamond}), and the normalization of the tetrahedron, and in
each case the algebraic relation eliminating them is displayed.  No entry rests on a floating-point evaluation.
...
whose gap at the surviving index $0$ is exactly $0$, at every $\tau$.  Theorem~\ref{thm:sharp} exploits that frozen equality: the Schur term
of \eqref{eq:schur} is matched in full at $t_z=0$ and falls short by a
quantity of first order in $t_z$ thereafter, with no downstairs slack
to close the gap.
