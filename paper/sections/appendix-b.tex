...
\[
  ...
  \;=\;\sum_{i<j}(x_i-x_j)^2 .
\]
Here $\ell_0=\lVert g_0\rVert^2=3$, so
$3\lVert x\rVert^2-\langle x,g_0\rangle^2$ is the Cauchy--Schwarz
deficiency of $x$ against $g_0$, and the last equality is Lagrange's
identity, whose general term $\bigl(x_i(g_0)_j-x_j(g_0)_i\bigr)^2$ is
$(x_i-x_j)^2$ because every entry of $g_0$ is $1$.  A sum of squares
vanishes only where every square vanishes, that is where $x_1=x_2=x_3$.
So the form vanishes exactly on $\R g_0$, the direction omitted from $C$.
The three squares vanish on three planes through $g_0$, drawn in
Figure~\ref{fig:tetracert}.

\begin{figure}[ht]
  \centering
  \input{figures/tetracert}
  \caption{The certificate of Example~\ref{ex:tetra} at the triple
  $C=\{1,2,3\}$.  Each square $(x_i-x_j)^2$ vanishes on a plane spanned by
  $g_0$ and one selected atom, drawn left as its great circle on the
  circumsphere; the three planes meet in $\R g_0$ alone, the eigenline of
  $\SC$ for the eigenvalue $1$ of \eqref{eq:tetraspec}.  Right: the same three
  planes seen along that line, six rays $60^{\circ}$ apart, with the unit
  normals $n_c$, signed to sum to zero, at $120^{\circ}$ and the level
  $x\tp(\SC-I_3)x=1$ the circle of radius $1/\sqrt3$ of
  Figure~\ref{fig:tetragap}.  The grouping drawn is
  the one of Example~\ref{ex:tetra}; the development certifies the triple
  $C=\{0,1,2\}$ instead, by the image of this certificate under
  $\diag(-1,-1,1)$, which permutes the atoms by $(0\,3)(1\,2)$.}
  \label{fig:tetracert}
\end{figure}
...
\subsection{The naive dual at \texorpdfstring{$(2,1)$}{(2,1)}}\label{app:uneven}
...
\subsection{The graphic instance}\label{app:graphicinstance}

Restricting the problem to the designs of \eqref{eq:graphicatom} gives a
statement of its own: every connected weighted multigraph with $m$ edges and
$k+1$ vertices has a $k$-subset of edges that dominates, which by
\texttt{Gtz.is\allowbreak Spanning\allowbreak Tree\_of\_dominates} is a
spanning tree.  The formal statement quantifies over the graph, the
conductances and the weights (\texttt{Gtz.Graphic\allowbreak Gtz} over
\texttt{Gtz.Graph\allowbreak Design\allowbreak Data}).  Graphic designs are
designs, so one direction is immediate: $\Gtz(m,k)$ implies its graphic
instance (\texttt{Gtz.graphic\allowbreak Gtz\_of\_gtz\allowbreak Weighted}).

The converse fails at the level of statements.  Fix the multigraph.  Global
rescaling of the conductances leaves \eqref{eq:graphicatom} fixed, since
$c\mapsto\lambda c$ sends $L$ to $\lambda L$ and $R$ to $\lambda^{-1/2}R$ and
the two factors cancel in $g_e$; so the family the graph carries is
parameterized by $c$ modulo scale and $t$ in the simplex, of dimension
$2m-2$.  Designs at $(m,k)$ modulo the orthogonal group have dimension
$mk+m-1-k^2$: the vectors and the weights, less the $\binom{k+1}2$ equations
of \eqref{eq:parseval} and the $\binom k2$ dimensions of $O(k)$.  There are
finitely many multigraphs on $k+1$ vertices with $m$ edges, so the graphic
designs form a finite union of families of positive codimension --- $10$
against $14$ at $(6,3)$, and $12$ against $18$ at $(7,3)$.

A proof of the graphic instance is therefore not a proof of $\Gtz(m,k)$.  The
instance gives a refutation lane instead: a graphic counterexample is a
counterexample, so \eqref{eq:graphicdominate} may be searched in the
electrical reading without the restriction weakening what a witness would
settle.  Nothing in this paper has found one.

The dimension count above is not formalized.
\texttt{Gtz.graphic\allowbreak Gtz\_of\_gtz\allowbreak Weighted} is, and it
is the only implication between the two statements that the development
carries.
