...
This section fixes the objects, derives the counting identities that later
arguments use, and settles the cases those identities decide on their
own.  A lemma and a counterexample then locate the point at which
counting stops working, which is where the rest of the paper begins.
...
The vectors need not be distinct or nonzero, so repeated vectors and the
zero vector are admitted.  Excluding them would change the problem without
simplifying it: a $k$-subset containing two parallel vectors is
rank-deficient and cannot dominate, and that mechanism, seen first in
Example~\ref{ex:octa}, makes selection combinatorial rather than metric.
The weights are real even over $\C$, and they do not enter $\SC$ at all,
appearing only through \eqref{eq:parseval}; domination therefore survives
any operation that keeps the vectors and moves the weights.

Since $g_c^{\vphantom{\mathsf{H}}}g_c\adj\psd0$ for every $c$, adding indices to
$C$ can only increase $\SC$ in the Loewner order, so a superset of a
dominating set dominates.  The cardinality restriction $\lvert C\rvert=k$
therefore carries the problem, and it is sharp on both sides: fewer than
$k$ vectors span a proper subspace and leave $\SC$ singular, while at
$\lvert C\rvert=m$ domination is automatic
...
\S\ref{sec:chart}: its inverse defines the pivot of a vector, and its
positive definiteness supplies the whitening that makes duality possible.
...
The two propositions dispose of the cells $m=k$ and $k=1$, and they
exhaust what the counting identities give directly.  The range
\eqref{eq:elementary} covered by the maximal-volume principle is larger by
one column, $m=k+1$.  That column does reduce to the scalar mechanism of
Proposition~\ref{prop:rankone}, but only after a change of variables
trading the vectors for the kernel of the associated projection; it is
Theorem~\ref{thm:corankone}, and the elementary methods end there.
...
At a fixed size the implication runs only this way, making the weighted
statement the stronger of the two; over all sizes they agree, for the
reason given in \S\ref{sec:intro}.
...
every $x$ on a sphere of dimension $k-1$.  No averaging identity produces
such a set.  The quantity to be dominated in \eqref{eq:dominate} is a
matrix; \eqref{eq:parseval} controls every scalar linear functional of the
family, and the least eigenvalue is not one of them.

The scalar relaxation of \eqref{eq:pointwise} shows how far that failure
goes.  If $C$ dominates then all $k$ eigenvalues of $\SC$ are at least
$1$, so
\begin{equation}\label{eq:tracetest}
  \tr\SC\;=\;\sum_{c\in C}\ell_c\;\ge\;k .
\end{equation}
A subset meeting this necessary condition costs no work to find: by
Lemma~\ref{lem:trace} some $\ell_{c_0}\ge k$, and any $k$-subset
containing $c_0$ already satisfies \eqref{eq:tracetest}.  The trace test
is thus satisfiable in every design, and it separates no dominating
subset from a non-dominating one.  The design below, a first illustration
of the cell $(6,3)$ as well, shows how far apart the two can be.
...
strictly.  What separates them is which subspaces the chosen vectors
miss, and the trace is blind to that.
\end{example}
...
\eqref{eq:parseval}.  Two instruments address it, and the remaining
sections develop them: a change of coordinates turning
\eqref{eq:dominate} into a statement about principal blocks of a
projection (\S\ref{sec:chart}), and, where that does not decide the cell,
a variational analysis of the worst design (\S\ref{sec:compactness} and
\S\ref{sec:interior}).

\subsection{The tetrahedron}

In Example~\ref{ex:octa} one triple dominates strictly.  The next design
admits no strict domination at all, and holds \eqref{eq:dominate} with
equality at every triple.
...
This is the smallest design at rank three holding \eqref{eq:dominate}
with equality, and it is the equality case of the corank-one
classification: the tie law \eqref{eq:tielaw} of \S\ref{sec:chart} reads
$3\cdot\tfrac14\cdot3=\tfrac94=3-1+\tfrac14$ here.  Splitting its
vectors produces extremal designs at every
size $m\ge4$ and rank $3$, by which route the no-margin principle of
\S\ref{sec:ties} reaches the open cells.  Its volume-sampling mixture,
computed in \S\ref{sec:ties}, is $(x-1)(x-4)^2$, whose least root sits at
the threshold.  It is also the calibration point of the first-order system
of \S\ref{sec:interior}, where the multiplier is $\Lambda=\tfrac13I_3$
with $\cv=1$ and each of the four triples carries multiplier $\tfrac14$.
