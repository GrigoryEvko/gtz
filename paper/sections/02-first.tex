...
\begin{lemma}[\LV]\label{lem:trace}
...
\begin{lemma}[\LV]\label{lem:share}
...
\begin{lemma}[\LV]\label{lem:coparseval}
For every design,
...
\begin{proposition}[\LV]\label{prop:sizerank}
...
\begin{proposition}[\LV]\label{prop:rankone}
...
\begin{proposition}[\LV]\label{prop:classical}
Let $n\ge1$ and let $A\in\FF^{n\times k}$ satisfy $A^{*}A=I_k$.  If
$\Gtz(n,k)$ holds, respectively $\GtzC(n,k)$, then there is an injection
$\rho:\{1,\dots,k\}\to\{1,\dots,n\}$ with
\[
  (A_\rho)^{*}A_\rho^{\vphantom{*}}\;\psd\;\tfrac1n I_k,
  \qquad A_\rho:=\bigl(A_{\rho(1)},\dots,A_{\rho(k)}\bigr)^{*}\!,
\]
...
dominate and let $\rho$ enumerate $C$ in increasing order.  Then
$(A_\rho)^{*}A_\rho^{\vphantom{*}}=\sum_{r\in C}A_r^{\vphantom{*}}A_r^{*}
=\frac1n\SC$, so
\[
  (A_\rho)^{*}A_\rho^{\vphantom{*}}-\tfrac1n I_k
...
\begin{lemma}[\LV]\label{lem:direction}
...
\[
  ...
  \;=\;\tfrac16\cdot3\cdot2\sum_{i=1}^{3}e_i^{\vphantom{T}}e_i^{T}\;=\;I_3 ,
\]
...
$\SC=6\,e_1^{\vphantom{T}}e_1^{T}+3\,e_2^{\vphantom{T}}e_2^{T}$, which is
...
\begin{example}[\LV]\label{ex:tetra}
...
$\sum_c g_c^{\vphantom{T}}g_c^{T}$ is $4$, and each off-diagonal entry is
$1-1-1+1=0$, so
\begin{equation}\label{eq:tetraparseval}
  \sum_{c=0}^{3} g_c^{\vphantom{T}}g_c^{T}\;=\;4I_3,
  \qquad\text{hence}\qquad
  \sum_{c=0}^{3}\tfrac14\,g_c^{\vphantom{T}}g_c^{T}\;=\;I_3 :
\end{equation}
...
\[
  \SC\;=\;4I_3-g_{c_0}^{\vphantom{T}}g_{c_0}^{T} .
\]
The matrix $g_{c_0}^{\vphantom{T}}g_{c_0}^{T}$ has eigenvalue
