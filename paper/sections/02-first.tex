...
arguments use, and settles the cases those identities settle by
themselves.  A lemma and a counterexample then locate the point at which
...
The vectors are not required to be distinct or nonzero, so repeated
vectors and the zero vector are admitted.  Excluding them would change the
problem without simplifying it: a $k$-subset containing two parallel
vectors is rank-deficient and cannot dominate, and that mechanism, seen
first in Example~\ref{ex:octa}, is what makes selection combinatorial
rather than metric.  The weights are real even over $\C$.  They also do
not enter $\SC$, appearing only through \eqref{eq:parseval}, so domination
survives any operation that keeps the vectors and moves the weights.

Since $g_c^{\vphantom{\mathsf{H}}}g_c\adj\psd0$ for every $c$, adding indices to
$C$ can only increase $\SC$ in the Loewner order, and a superset of a
dominating set dominates.  The cardinality restriction
$\lvert C\rvert=k$ therefore carries the problem, and it is
...
\S\ref{sec:chart}, where its inverse defines the pivot of a vector and its
...
\end{equation}
Every design satisfies this necessary condition by a subset found without
any work: by Lemma~\ref{lem:trace} some $\ell_{c_0}\ge k$, and any
$k$-subset containing $c_0$ already satisfies \eqref{eq:tracetest}.  So
the trace test is satisfiable in every design, and it separates no
dominating subset from a non-dominating one.  The design below, which
serves also as a first illustration of the cell $(6,3)$, shows how far
apart the two can be.
...
This is the smallest design at rank three in which \eqref{eq:dominate}
holds with equality, and it is the equality case of the corank-one
classification: the tie law \eqref{eq:tielaw} of \S\ref{sec:chart} reads
$3\cdot\tfrac14\cdot3=\tfrac94=3-1+\tfrac14$ here.  It recurs throughout
what follows.  Splitting its vectors produces extremal designs at every
