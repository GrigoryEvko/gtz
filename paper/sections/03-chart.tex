...
\begin{figure}[ht]
  ...
  $P=VV\adj$ of \eqref{eq:chart} and the gap $W=P-\dsn$ of
...
\begin{equation}\label{eq:dualgram}
  T\;:=\;\sum_c t_c\,w_c^{\vphantom{\mathsf{H}}}w_c\adj .
\end{equation}
...
Every $T\pd0$ admits an invertible $R$ with $R\adj TR=I$.
\end{lemma}

\begin{proof}
Write $\Sdual=LL\adj$ with $L$ invertible, by Cholesky factorization, and put
$R:=(L\adj)^{-1}$.  Then $R\adj TR=L^{-1}LL\adj(L\adj)^{-1}=I$.
\end{proof}
...
$\sum_{c\in E}h_c^{\vphantom{\mathsf{H}}}h_c\adj\psd R\adj TR=I_{m-k}$, which is
