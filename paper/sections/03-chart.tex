...
carried out in these coordinates.
...
\eqref{eq:twoproducts}.  We record the two steps separately: the first
holds at every selection size, the second only at $\lvert C\rvert=k$.
...
minor by minor.  Over the rationals the minors decide the question by
exact arithmetic, with no eigenvalue computation and no genericity
assumption.
...
semidefinite principal submatrix of any nonempty size.  Such matrices
realize the inertia of Lemma~\ref{lem:inertia} at the two smallest cells.
At $(m,k)=(2,1)$ take
\[
...
Proposition~\ref{prop:chartid} it has $W_{cc}=t_c(\ell_c-1)\ge-t_c>-1$ at
every index and $\sum_cW_{cc}=k-1\ge0$, so at least one diagonal entry is
nonnegative.
\end{remark}
