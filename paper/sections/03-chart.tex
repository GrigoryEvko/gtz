...
may be varied independently, on which the compactification of
\S\ref{sec:compactness} depends; and $W[C]\psd0$ is a closed condition
in both arguments.
...
The gap $W$ has a fixed signature, and the signature by itself decides
nothing.
...
$W_{cc}=t_c(\ell_c-1)\ge-t_c>-1$, and they sum to $k-1\ge0$, so at least one of them is nonnegative.
\end{remark}
...
$h_c$ by a common unitary, which changes no domination.  The normalization, by contrast, is forced.
...
The argument is insensitive to the field, which is why the complex
collapse of \S\ref{sec:complex} occurs one size higher and not here.
Part (iv) produces a family of
