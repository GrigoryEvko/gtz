...
proves the criterion, determines what the criterion does and does not
follow from, and settles the two coranks that the criterion reaches:
corank one over both fields, corank two over $\R$.
...
rows are $g_c^{*}$, $c\in C$, and
\[
  \Gram_C\;:=\;M_C^{\vphantom{*}}M_C^{*}
...
\begin{equation}\label{eq:twoproducts}
  M_C^{*}M_C^{\vphantom{*}}\;=\;\sum_{c\in C}g_c^{\vphantom{*}}g_c^{*}
  \;=\;\SC \quad (k\times k),
  \qquad
  M_C^{\vphantom{*}}M_C^{*}\;=\;\Gram_C \quad (r\times r).
\end{equation}
...
\]
and let $V$ be the $m\times k$ matrix whose $c$-th row is $v_c^{*}$; thus
...
\begin{equation}\label{eq:isometry}
  V^{*}V\;=\;\sum_{c}v_c^{\vphantom{*}}v_c^{*}
  \;=\;\sum_c t_c\,g_c^{\vphantom{*}}g_c^{*}\;=\;I_k .
\end{equation}
Accordingly
...
\begin{proposition}[\LV]\label{prop:chartid}
...
\begin{proof}
$P^{*}=(VV^{*})^{*}=VV^{*}=P$, and, using \eqref{eq:isometry} once,
$P^2=V(V^{*}V)V^{*}=VIV^{*}=P$.  A Hermitian idempotent is the orthogonal
projection onto its range, which is the column space of $V$ because
$V^{*}V=I_k$ forces $V$ to have rank $k$.  The entries are
$P_{cd}=\langle v_c,v_d\rangle=\sqrt{t_ct_d}\langle g_c,g_d\rangle$; at
$c=d$ this is $t_c\ell_c=s_c$.  Finally
$\tr P=\tr(VV^{*})=\tr(V^{*}V)=\tr I_k=k$.  The diagonal of $W$ is
$s_c-t_c=t_c(\ell_c-1)$, and $\sum_c(s_c-t_c)=k-1$ by
Lemma~\ref{lem:trace}.
\end{proof}

The construction loses nothing.
...
$P$; then $V^{*}V=I_k$ and $VV^{*}=P$.  Writing $v_c^{*}$ for the $c$-th
row of $V$ and setting $g_c:=t_c^{-1/2}v_c$ produces a family with
$\sum_ct_cg_c^{\vphantom{*}}g_c^{*}=\sum_cv_c^{\vphantom{*}}v_c^{*}
=V^{*}V=I_k$, a design with weight vector $t$ and chart $P$.  If two
...
for a unitary $U$ of $\FF^k$, whence $g'_c=U^{*}g_c$; conversely
replacing $g_c$ by $Ug_c$ replaces $V$ by $VU^{*}$ and leaves
$P=VV^{*}$ unchanged.
\end{proof}
...
where $a_c^{*}$ is the $c$-th row of a matrix $A$ with $A^{*}A=I_k$, the
factor $\sqrt n$ in the vectors cancels the factor $1/n$ in the weights
and $P=AA^{*}$; domination of $C$ reads
$\sum_{c\in C}a_c^{\vphantom{*}}a_c^{*}\psd n^{-1}I_k$, which is the
row-selection problem of \cite{GTZ1997}.  In
that slice $D=n^{-1}I$ and the chart criterion below is the condition
$\lmin(P[C])\ge1/n$ used in \cite{Nesterenko2023,Nesterenko2025}.  What
is new here is the general weight vector, for which $D$ is a genuine
...
\begin{lemma}[\LV]\label{lem:congruence}
...
$X\psd0$ if and only if $B^{*}XB\psd0$: the substitutions $x\mapsto Bx$
and $x\mapsto B^{-1}x$ carry each quadratic form to the other.
\end{proof}

\begin{lemma}[\LV]\label{lem:flip}
...
\[
  ...
  I-XX^{*}\psd0 .
\]
If $M$ is square, then $M^{*}M\psd I$ if and only if $MM^{*}\psd I$.
\end{lemma}

\begin{proof}
The first statement is symmetric in $X$ and $X^{*}$, so one implication
suffices.  Assume $\lVert Xu\rVert\le\lVert u\rVert$ for every $u$, which
is $I-X^{*}X\psd0$.  For any $w$, by Cauchy--Schwarz and then the
hypothesis at $u=X^{*}w$,
\[
  \lVert X^{*}w\rVert^4
  \;=\;\langle w,XX^{*}w\rangle^2
  \;\le\;\lVert w\rVert^2\,\lVert X(X^{*}w)\rVert^2
  \;\le\;\lVert w\rVert^2\,\lVert X^{*}w\rVert^2 .
\]
If $X^{*}w\ne0$ divide by $\lVert X^{*}w\rVert^2$; in either case
$\lVert X^{*}w\rVert\le\lVert w\rVert$, which is $I-XX^{*}\psd0$.

For the second statement let $M$ be $n\times n$ with $M^{*}M\psd I$, that
is $\lVert Mu\rVert\ge\lVert u\rVert$ for every $u$.  Then $Mu=0$ forces
$u=0$, so $M$ is invertible.  Put $X:=M^{-1}$ and substitute $u=Xw$:
$\lVert w\rVert\ge\lVert Xw\rVert$, so $I-X^{*}X\psd0$.  By the first
statement $I-XX^{*}\psd0$, that is
$\lVert (M^{*})^{-1}w\rVert\le\lVert w\rVert$ for every $w$; substituting
$w=M^{*}u$ gives $\lVert u\rVert\le\lVert M^{*}u\rVert$, which is
$MM^{*}\psd I$.  The converse is the same argument with $M$ and $M^{*}$
exchanged.
\end{proof}

\begin{lemma}[\LV]\label{lem:chart}
...
$\SC=M_C^{*}M_C^{\vphantom{*}}\psd I_k$ if and only if
$\Gram_C=M_C^{\vphantom{*}}M_C^{*}\psd I_k$.  By
...
positive diagonal matrix of trace one}.  Two features of this formulation
are used repeatedly.  The weights enter only through $D$, so the
projection and the weights may be varied independently, which is what
makes the compactification of \S\ref{sec:compactness} possible; and
$W[C]\psd0$ is a closed condition in both arguments.
...
\begin{figure}[ht]
  ...
  $P=VV^{*}$ of \eqref{eq:chart} and the gap $W=P-D$ of
...
The hypothesis $\lvert C\rvert=k$ in Lemma~\ref{lem:chart} is not
cosmetic.  Domination is the $k\times k$ statement
$M_C^{*}M_C\psd I_k$, while the block condition is the
$r\times r$ statement $M_C^{\vphantom{*}}M_C^{*}\psd I_r$ with
...
\begin{corollary}[\LV]\label{cor:minor}
...
The factor is strictly positive, so the two matrices $W[C]$ and
$\Gram_C-I$ have the same principal minors up to positive factors and are
semidefinite together, minor by minor.  Over the rationals this is an
exact decision procedure, requiring no eigenvalue computation and no
genericity assumption.

\subsection{Inertia, and what it does not decide}\label{sec:inertia}

The gap $W$ has a fixed signature.  The signature by itself decides
nothing.  What follows locates the feature of the chart that carries the
theorem.

\begin{lemma}[\LV]\label{lem:rayleigh}
...
\begin{remark}[\LV]\label{rem:nogo}
...
$-1$.  Hence no statement of the form ``a Hermitian matrix of inertia
$(k,m-k,0)$ possesses a positive semidefinite $k\times k$ principal
block'' is available, and no proof of $\Gtz$ can run on the signature of
$W$ alone.

Neither matrix is a chart gap, and the obstruction is the diagonal.  By
Proposition~\ref{prop:chartid} the diagonal entries of a chart gap are
$W_{cc}=t_c(\ell_c-1)\ge-t_c>-1$, and they sum to $k-1\ge0$, so at least
one of them is nonnegative.  The projection structure, not the signature,
is what carries the theorem.
\end{remark}
...
operation.  Executed with the correct normalization, this turns a design
of rank $k$ into a design of rank $m-k$ and exchanges domination with
...
\[
  Z^{*}Z=I_r,\qquad ZZ^{*}=I_m-P ,
\]
and write $z_c^{*}$ for the $c$-th row of $Z$.  As in
\eqref{eq:isometry} and Proposition~\ref{prop:chartid},
\[
  \sum_c z_c^{\vphantom{*}}z_c^{*}=Z^{*}Z=I_r,
  \qquad
  \lVert z_c\rVert^2=(ZZ^{*})_{cc}=1-P_{cc}=1-s_c .
\]
...
\begin{equation}\label{eq:kernelframe}
  \sum_c(1-t_c)\,w_c^{\vphantom{*}}w_c^{*}\;=\;I_r .
\end{equation}
...
\begin{proof}
$T\psd0$ as a nonnegative combination of rank-one positive semidefinite
matrices.  If $Tu=0$ then $\langle u,Tu\rangle=\sum_ct_c\lvert\langle
...
\begin{lemma}[\LV]\label{lem:whiten}
Every $T\pd0$ admits an invertible $R$ with $R^{*}TR=I$.
\end{lemma}

\begin{proof}
Write $T=LL^{*}$ with $L$ invertible, by Cholesky factorization, and put
$R:=(L^{*})^{-1}$.  Then $R^{*}TR=L^{-1}LL^{*}(L^{*})^{-1}=I$.
\end{proof}

\begin{theorem}[\LV]\label{thm:duality}
Let $k\ge1$ and $m\ge k+1$, and let $\mathcal D$ be a design at $(m,k)$
over $\FF$ with weight vector $t$.  With $w_c$, $T$ and $R$ as above, the
vectors
\[
  h_c\;:=\;R^{*}w_c\in\FF^{m-k}
\]
with the \emph{same} weights $t_c$ form a design $\mathcal D^{\perp}$ at
$(m,m-k)$, and for every $k$-subset $C$,
\[
  C\ \text{dominates in}\ \mathcal D
  \quad\Longleftrightarrow\quad
  C^{\complement}\ \text{dominates in}\ \mathcal D^{\perp} .
\]
\end{theorem}
...
\[
  \sum_c t_c\,h_c^{\vphantom{*}}h_c^{*}
  \;=\;R^{*}\Bigl(\sum_ct_c\,w_c^{\vphantom{*}}w_c^{*}\Bigr)R
  \;=\;R^{*}TR\;=\;I_{m-k}
\]
by \eqref{eq:dualgram} and Lemma~\ref{lem:whiten}; here the conjugation
identity $R^{*}(ww^{*})R=(R^{*}w)(R^{*}w)^{*}$ is used termwise.  So
$\mathcal D^{\perp}$ is a design at $(m,m-k)$.

Fix $C$ with $\lvert C\rvert=k$ and put $E:=C^{\complement}$, of size
$r=m-k$.  Let $N$ be the $k\times r$ matrix whose rows are $w_c^{*}$,
$c\in C$; equivalently
$N=\diag\bigl((1-t_c)^{-1/2}\bigr)_{c\in C}\,Z_C$, where $Z_C$ is the
submatrix of $Z$ on the rows $C$.  Then, as in \eqref{eq:twoproducts},
\[
  N^{\vphantom{*}}N^{*}=\bigl(\langle w_c,w_d\rangle\bigr)_{c,d\in C},
  \qquad
  N^{*}N^{\vphantom{*}}=\sum_{c\in C}w_c^{\vphantom{*}}w_c^{*} .
\]
Now run the equivalences.  Since $P=I-ZZ^{*}$,
\[
  W[C]\;=\;\diag(1-t_c)_{c\in C}-(ZZ^{*})[C],
\]
and $(ZZ^{*})[C]=Z_C^{\vphantom{*}}Z_C^{*}$.  Conjugating by the
invertible diagonal $\diag\bigl((1-t_c)^{-1/2}\bigr)_{c\in C}$ as in the
proof of Lemma~\ref{lem:congruence},
\[
  W[C]\psd0
  \quad\Longleftrightarrow\quad
  I_k-N^{\vphantom{*}}N^{*}\psd0
  \quad\Longleftrightarrow\quad
  I_r-N^{*}N^{\vphantom{*}}\psd0 ,
\]
the second step by Lemma~\ref{lem:flip} applied to $X:=N^{*}$.  By
\eqref{eq:kernelframe} the right-hand condition reads
\[
  \sum_{c\in C}w_c^{\vphantom{*}}w_c^{*}
  \;\preceq\;\sum_{c}(1-t_c)\,w_c^{\vphantom{*}}w_c^{*} .
\]
Subtracting both sides from $\sum_c w_c^{\vphantom{*}}w_c^{*}$ turns this
into
\[
  \sum_{c\in E}w_c^{\vphantom{*}}w_c^{*}
  \;\psd\;\sum_{c}t_c\,w_c^{\vphantom{*}}w_c^{*}\;=\;T,
\]
and conjugating by the invertible $R$ turns it into
$\sum_{c\in E}h_c^{\vphantom{*}}h_c^{*}\psd R^{*}TR=I_{m-k}$, which is
domination of $E$ in $\mathcal D^{\perp}$.  Every step is an
equivalence, and $\lvert E\rvert=m-k$ is the rank of $\mathcal D^{\perp}$,
so Lemma~\ref{lem:chart} applies at both ends.
\end{proof}

\begin{corollary}[\LV]\label{cor:dualcell}
...
to $\mathcal D^{\perp}$ and complement the resulting subset.  The
...
$h_c$ by a common unitary, which changes no domination.  What is not
free is the normalization.

\begin{remark}\label{rem:naive}
Rescaling the kernel rows by the weights rather than the co-weights ---
that is, taking the dual vectors to be $u_c:=z_c/\sqrt{t_c}$ --- also
produces a design at $(m,m-k)$, since $\sum_ct_cu_c^{\vphantom{*}}u_c^{*}
=\sum_cz_c^{\vphantom{*}}z_c^{*}=I_{m-k}$, and it is the construction one
writes down first.  It does not satisfy the conclusion of
Theorem~\ref{thm:duality}.  At uniform weights the two agree: for
$t_c\equiv1/m$ one has $T=\frac{1}{m-1}I$ by \eqref{eq:kernelframe},
...
$T=\tfrac15\cdot\tfrac14+\tfrac45\cdot4=\tfrac{13}{4}$ and
$h_c^2=w_c^2/T$, that is $h_1^2=\tfrac1{13}$ and $h_2^2=\tfrac{16}{13}$.
Parseval holds, $\tfrac15\cdot\tfrac1{13}+\tfrac45\cdot\tfrac{16}{13}=1$,
and the correspondence is exact: $\{2\}$ dominates in $\mathcal
D^{\perp}$ because $\tfrac{16}{13}\ge1$, matching $\{1\}$ upstairs, and
...
vector $z\in\FF^{m}$, and $P=I-zz^{*}$.  Set
...
\begin{equation}\label{eq:pnorm}
  ...
  \sum_c (1-t_c)\;=\;m-1\;=\;k .
\end{equation}
Thus the $p_c$ have co-weighted mean exactly $1/k$.  Domination is a
rank-one perturbation of a positive diagonal, and the following criterion
converts it into a scalar inequality.

\begin{lemma}[\LV]\label{lem:rankone}
Let $A\pd0$ and let $b$ be a vector, and put $q:=b^{*}A^{-1}b$.  Then
\[
  A-bb^{*}\psd0\iff q\le1,
  \qquad
  A-bb^{*}\pd0\iff q<1 .
...
$\langle x,(A-bb^{*})x\rangle=\langle x,Ax\rangle-\lvert\langle
b,x\rangle\rvert^2$.
...
$\langle x,(A-bb^{*})x\rangle\ge(1-q)\langle x,Ax\rangle$, which is
nonnegative, and strictly positive for $x\ne0$ when $q<1$.

Conversely, evaluate at $x=y$: the form equals $q-q^2=q(1-q)$.  If
$A-bb^{*}\psd0$ this is nonnegative, which forces $q\le1$ since $q\ge0$.
If moreover $q=1$ then $y\ne0$, because $\langle y,Ay\rangle=1$, and the
form vanishes at $y$, so $A-bb^{*}$ is not positive definite.
\end{proof}

\begin{theorem}[\LV]\label{thm:corankone}
...
$W[C]=(I-zz^{*}-D)[C]\psd0$, that is when $A-bb^{*}\psd0$ with
...
\]
It remains to realize $P=I-zz^{*}$ by a design with weight vector $t$,
...
Lemma~\ref{lem:realize}.  Then $V^{*}V=I_k$ and
$VV^{*}=HH^{*}-zz^{*}=I-zz^{*}$, so the design so obtained has weight
vector $t$ and chart $P=I-zz^{*}$, whose diagonal gives
$t_c\ell_c=1-(1-t_c)/k$, that is \eqref{eq:tielaw}.
\end{proof}

Two features of this proof matter later.  The argument is insensitive to
the field, which is why the complex collapse of \S\ref{sec:complex}
occurs one size higher and not here.  And part (iv) produces a family of
designs attaining the threshold with no slack, one over each point of the
open weight simplex; \S\ref{sec:ties} draws the consequence.
...
Corank two is reached from here by one further step.
Corollary~\ref{cor:dualcell} converts it into rank two, and rank two is
Theorem~\ref{thm:ranktwo}; the conversion is carried out in
\S\ref{sec:corank}, after its input has been proved.
