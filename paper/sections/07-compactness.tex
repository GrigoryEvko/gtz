...
well founded on the size alone.  At $k=1$ the second hypothesis is
...
$(x^{C})\tp Wx^{C}<0$, scaled so that $(x^{C})\tp Wx^{C}=-1$.  The common
value lets one mixing parameter serve every selection.  Put
...
along the way.  The theorem adds the minimality of the counterexample.
...
the bracket is positive.  The lift therefore fails wherever the
compressed slack vanishes against a positive bracket, however small
$t_z$ is.
