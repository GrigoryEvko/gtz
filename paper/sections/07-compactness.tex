...
conditions.  The last subsection locates the limit of the method:
...
That formula is the boundary's only singularity, and it does not
...
Let $(P,t)$ be a chart point with $t_z=0$ for some index $z$.  Two identities dispose of it; the diagonal entry $P_{zz}$ decides which
applies, and the dichotomy is exhaustive by the following lemma.
...
the Schur complement of the pivot $P_{zz}$ in $P$.  The computation chooses no basis of that complement and takes no square
root; it performs one division, by the pivot.  Since $P=P'+rr\tp/P_{zz}$ with $r=Pe_z$, one has for every
$x\in\R^m$
\begin{equation}\label{eq:transverse}
  x\tp Wx\;=\;x\tp(P'-D)x+\frac{\langle Pe_z,x\rangle^{2}}{P_{zz}} .
\end{equation}
The upstairs gap form exceeds the deflated one by the rank-one pivot
term; the lift below consumes that excess in full, so it is not slack.
Splitting the probe along the escaping coordinate turns
\eqref{eq:transverse} into the identity that the rest of the section
uses.
...
\eqref{eq:schur} restores the same amount.
...
well founded on the size alone; the rank window is a consequence rather than an extra hypothesis.  At $k=1$ the second hypothesis is
$\Gtz^{\mathrm{ch}}(m-1,0)$, which holds because the empty selection
dominates.  The reduction transports neither margin nor rate nor subsequence, which
keeps it compatible with Corollary~\ref{cor:nomargin}.
...
Theorem~\ref{thm:variational} does not weaken the problem: by
Corollary~\ref{cor:equivalent} the statement about chart points with
positive weights is the problem itself.  It adds the extremality of the
counterexample, which no cheaper device produces.  The mixing of Lemma~\ref{lem:relax} relocates a counterexample into the
interior but destroys whatever distinguished the original point, so it
cannot be applied to a minimizer; and minimizing over the design space
directly fails, since that space is not compact and a minimizing
sequence of designs need not converge to a design
(Example~\ref{ex:escape}).  The minimum exists only on the
compactification, and Corollary~\ref{cor:interiorcex} places it back
inside.
...
Lemma~\ref{lem:delete} and Theorem~\ref{thm:schurstep} , which deflates at $z$, deletes $z$, renormalizes, and lifts a
dominating subset back, is meaningful at any $t_z<1$, and a minimizing-sequence argument would need this composite rather
than the boundary statement: along such a
sequence the weight at the escaping label is positive and merely tends to
zero.  The composite fails at every positive weight, no threshold repairs it,
and a single determinant shows why.
...
positive, however small $t_z$ is.  The lift therefore fails wherever the compressed slack is zero, no
matter how small $t_z$; it remains to exhibit a family on which $\varepsilon$ is frozen at zero.
...
The obstruction lies on the equality locus of \S\ref{sec:ties}.  Shrinking $t_z$ shrinks the deficit, but
...
corank one for every rank, and at rank three for every size.  The refutation follows the pattern of Corollary~\ref{cor:nomargin},
which is why the boundary is discharged at $t_z=0$ by the identity
