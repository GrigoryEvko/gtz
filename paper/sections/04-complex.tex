...
Over $\C$ the problem is solved, and the solution coincides with the
reach of the elementary mechanisms of
\S\ref{sec:first}--\S\ref{sec:chart}: $\GtzC(m,k)$ holds for $k\ge2$ if
and only if $m\le k+1$.  The positive half is assembled in
...
design of size four and rank two by two operations on designs, each raising the size or the rank by one
while preserving a bound on the value.  \S\ref{sec:cvalue} fixes that
value, \S\ref{sec:cseed} gives the seed, \S\ref{sec:cpadding} the
operations, and \S\ref{sec:cladder} the assembly.

\subsection{The value of a design}\label{sec:cvalue}

Once the Boolean question has answer \emph{no}, the object of study is a number, which the operations of
\S\ref{sec:cpadding} propagate.  Fix a design $\dsn$ at $(m,k)$ over $\FF$.  For every $k$-subset
...
At every rank and over either field the value is bounded below, which
limits what the complex counterexamples can achieve.
...
visible in the new one.  The factor $1/(1-w)$ is a genuine loss: for
$b>0$ one has
