...
eigenvalue that any $k$-subset can achieve.  That bound is the quantity
to track, so \S\ref{sec:cvalue} defines it before \S\ref{sec:cseed}
exhibits the seed, \S\ref{sec:cpadding} the operations and
\S\ref{sec:cladder} the assembly.

\subsection{The value of a design}\label{sec:cvalue}

Once the Boolean question is answered \emph{no}, what remains to study is
a number, and the operations of \S\ref{sec:cpadding} propagate it.  Fix a
...
One size lower the behaviour differs, and the two cells are easily
confused: at $m=k$ every design has value bounded away from $1$, and $1$
is approached only in a degenerate limit.
...
no design at all: the assertion holds there for want of anything to
