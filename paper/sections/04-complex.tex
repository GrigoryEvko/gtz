...
propagate.  Fix a design $\dsn$ at $(m,k)$ over $\FF$.  For every $k$-subset
...
phases is the single real number $\operatorname{Re}\beta_{abc}$, entering affinely.
...
$\operatorname{Re}\beta_{abc}=\pm\lvert T\rvert$ is forced to an extreme of its
range; over $\C$ it may sit anywhere in $[-\lvert T\rvert,\lvert
T\rvert]$, and the separating quantity is the phase defect
$\lvert T\rvert^{2}-(\operatorname{Re}\beta_{abc})^{2}=(\operatorname{Im}\beta_{abc})^{2}$,
...
$\operatorname{Re}\beta_{abc}$ is exactly $0$, leaving $\det(\SC-I_3)=-2$.  No
