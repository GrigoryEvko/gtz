...
Over $\C$ the problem has a complete answer, and the answer is that
selection succeeds exactly in the range where the elementary mechanisms of
\S\ref{sec:first}--\S\ref{sec:chart} already reach: $\GtzC(m,k)$ holds for
$k\ge2$ if and only if $m\le k+1$.  The positive half is assembled in
\S\ref{sec:cpositive} from statements already proved, all of which are
insensitive to the field.  The negative half is produced from a single
design of size four and rank two by two operations on designs, each of
which raises the size or the rank by one and each of which controls the
quantity that must be controlled.  That quantity is fixed in
\S\ref{sec:cvalue}, the seed is \S\ref{sec:cseed}, the operations are
\S\ref{sec:cpadding}, and the assembly is \S\ref{sec:cladder}.

\subsection{The value of a design}\label{sec:cvalue}

Once the Boolean question has answer \emph{no}, the object of study is a
number, and that number is what the operations of \S\ref{sec:cpadding}
propagate.  Fix a design $D$ at $(m,k)$ over $\FF$.  For every $k$-subset
$C$ and every $x\in\FF^k$,
\begin{equation}\label{eq:cover}
  x^{*}\SC x\;=\;\sum_{c\in C}\lvert\langle g_c,x\rangle\rvert^{2},
  \qquad\text{so}\qquad
  \SC\psd s\,I_k
  \iff
...
\begin{equation}\label{eq:value}
  \operatorname{val}(D)\;:=\;\max_{\lvert C\rvert=k}\lmin(\SC),
\end{equation}
a maximum over a finite set.  Since $\SC\psd sI_k$ is equivalent to
$\lmin(\SC)\ge s$, the design $D$ admits a dominating $k$-subset if and
only if $\operatorname{val}(D)\ge1$.
We say $D$ \emph{has value at most} $b$ when $\SC-sI_k$ fails to be
positive semidefinite for every $k$-subset $C$ and every $s>b$; this is
$\operatorname{val}(D)\le b$.  Finally set
\[
  \alpha_k(\FF)\;:=\;\inf_{m}\ \inf_{D}\ \operatorname{val}(D),
\]
...
One bound holds at every rank, over either field, and bounds the damage
the complex counterexamples can do.

\begin{proposition}[\LV]\label{prop:rankfloor}
Every weighted design of size $m$ and rank $k$ over $\FF$ admits a
$k$-subset $C$ with $\SC\psd\frac1k I_k$.  Hence $\alpha_k(\FF)\ge1/k$.
\end{proposition}

\begin{proof}
Let $A$ be the $m\times k$ matrix whose $c$-th row is $g_c^{*}$, so that
$(Ax)_c=\langle g_c,x\rangle$, and let $D=\diag(t_1,\dots,t_m)$.
Condition \eqref{eq:parseval} reads $A^{*}DA=I_k$; in particular $A$ has
...
\begin{proposition}[\LV]\label{prop:cpositive}
...
The bound $1$ is attained at the last size at which it holds, and
attained on a set of full dimension in the weights: by
Theorem~\ref{thm:corankone}(iii)--(iv), for every $k\ge1$, every weight
vector $t$ on $k+1$ atoms and over either field there is a design at
$(k+1,k)$ with weights exactly $t$ in which every $k$-subset dominates and
none dominates strictly.  Its chart is $P=I_{k+1}-zz^{*}$ with
$\lvert z_c\rvert^{2}=(1-t_c)/k$, and its leverages obey the tie law
\eqref{eq:tielaw}.

One size lower the behaviour is different, and the two cells are easily
confused.  At $m=k$ the value is bounded away from $1$ at every design and
approaches $1$ only in a degenerate limit.

\begin{proposition}\label{prop:square}
Let $D$ be a design at $(k,k)$ over $\FF$.  Then $\ell_c=1/t_c$ for every
...
$V^{*}V=I_k$, hence unitary, so its rows are orthonormal:
$\langle v_c,v_d\rangle=\delta_{cd}$ with $v_c=\sqrt{t_c}\,g_c$.  Thus
$\ell_c=\lVert v_c\rVert^2/t_c=1/t_c$ and
\[
  S_{\{1,\dots,k\}}\;=\;\sum_c g_c^{\vphantom{*}}g_c^{*}
  \;=\;\sum_c \frac{1}{t_c}\,v_c^{\vphantom{*}}v_c^{*},
\]
...
Everything negative over $\C$ descends from one design.  Let
$\omega:=e^{2\pi i/3}$, so that $1+\omega+\omega^{2}=0$ and
$\omega+\bar\omega=-1$.

\begin{example}[\LV]\label{ex:sic}
Put
...
$\sum_c g_c^{\vphantom{*}}g_c^{*}=4I_2$ and \eqref{eq:parseval} holds at
...
\begin{lemma}[\LV]\label{lem:sicvalue}
...
$g_a^{*},g_b^{*}$, so that $\SC=A^{*}A$ and
\[
  \det\SC=\lvert\det A\rvert^{2}=\det AA^{*}
...
The contrast with $\R$ is exactly here.  Over $\R$ rank two holds at every
size (Theorem~\ref{thm:ranktwo}), so $\alpha_2(\R)=1$; over $\C$ the same
cell has value below $1$ already at size four.  No proof of $\Gtz$ can
therefore be field-agnostic.  The constant $\alpha_{\mathrm{SIC}}$ is
...
arbitrary weights in two steps.  A design with rational weights is
obtained from a uniform design of larger size by merging equal vectors, so
by Lemma~\ref{lem:split}, read backwards, it has the same value and the
bound holds there.  For arbitrary weights, hold the chart $P$ fixed: by
...
by a one-line computation, and each preserves a value bound; the
inequality preserved is displayed in the statement.

\begin{lemma}[\LV]\label{lem:spike}
Let $D$ be a complex weighted design at $(m,k)$ and let $0<w<1$.  Define
$\widehat D$ at $(m+1,k+1)$ by
...
\end{equation}
Then $\widehat D$ is a design, and if $D$ has value at most $b\ge0$ then
$\widehat D$ has value at most $b/(1-w)$.
\end{lemma}
...
\[
  \sum_{c\le m}(1-w)t_c\,
  \frac{1}{1-w}\begin{pmatrix}g_c^{\vphantom{*}}g_c^{*}&0\\0&0\end{pmatrix}
  \;=\;\begin{pmatrix}I_k&0\\0&0\end{pmatrix},
\]
and the last contributes $w\cdot w^{-1}e_{k+1}^{\vphantom{*}}e_{k+1}^{*}$;
the sum is $I_{k+1}$.
...
$(1-w)s>b$, contradicting the hypothesis on $D$.
\end{proof}

The spike is invisible in the flat directions and is the only atom
visible in the new one; that is the whole content of the two cases.  The
factor $1/(1-w)$ is the price, and it is a genuine loss: for $b>0$ one has
$b<b/(1-w)$ for every admissible $w$, so no member of the family attains
the level $b$, though the levels descend to it as $w\to0$.

\begin{lemma}[\LV]\label{lem:split}
Let $D$ be a complex weighted design at $(m,k)$, let $c_0\le m$ and let
$0<f<1$.  Define $\widetilde D$ at $(m+1,k)$ by listing the vector
$g_{c_0}$ twice,
...
\end{equation}
Then $\widetilde D$ is a design, and if $D$ has value at most $b\ge0$ then
so does $\widetilde D$.
\end{lemma}

\begin{proof}
The weights are positive and still sum to one, and in
\eqref{eq:parseval} the term $t_{c_0}g_{c_0}^{\vphantom{*}}g_{c_0}^{*}$ is
replaced by
$f t_{c_0}g_{c_0}^{\vphantom{*}}g_{c_0}^{*}+(1-f)t_{c_0}
g_{c_0}^{\vphantom{*}}g_{c_0}^{*}$, which is the same matrix.
...
semidefinite by the hypothesis on $D$.  If $C$ contains both labels, its
$k$ vectors include two equal ones, hence span a subspace of dimension at
most $k-1$; choosing $x\ne0$ orthogonal to that subspace gives
$x^{*}S_Cx=0$ by \eqref{eq:cover}, while
$x^{*}(sI_k)x=s\lVert x\rVert^{2}>0$.
\end{proof}

\subsection{The ladder}\label{sec:cladder}

\begin{theorem}[\LV]\label{thm:ladder}
...
\begin{theorem}[\LV]\label{thm:complex}
Let $k\ge2$.  Then
\[
...
For $k\ge2$ the equivalence is vacuous below $m=k$: there Parseval admits
no design at all, and the assertion holds because there is nothing to
...
\cite{Nesterenko2026complex}.  At rank three the two ends are far apart,
and the upper one is not the padded seed.  Two designs improve it.

\begin{example}[\LV]\label{ex:trine}
...
$\sum_{j}g_j^{\vphantom{*}}g_j^{*}=\diag(6,0,3)$,
$\sum_{j}g_{3+j}^{\vphantom{*}}g_{3+j}^{*}=\diag(0,6,3)$, the off-diagonal
...
To see the last assertion, and to prepare the rank-three record, we record
the determinant of a triple once.  Let $C=\{a,b,c\}$ and let $A$ be the
$3\times3$ matrix with rows $g_a^{*},g_b^{*},g_c^{*}$.  Then $\SC=A^{*}A$
while the Gram matrix is $\Gram_C=AA^{*}$, with entries
...
\begin{equation}\label{eq:tripledet}
  ...
  \;=\;d^{3}
      ...
      +2\operatorname{Re}T,
  \qquad
  T:=\beta_{ab}\beta_{bc}\beta_{ca}.
\end{equation}
The three moduli form a phase-free budget; the entire dependence on the
phases is the single real number $\operatorname{Re}T$, entering affinely.
...
\[
  T=(2+\omega^{\,j-i})\,\omega^{\,i-j}=2\omega^{\,i-j}+1,
  \qquad \operatorname{Re}T=1+2\cdot(-\tfrac12)=0 .
\]
...
\begin{example}[\LV]\label{ex:hesse}
...
$\sum_c u_c^{\vphantom{*}}u_c^{*}=6I_3$: the family $p$ contributes $1$ to
...
satisfies $T^{3}=(\beta_{ab}\beta_{bc}\beta_{ca})^{3}=-(27/8)^{3}$, so $T$
...
\begin{equation}\label{eq:hessedet}
  \det(\SC-sI_3)\;=\;(3-s)^{3}-\tfrac{27}4(3-s)+2\operatorname{Re}T,
  \qquad \operatorname{Re}T\in\Bigl\{-\tfrac{27}8,\ \tfrac{27}{16}\Bigr\},
\end{equation}
the first alternative occurring exactly when $\det\SC=\tfrac{27}4+2\operatorname{Re}T$
...
$\operatorname{Re}T=\tfrac{27}{16}$ the right-hand side of
...
\end{equation}
and with $\operatorname{Re}T=-\tfrac{27}8$ it is $-s(s-\tfrac92)^{2}$.

\begin{proposition}[\LV]\label{prop:hesse}
...
$\cos3\theta=-\tfrac12$.  On $[0,\pi]$ the solutions are
...
The eigenvalue computation may be replaced by an explicit certificate,
which is what shows positive semidefiniteness at the threshold without
reference to the spectral theorem.  Take the triple $\{u_0,u_1,u_3\}$.
From $1+\omega+\omega^{2}=0$,
\[
  u_0^{\vphantom{*}}u_0^{*}+u_1^{\vphantom{*}}u_1^{*}
  +u_3^{\vphantom{*}}u_3^{*}
  \;=\;\begin{pmatrix}1&0&-1\\0&2&\omega\\-1&\omega^{2}&3\end{pmatrix},
\]
so that $\SC$ is $\tfrac32$ times this matrix.  Writing
$p:=\tfrac32-s$ and $q:=3-s$ and $\operatorname{atom}(v):=vv^{*}$, a
...
\begin{equation}\label{eq:hesseldl}
  ...
  \;=\;q\cdot\operatorname{atom}\bigl(p,0,-\tfrac32\bigr)
      ...
      -\tfrac18h(s)\,E_{33},
\end{equation}
$E_{33}$ being the matrix unit in the last diagonal slot: the entries
other than $(3,3)$ agree as polynomials in $s$, and the residue at $(3,3)$
is
\[
...
Identity \eqref{eq:tripledet} isolates what distinguishes the two fields.
Over $\R$ the entries $\beta_{ij}$ are real, so $T$ is real and
$\operatorname{Re}T=\pm\lvert T\rvert$ is forced to an extreme of its
range; over $\C$ it may sit anywhere in $[-\lvert T\rvert,\lvert
T\rvert]$, and the separating quantity is the phase defect
$\lvert T\rvert^{2}-(\operatorname{Re}T)^{2}=(\operatorname{Im}T)^{2}$,
which vanishes identically for every real configuration.  The trine of
Example~\ref{ex:trine} illustrates the mechanism in its purest form: its
modulus budget $5$ would permit $2\operatorname{Re}T=2\sqrt3$ and hence a
positive determinant at $s=1$, but its phase defect is $3$ and its
$\operatorname{Re}T$ is exactly $0$, leaving $\det(\SC-I_3)=-2$.  No
...
Proposition~\ref{prop:hesse}.  Nothing here closes the gap: every
rank-three statement above is an upper bound, and upper bounds move the
record down without pinning it.  Determining $\alpha_3(\C)$ is
