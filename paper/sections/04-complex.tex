...
Over $\C$ the problem is solved, and the answer stops where the
...
\begin{equation}\label{eq:value}
  F(\dsn)\;:=\;\max_{\lvert C\rvert=k}\lmin(\SC),
\end{equation}
a maximum over a finite set: the \emph{design objective}.  Since
$\SC\psd sI_k$ is equivalent to $\lmin(\SC)\ge s$, the design $\dsn$
admits a dominating $k$-subset if and only if $F(\dsn)\ge1$.
...
$F(\dsn)\le b$.  Finally set
\[
  \alpha_k(\FF)\;:=\;\inf_{m}\ \inf_{\dsn}\ F(\dsn),
\]
...
rank $k$.  Choose $C$ maximizing $\lvert\det A_C\rvert$ among all
$k$-subsets, put $M:=A_C$ and $B:=AM^{-1}$.  Then $A=BM$, so the $c$-th
row of $B$ holds the coefficients expressing $g_c\adj$ in the rows of $M$,
and by Lemma~\ref{lem:maxvolume} the rows of $B$ indexed by $C$ form $I_k$
and $\lvert B_{ci}\rvert\le1$ for every $c$ and $i$.  By
...
realized, so for $k\ge2$ the infimum of $F$ over designs
at $(k,k)$ equals $1$ and is not attained.
\end{proposition}

\begin{proof}
In the chart of \S\ref{sec:chart} the matrix $V$ is square with
$V\adj V=I_k$, hence unitary; then $VV\adj=I_k$ as well, so the rows are
orthonormal too:
...
can therefore be field-agnostic.  Figure~\ref{fig:bloch} draws the design
on the Bloch sphere.

\begin{figure}[ht]
  \centering
  \input{figures/bloch}
  \caption{The design \eqref{eq:sic} on the Bloch sphere.  A line in $\C^{2}$
  is a point of $S^{2}$, and unit vectors obey
  $\lvert\langle u,v\rangle\rvert^{2}=\tfrac12\bigl(1+n_u\cdot n_v\bigr)$.  At
  leverage $2$ the value $\tfrac43$ of Example~\ref{ex:sic} therefore reads
  $n_a\cdot n_b=-\tfrac13$, and the four Bloch vectors are the vertices of a
  regular tetrahedron: all six pairs are congruent, so all six share the
  characteristic polynomial of Lemma~\ref{lem:sicvalue}.  At $k=2$ the argument
  of $\langle g_a,g_b\rangle\langle g_b,g_c\rangle\langle g_c,g_a\rangle$ is
  half the oriented area of the geodesic triangle $n_a n_b n_c$; the hatched
  face has area $\pi$, so that product is purely imaginary.  Over $\R$ the
  Bloch map is the angle-doubling map \eqref{eq:doubleangle} normalized, and
  its image is a great circle.  Three points of a circle bound no area, which
  is the phase defect of Remark~\ref{rem:phase} vanishing over $\R$.}
  \label{fig:bloch}
\end{figure}
...
$(k+1,k+1)$ entry of $S_C-sI_{k+1}$ is $-s$, negative because
$s>b/(1-w)\ge0$, and a positive semidefinite matrix has nonnegative
diagonal.  Suppose instead $m+1\in C$ and put
...
Figure~\ref{fig:padding} draws both operations as block matrices.
...
\begin{figure}[ht]
  \centering
  \input{figures/padding}
  \caption{The two operations of \S\ref{sec:cpadding}, and the ladder they
  compose.  Above: the matrix $A$ of a design has $c$-th row $g_c\adj$ and
  satisfies $A\adj DA=I_k$; the spike borders it with one column and one row,
  the split repeats one row, and in each case the bordered Parseval product is
  the identity of the new rank.  The spike divides the value bound by $1-w$
  and the split leaves it alone, which is the whole cost accounting of
  Theorem~\ref{thm:ladder}.  Below: the cells $(m,k)$ for $2\le k\le6$, the
  shaded band $m\le k+1$ where $\GtzC$ holds by
  Proposition~\ref{prop:cpositive}, and the staircase at $(9,5)$: three spikes
  up the diagonal $m=k+2$, then two splits along $k=5$.  The tabulated
  $w=1-(\tfrac9{10})^{1/e}$ gives $(1-w)^{e}=\tfrac9{10}$ at every rank, so the
  bound after the spikes is $\tfrac{10}9\alpha_{\mathrm{SIC}}$ whatever $k$ is,
  and since $b<b/(1-w)$ for every $b>0$ and every admissible $w$ no member of
  the family attains $b$.}
  \label{fig:padding}
\end{figure}

The constant $\alpha_{\mathrm{SIC}}$ of Lemma~\ref{lem:sicvalue} is
sharp: by \cite{Nesterenko2026complex} no uniform-weight design of rank
two has value below $2-2/\sqrt3$, the bound being attained exactly when
$4\mid m$, at a repeated copy of \eqref{eq:sic}.  The bound passes to
arbitrary weights in two steps.  Split each atom of a rational-weight
design into equally weighted copies: the result is a uniform design of
larger size, and its value is at most the original's by
Lemma~\ref{lem:split}, so the uniform bound passes back to the
rational-weight design.  For arbitrary weights, hold the chart $P$
fixed: by Lemma~\ref{lem:realize} every weight vector $t'$ with positive
entries carries a design with that chart, namely
$g'_c=\sqrt{t_c/t'_c}\,g_c$, and $F$ is a continuous
function of $t'$ there, being a maximum of finitely many least
eigenvalues of matrices depending continuously on $t'$.  Rational weight
vectors are dense in the open simplex, so the bound persists in the
limit.

Hence $\alpha_2(\C)=2-2/\sqrt3$.
...
whose value is at most $\tfrac{10}9\alpha_{\mathrm{SIC}}$; that cell, $(5,3)$, is the
...
bound by Lemma~\ref{lem:sicvalue}, the lower by
\cite{Nesterenko2026complex} extended to arbitrary weights in
\S\ref{sec:cpadding}.  At rank three the two ends are far apart, and two
designs push the upper end below the padded seed.
...
compute the determinant of a triple once and for all.  Let $C=\{a,b,c\}$
and let $A$ be the $3\times3$ matrix with rows
$g_a\adj,g_b\adj,g_c\adj$.  Then $\SC=A\adj A$
...
$\Gram_C-sI_3$ gives, when all three leverages equal $3$,
\begin{equation}\label{eq:tripledet}
  \det(\SC-sI_3)
  \;=\;(3-s)^{3}
      -(3-s)\bigl(\lvert\beta_{ab}\rvert^{2}+\lvert\beta_{bc}\rvert^{2}
...
side and one from the other.  The coordinate swap $e_0\leftrightarrow e_1$
exchanges the two sides, so take the pair on the first axis, say
$g_i,g_j,g_{3+l}$ with $i\ne j$; then
\[
...
\]
By \eqref{eq:tripledet},
$\det(\SC-sI_3)=(3-s)^{3}-5(3-s)=(3-s)\bigl((3-s)^{2}-5\bigr)$, so the
...
Figure~\ref{fig:hesse} draws the twelve lines.

\begin{figure}[ht]
  \centering
  \input{figures/hesse}
  \caption{The nine directions of Example~\ref{ex:hesse}, slot $p$ across and
  phase $q$ down as in the display, and the twelve triples that
  \eqref{eq:hessedet} puts at $\operatorname{Re}\beta_{abc}=-\tfrac{27}{8}$.
  They are the twelve lines of the affine plane of order three on the residues
  $(p,q)$: rows, columns and two diagonal classes closing through the opposite
  edge, with every one of the thirty-six pairs on one of them.  A drawing by
  straight lines in the real plane would then have no ordinary line, which
  Sylvester--Gallai forbids.  The remaining seventy-two sit at
  $\tfrac{27}{16}$, among them the ringed $\{u_0,u_1,u_3\}$ that carries the
  certificate \eqref{eq:hesseldl}.}
  \label{fig:hesse}
\end{figure}
...
so $h_3$ is the least.  The leading coefficient of $h$ is positive, so $h$
is negative below $h_3$ and positive between $h_3$ and the next root.
Exact evaluation now isolates $h_3$: $h$ is negative at
...
direct expansion of both sides gives, for every real $s$, the polynomial
identity
...
lie below $2$, and $h(2)=19>0$ makes the number of roots above $2$ even;
exactly one root is therefore below $2$, which places $2$ strictly
between the two smallest.  As $3-\sqrt5<2$ and $h(3-\sqrt5)>0$, the
number $3-\sqrt5$ lies in the same interval, hence above $h_3$.
...
Example~\ref{ex:trine} isolates the mechanism.  Its three squared moduli
are $3,1,1$, so $\lvert\beta_{abc}\rvert^{2}=3$ and its modulus budget
$5$ would permit $2\operatorname{Re}\beta_{abc}=2\sqrt3$ and hence a
...
either problem: at the icosahedral design of \S\ref{app:icosa} all twenty
triples have the same moduli and exactly ten dominate.
Figure~\ref{fig:phasedefect} draws the law at $s=1$ for both designs.
\end{remark}

\begin{figure}[ht]
  \centering
  \input{figures/phasedefect}
  \caption{Identity \eqref{eq:tripledet} at $s=1$, where it reads
  $\det(\SC-I_3)=8-2B+2\operatorname{Re}\beta_{abc}$ with $B$ the modulus
  budget.  Left: the two lines share the slope $2$ and differ only in $B$, so
  each crosses zero at $\operatorname{Re}\beta_{abc}=B-4$: $\tfrac75$ for the
  icosahedron, marked, and $1$ for the trine.
  A filled dot is a value some triple of the named design takes, an open dot
  an endpoint of the permitted range that it misses.  The trine sits at the
  centre of its interval with $\operatorname{Re}\beta_{abc}=0$, so its budget
  $5$ would have allowed $-2+2\sqrt3>0$ and its phase does not, while the
  icosahedron is real and sits at both ends, ten of its triples landing above
  the zero level.  Right: the disc $\lvert\beta_{abc}\rvert\le\sqrt3$, the
  trine's eighteen mixed triples all on its boundary, the real locus its
  horizontal diameter, the shaded lens the region
  $\operatorname{Re}\beta_{abc}>1$ where the determinant is positive, and the
  phase defect the squared height above the diameter, which is $3$ at
  $\pm i\sqrt3$.  Every abscissa drawn is a genuine triple of leverage-$3$
  vectors, since the leading minors $3$, $9-\lvert\beta_{ab}\rvert^{2}$ and
  $\det\Gram_C=27-3B+2\operatorname{Re}\beta_{abc}$ stay
  positive along both intervals, though the figure does not assert that each
  such triple sits inside a design; the icosahedral split of ten and ten is the
  one count \S\ref{app:icosa} records without a theorem.}
  \label{fig:phasedefect}
\end{figure}
...
Figure~\ref{fig:constants} draws the interval each rank closes.
\end{remark}

\begin{figure}[ht]
  \centering
  \input{figures/constants}
  \caption{The six constants of \S\ref{sec:complex} on one axis.  Below the
  axis, the ledger giving each its exact form and, for three of them, a
  polynomial it is the least root of: the floor
  $1/k$, the Hesse record $h_3$, the trine value $3-\sqrt5$, the seed value
  $\alpha_{\mathrm{SIC}}$, the padded bound
  $\tfrac{10}9\alpha_{\mathrm{SIC}}$, and the threshold $1$.  Above it each
  rank carries the interval its two ends close, a filled dot marking an end
  attained by a design and an arrowhead an end running to $0$ with $1/k$.
  Rank two over $\C$
  is a point, while rank three spans $[\tfrac13,h_3]$ and stays open: the light
  arrow on that row is the record of Remark~\ref{rem:sharpc} moving down
  through $\tfrac{10}9\alpha_{\mathrm{SIC}}$ and $3-\sqrt5$ without pinning
  anything.  The lower end that pins $\alpha_2(\C)$ is
  \cite{Nesterenko2026complex} at uniform weights carried to arbitrary weights
  in \S\ref{sec:cpadding}, and the top row's $\alpha_k(\R)=1$ is the
  conjecture; what is proved on that row is the bracket $[1/k,1]$ together with
  a design whose value is exactly $1$.}
  \label{fig:constants}
\end{figure}
