...
The inequality \eqref{eq:dominate} is an equality on a large set, and how
large that set is governs what a proof of the conjecture may look like.
This section constructs the set at every size, recalls its classification
at corank one, and computes its dimension: it meets every point of the
open weight simplex.  The consequence, used throughout the rest of the
paper, is that no argument for $\Gtz$ can carry a margin.

A second theme runs alongside.  Volume sampling is the probability
measure on $k$-subsets adapted to \eqref{eq:parseval}, and under it the
selection inequality holds on average, by a factor growing with the rank.
Turning that average into a single subset is the derandomization the
problem asks for; \S\ref{sec:averaging} says exactly how much of it the
identities give and where the gap begins.
...
The quantity in Lemma~\ref{lem:tiemax} will recur; for $m\ge k$ put
...
occur at every weight vector; the next subsection extends them to
every larger size.

\subsection{Splitting, and existence at every size}\label{sec:split}

The construction uses that $\SC$ does not see the weights, being by
\eqref{eq:dominate} the sum of the rank-one matrices attached to the
selected vectors, and it uses the \emph{gap form} behind
Lemma~\ref{lem:parallel}: for
...
Corollary~\ref{cor:nomargin} constrains what a proof of the conjecture
can look like.
...
The exactness, finally, occupies a set of full dimension.  By
...
\S\ref{sec:interior} supply it.  Appendix~\ref{app:graveyard} collects the strategies that fail for
these reasons, together with their refuting witnesses.
...
as Lemma~\ref{lem:trace} requires.  The substitution $y=Rx$, a bijection of $\R^3$, gives for every $3$-subset $C$
\[
...
otherwise; and $C$ dominates if and only if $Q_C\ge0$ on $\R^3$.
...
\eqref{eq:parseval}, and under it the selection inequality holds on
average.  We work
...
asks for a single $C$.  The average dominates with a margin that grows with the rank.
