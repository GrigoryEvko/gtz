...
unitary $U=V\adj V'$.  Replacing $z$ by $\diag(\varepsilon)z$ replaces $P$
by $\diag(\varepsilon)P\diag(\varepsilon)\adj$, hence $V$ by
$\diag(\varepsilon)V$, hence $g_c=v_c/\sqrt{t_c}$ by
$\varepsilon_cg_c$; and $g_c^{\vphantom{\mathsf{H}}}g_c\adj$ is invariant under
...
Apply the lemma with $X=V\adj$ and $Y=V$.  Each term is
$\det\bigl((V\adj)^{C}\bigr)\det(V_C)=\lvert\det V_C\rvert^{2}
...
$V\adj D_\sigma V=V\adj V+(\sigma-1)v_c^{\vphantom{\mathsf{H}}}v_c\adj$ is a
rank-one perturbation of the identity, with
\[
  \det\bigl(V\adj D_\sigma V\bigr)\;=\;1+(\sigma-1)\lVert v_c\rVert^{2}
  \;=\;1+(\sigma-1)s_c .
\]
On the other hand Lemma~\ref{lem:cauchybinet} applied to
$X=V\adj D_\sigma$ and $Y=V$ gives, since scaling the column $e$ of
$V\adj$ multiplies every minor containing it by that factor,
\[
  \det\bigl(V\adj D_\sigma V\bigr)
