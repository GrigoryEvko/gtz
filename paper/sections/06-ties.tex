...
The inequality \eqref{eq:dominate} is an equality on a large set.  This
section constructs that set at every size, recalls its classification at
corank one, computes its dimension, and draws the consequence which
governs the rest of the paper.  It then examines the one probability
measure on $k$-subsets adapted to \eqref{eq:parseval}, under which the
selection inequality holds on average with room to spare, and states
precisely what that does and does not give.
...
dominates strictly: there is $C$ with $\SC\psd I_k$, and there is no $C$
with $\SC\pd I_k$.
\end{definition}

For a Hermitian matrix $A$ one has $A\psd I_k$ exactly when
$\lmin(A)\ge1$, and $A\pd I_k$ exactly when $\lmin(A)>1$.  The definition
...
dominating $k$-subset has $\lmin(\SC)=1$ exactly, so that $\SC-I_k$ is
positive semidefinite and singular.
\end{lemma}

\begin{proof}
The maximum is at least $1$ precisely when some $C$ dominates, and exceeds
$1$ precisely when some $C$ dominates strictly.  A dominating $C$ has
$\lmin(\SC)\ge1$, and the maximum being $1$ forces equality.
\end{proof}

It is convenient to name the quantity Lemma~\ref{lem:tiemax} pins.  For
$m\ge k$ put
\begin{equation}\label{eq:alpha}
  \alpha(m,k)\;:=\;\inf_{D}\ \max_{\lvert C\rvert=k}\lmin(\SC),
\end{equation}
...
size.  By construction $\alpha_k(\FF)=\inf_{m\ge k}\alpha(m,k)$, so the
...
the unique $k$-subset satisfies
\[
  \SC-I_k\;=\;\sum_{c}(1-t_c)\,g_c^{\vphantom{*}}g_c^{*}\;\pd\;0 ,
\]
because the $g_c$ are then a basis of $\FF^k$ and $m\ge2$ forces $t_c<1$
for every $c$.  So no design at $m=k$, $k\ge2$, is extremal.  Extremal
designs first occur at corank one, where by Theorem~\ref{thm:corankone}(iii)--(iv) they
occur at every weight vector; the content of the next subsection is that
they then occur at every larger size as well.

\subsection{Splitting, and existence at every size}\label{sec:split}

Two elementary facts drive the construction.  The first is that $\SC$ does
not see the weights: by \eqref{eq:dominate} it is the sum of the rank-one
matrices attached to the selected vectors, and nothing else.  The second
is the identity behind Lemma~\ref{lem:parallel}, the \emph{gap form}: for
every $k$-subset $C$ and every $x\in\FF^k$,
\begin{equation}\label{eq:gapform}
  x^{*}\bigl(\SC-I_k\bigr)x
...
$g_{c'}=\rho\,g_c$ for some scalar $\rho$.  Then $\SC$ is singular and $C$
does not dominate.
\end{lemma}
...
Now let $D$ be a design at $(m,k)$, let $e$ be an index and let
...
\[
  a\,g_e^{\vphantom{*}}g_e^{*}+(t_e-a)\,g_e^{\vphantom{*}}g_e^{*}
  \;=\;t_e\,g_e^{\vphantom{*}}g_e^{*},
\]
...
these directly, which is shorter and displays the reweighting.

\begin{theorem}[\LV]\label{thm:tieseverywhere}
...
\[
  \sum_{c=1}^{m}t_c\,g_c^{\vphantom{*}}g_c^{*}
  \;=\;\sum_{j=1}^{k+1}\Bigl(\sum_{\varphi(c)=j}t_c\Bigr)
        h_j^{\vphantom{*}}h_j^{*}
  \;=\;\sum_{j=1}^{k+1}T_j\,h_j^{\vphantom{*}}h_j^{*}\;=\;I_k,
\]
...
\[
  \SC\;=\;\sum_{c\in C}h_{\varphi(c)}^{\vphantom{*}}h_{\varphi(c)}^{*}
      \;=\;S_{\varphi(C)},
\]
...
For fixed $\varphi$ the assignment $t\mapsto D(t,\varphi)$ of
...
\]
a transformation under which no $\SC$ changes.  Consequently the extremal
locus at corank one has dimension exactly $m-1=k$ modulo that group, and
...
extremal if and only if its chart projection is $P=I-zz^{*}$ with
...
$\sum_ct_cp_c$; a family none of whose members exceeds its own weighted
mean is constant, and by \eqref{eq:pnorm} the constant is $1/k$, which is
the stated condition on $z$.  These moduli determine
$z$ up to the individual phases $\varepsilon_c$, and $P$ determines $V$ up
to $V\mapsto VU$, since two isometries with the same range differ by the
unitary $U=V^{*}V'$.  Replacing $z$ by $\diag(\varepsilon)z$ replaces $P$
by $\diag(\varepsilon)P\diag(\varepsilon)^{*}$, hence $V$ by
$\diag(\varepsilon)V$, hence $g_c=v_c/\sqrt{t_c}$ by
$\varepsilon_cg_c$; and $g_c^{\vphantom{*}}g_c^{*}$ is invariant under
that.  So the fibre of the weight map over each $t$ is a single orbit, the
quotient is mapped bijectively onto the open simplex, and the dimension is
$m-1=k$.
\end{proof}

The corank-one classification is not the beginning of a pattern.  The
equations \eqref{eq:tielaw} which cut it out have no solutions above
corank one at all.

\begin{proposition}[\LV]\label{prop:tielawcorank}
...
system, and indeed the designs of Theorem~\ref{thm:tieseverywhere} violate
\eqref{eq:tielaw} at most of their atoms.  The following instances are
used again in \S\ref{sec:interior} and in Appendix~\ref{app:data}.

\begin{example}[\LV]\label{ex:split}
...
\[
  S_T\;=\;4I_3-h_j^{\vphantom{T}}h_j^{T},
  \qquad
  x^{T}(S_T-I_3)x\;=\;3\lVert x\rVert^{2}-\langle h_j,x\rangle^{2},
\]
...
\begin{corollary}[\LV]\label{cor:nomargin}
...
\begin{proof}
Let $D$ be the extremal design of Theorem~\ref{thm:tieseverywhere} with
...
Corollary~\ref{cor:nomargin} constrains the form of any solution, in three
ways.
...
function of the design data --- is therefore refuted at the outset,
whatever its degree or complexity.
...
weight, the failure being located exactly on the extremal locus.

And the exactness occupies a set of full dimension.  By
...
therefore be first-order; this is what \S\ref{sec:compactness} and
\S\ref{sec:interior} supply.  The strategies which fail for these reasons,
and for no other, are collected with their refuting witnesses in
...
carries a pair of equal vectors, and takes at most $k+1$ distinct values.
Neither feature is forced.  We exhibit an extremal design at $(5,3)$ whose
five vectors are pairwise non-parallel.  It is described by a graph, and
its data are exactly rational even though its vectors are not.

Let $\Gamma$ be the complete graph on the vertices $1,2,3,4$ with the edge
...
\begin{equation}\label{eq:laplacian}
  L\;:=\;\sum_{e=0}^{4}c_e\,b_e^{\vphantom{T}}b_e^{T}
  \;=\;\begin{pmatrix} 8&-2&-3\\ -2&8&-3\\ -3&-3&6\end{pmatrix},
  \qquad
  y^{T}Ly\;=\;\sum_{e}c_e\,d_e(y)^{2}.
\end{equation}
Its leading principal minors are $8$, $60$ and $\det L=180$, so $L\pd0$.
Choose any $R$ with $R^{T}LR=I_3$ --- for instance $R=L^{-1/2}$ --- and
put
\begin{equation}\label{eq:diamondatoms}
  g_e\;:=\;\sqrt{5c_e}\;R^{T}b_e \qquad (0\le e\le 4).
\end{equation}
Then
\[
  \sum_{e}t_e\,g_e^{\vphantom{T}}g_e^{T}
  \;=\;\sum_e \tfrac15\cdot 5c_e\,R^{T}b_e^{\vphantom{T}}b_e^{T}R
  \;=\;R^{T}LR\;=\;I_3,
\]
so $(g_e,t_e)$ is a design at $(5,3)$; call it $D_\Diamond$.  It is
determined by the graph data up to the ambiguity $R\mapsto RU$ with $U$
orthogonal, which replaces every $g_e$ by $U^{T}g_e$ and changes no $\SC$.

Two computations describe it.  Since $RR^{T}=L^{-1}$,
\begin{equation}\label{eq:diamondlev}
  \ell_e\;=\;5c_e\,b_e^{T}L^{-1}b_e ,
  \qquad
  L^{-1}\;=\;\frac1{180}
  \begin{pmatrix}39&21&30\\ 21&39&30\\ 30&30&60\end{pmatrix},
\end{equation}
whence $b_0^{T}L^{-1}b_0=\tfrac{36}{180}=\tfrac15$ and
$b_e^{T}L^{-1}b_e=\tfrac{39}{180}=\tfrac{13}{60}$ for $e\ge1$, so that
\[
  \ell_0=2,\qquad \ell_1=\ell_2=\ell_3=\ell_4=\tfrac{13}4 .
\]
As a check, $\sum_et_e\ell_e=\tfrac15\bigl(2+4\cdot\tfrac{13}4\bigr)=3=k$,
as Lemma~\ref{lem:trace} requires.  Second, the substitution $y=Rx$, a
bijection of $\R^3$, gives for every $3$-subset $C$
\[
  x^{T}\SC x=\sum_{e\in C}5c_e\,d_e(y)^{2},
  \qquad
  x^{T}x=x^{T}R^{T}LRx=\sum_{e}c_e\,d_e(y)^{2},
\]
so that domination becomes a statement about potentials alone:
\begin{equation}\label{eq:diamondgap}
  x^{T}\bigl(\SC-I_3\bigr)x\;=\;Q_C(y),
  \qquad
  Q_C(y):=\sum_{e=0}^{4}\bigl(5\cdot[e\in C]-1\bigr)c_e\,d_e(y)^{2},
\end{equation}
where $[\,\cdot\,]$ is $1$ when the enclosed statement holds and $0$
otherwise; and $C$ dominates if and only if $Q_C\ge0$ on $\R^3$.  The whitening $R$
has disappeared.

\begin{example}[\LV]\label{ex:diamond}
The design $D_\Diamond$ is extremal at $(5,3)$; its five vectors are
...
\end{equation}
So $C$ dominates.  The form \eqref{eq:diamondstar} vanishes at
$y=(1,4,0)$, so it is singular and $C$ does not dominate strictly.

There are ten $3$-subsets: eight spanning trees of $\Gamma$, and its two
...
requires a separate treatment of $D_\Diamond$, and of every extremal
...
\begin{figure}[ht]
  ...
  \caption{The diamond $D_\Diamond$.  Left: the graph $K_4$ with the edge
...
One probability measure on $k$-subsets is adapted to \eqref{eq:parseval}
exactly, and under it the selection inequality holds on average.  We work
in the chart of \S\ref{sec:chart}: $v_c=\sqrt{t_c}\,g_c$, the matrix $V$
has rows $v_c^{*}$ and satisfies $V^{*}V=I_k$, and $P=VV^{*}$ by
\eqref{eq:chart}.  For a $k$-subset $C$ write $V_C$ for the $k\times k$
submatrix of $V$ on the rows $C$, so that $P[C]=V_C^{\vphantom{*}}V_C^{*}$.
...
Apply the lemma with $X=V^{*}$ and $Y=V$.  Each term is
$\det\bigl((V^{*})^{C}\bigr)\det(V_C)=\lvert\det V_C\rvert^{2}
=\det\bigl(V_C^{\vphantom{*}}V_C^{*}\bigr)=\det P[C]$, while the left side
is $\det(V^{*}V)=1$.  Setting
...
\begin{proof}
Fix $c$ and let $D_\sigma:=\diag(1,\dots,\sigma,\dots,1)$ carry $\sigma$
in the position $c$.  On one hand
$V^{*}D_\sigma V=V^{*}V+(\sigma-1)v_c^{\vphantom{*}}v_c^{*}$ is a
rank-one perturbation of the identity, with
\[
  \det\bigl(V^{*}D_\sigma V\bigr)\;=\;1+(\sigma-1)\lVert v_c\rVert^{2}
  \;=\;1+(\sigma-1)s_c .
\]
On the other hand Lemma~\ref{lem:cauchybinet} applied to
$X=V^{*}D_\sigma$ and $Y=V$ gives, since scaling the column $e$ of
$V^{*}$ multiplies every minor containing it by that factor,
\[
  \det\bigl(V^{*}D_\sigma V\bigr)
...
\begin{theorem}[\LV]\label{thm:leverageaverage}
For every design over either field,
\[
  \sum_{c}s_c\,g_c^{\vphantom{*}}g_c^{*}\;\psd\;I_k .
\]
\end{theorem}

\begin{proof}
Let $x\in\FF^k$ be arbitrary.  Cauchy--Schwarz at each atom gives
...
below by $\bigl(\sum_ct_c\lvert\langle g_c,x\rangle\rvert^{2}\bigr)^{2}$,
which is $\lVert x\rVert^{4}$ by \eqref{eq:parseval}.  Dividing by
$\lVert x\rVert^{2}>0$ gives
$x^{*}\bigl(\sum_cs_cg_c^{\vphantom{*}}g_c^{*}\bigr)x\ge\lVert x\rVert^2$.
Every step is homogeneous in $x$, so no normalization is used.
\end{proof}
...
\[
  ...
  g_c^{\vphantom{*}}g_c^{*}
  \;=\;\sum_{c}\Bigl(\sum_{C\ni c}\pi_C\Bigr)g_c^{\vphantom{*}}g_c^{*}
  \;=\;\sum_c s_c\,g_c^{\vphantom{*}}g_c^{*} ,
\]
...
asks for a single $C$.  The average dominates with room, and the room
grows with the rank.

\begin{proposition}[\LV]\label{prop:jensen}
...
trace.  No single subset need clear it at all: that is
Corollary~\ref{cor:nomargin}.

The measure sees the extremal locus through the mixture of characteristic
polynomials
...
\begin{proposition}[\LV]\label{prop:qonezero}
...
exactly at the threshold.  Meanwhile
$\E_\pi[\SC]=\tfrac34\sum_cg_c^{\vphantom{T}}g_c^{T}=3I_3$ while
$\max_C\lmin(\SC)=1$: the average clears the threshold by a factor of
three, and no triple clears it at all.

\begin{remark}\label{rem:interlace}
Only the identities above are used, and three statements which do not
follow from them should be separated off.  First, $q$ is not asserted to
