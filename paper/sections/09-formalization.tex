...
The ledger is a list, and its coverage
...
elaborated, but its axiom use is not certified by the ledger.  The development also uses classical logic freely; nothing in it is a
constructive claim.
...
Two steps of the argument are not formal, both in
\S\ref{sec:compactness}--\S\ref{sec:interior}, and they are of
different kinds.
...
inclusion rather than an equality, is \cite{Clarke1990}.  None of it is in the ambient library, so \S\ref{sec:interior} takes the
system as its hypothesis and formalizes its consequences
