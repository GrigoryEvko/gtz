...
several.  Eleven tags are not discharged in full, and \S\ref{sec:boundaries}
records which.
...
The design structure and the elementary theory are written once for scalars
in an \texttt{RCLike} field with real weights, and $\R$ and $\C$ are
...
Corollary~\ref{cor:corank}     & \texttt{Gtz.gtzWeighted\_of\_le\_five},
                                 \texttt{Gtz.gtzWeighted\_corank\_two} \\
...
Eleven tags are narrower than the statement they head.  Seven are one
mechanism: the statement is printed over $\FF$, or over both fields, and the
formal statement is over $\R$.  The field-generic layer is the design
structure, the trace identity, the chart of \S\ref{sec:chart} with its
congruence and its two Rayleigh halves, the weak form of
Lemma~\ref{lem:rankone}, the co-Parseval operator,
Proposition~\ref{prop:sizerank} and Theorem~\ref{thm:corankone}.  The seven lie
outside it: the spanning assertion of Lemma~\ref{lem:span}, the complex half of
Proposition~\ref{prop:classical}, Theorem~\ref{thm:duality},
Corollary~\ref{cor:dualcell}, Theorem~\ref{thm:leverageaverage},
Proposition~\ref{prop:rankfloor}, and Theorem~\ref{thm:tieseverywhere} over
$\C$ above size $k+1$.

Proposition~\ref{prop:rankfloor} is formalized as
\texttt{Gtz.gtzWeightedFloor\_inv\_rank}, and its proof does not consult the
field, so its complex case is a transcription and not a mathematical gap.

No complex Naimark duality is stated anywhere in the development.

The other four are gaps inside one field.
\begin{enumerate}
\item[\rm(i)] In Proposition~\ref{prop:tielawcorank} the per-atom
  equivalence is formal, as\\
  \texttt{Gtz.jensenRatio\_eq\_inv\_rank\_iff\_leverageIdentity},\\
  and the summation that concludes $m=k+1$ is not.
\item[\rm(ii)] In Proposition~\ref{prop:jensen} the inequality is formal, as\\
  \texttt{Gtz.sq\_rank\_le\_sum\_weight\_mul\_leverage\_sq},\\
  and the equality case, $\ell_c=k$ throughout, is not stated.
\item[\rm(iii)] In Lemma~\ref{lem:overlap} the covering asserted in~(iv) is
  formal, as\\
  \texttt{Gtz.exists\_mem\_activeSubset\_of\_isQuadricStationaryData},\\
  and the refinement that the covering index carries $\lambda_i>0$ and
  $\langle u_i,g_c\rangle\ne0$ is not.
\item[\rm(iv)] In Lemma~\ref{lem:defect} the two consequences drawn from the
