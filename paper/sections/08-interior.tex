...
\begin{proof}
Let $(H,a)\in\mathcal{S}^k\times\R$ be given.  Choose $\dot t$ with
$\sum_c\dot t_c=a$, put
$H':=H-\sum_c\dot t_c\,g_c^{\vphantom{\mathsf{T}}}g_c\tp$, and take
...
\[
  ...
  =\tfrac12H'\sum_c t_c\,g_c^{\vphantom{\mathsf{T}}}g_c\tp
   +\tfrac12\Bigl(\sum_c t_c\,g_c^{\vphantom{\mathsf{T}}}g_c\tp\Bigr)H'
  =H' ,
\]
so $\Phi'[\dot g,\dot t]=H$ and $\sigma'[\dot t]=a$.  The dimension count
...
impose no condition: for $\Theta\in\partial\lmin(\SC)$ one has
...
\item[\rm(ii)] $t_cv\le1$ for every $c$, and hence $\cv\le m$;
...
$t_cv=\tfrac14\le1$ and $\cv=1\le4=m$.  Part (iii) of that lemma is not
...
Then $t_cv=1$ for every $c$; hence $\cv=m$ and $t_c=1/m$ for every $c$.
