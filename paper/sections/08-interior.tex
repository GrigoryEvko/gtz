...
\S\ref{sec:compactness} upgrades a failure of $\Gtz(m,k)$ to a
minimizing failure: not merely some design below the threshold, but one
minimizing the chart objective.  What does minimality cost such a design?
The constraint
set is a smooth manifold, which gives the question a tangent space to be
answered in.  The objective is a maximum of least-eigenvalue functions,
locally Lipschitz but in general not differentiable, so the answer
comes from nonsmooth first-order theory rather than from a gradient.  The
two together yield one vector identity and one scalar identity at each
index.

A design at $(m,k)$ over $\R$ is written as a pair $(g,t)$ with
$g=(g_1,\dots,g_m)\in(\R^k)^m$ and $t=(t_1,\dots,t_m)$.  In these
coordinates the design objective \eqref{eq:value} reads
...
$\lmin(\SC)\ge1$, so $\Gtz(m,k)$ asserts $F\ge1$ at every design and a
failure of it is a design with $F<1$.
...
The cell $(m,k)$ \emph{has a minimizing counterexample} if $F$ attains a
...
$\Gtz(m-1,k-1)$, Corollary~\ref{cor:interiorcex} places all of that
locus, within the compactification of \S\ref{sec:compactness}, strictly
inside the open weight simplex.

Whether the infimum of $F$ over that locus is attained, rather than
approached at the boundary of the compactification where by
Theorem~\ref{thm:boundary} some subset dominates, is not automatic.
Theorem~\ref{thm:variational} settles it for the chart objective $G$, and
by Remark~\ref{rem:objectives} $G$ and $F$ agree in sign but are related
by a congruence which is not a scalar, so a minimizer of one need not
minimize the other.  Definition~\ref{def:extremalcex} names what the
passage requires, and Theorem~\ref{thm:rankthree} carries it as a
hypothesis; nothing else in the paper does.
...
open condition $t_c>0$ at every index.  Both maps are polynomial, with
differentials
...
$\sum_c\dot t_c=a$, put
$Y':=Y-\sum_c\dot t_c\,g_c^{\vphantom{\mathsf{T}}}g_c\tp$, again
symmetric, and take
...
A local minimum of $F$ ties the vectors to the weights through a system in
an index set $A$, $k$-subsets $C_i$ ($i\in A$), multipliers
$\lambda_i\ge0$ summing to one, unit vectors $u_i$ with
$S_{C_i}u_i=\cv u_i$ at the value $\cv=F(g,t)$, a symmetric matrix
$\Lambda$ and a scalar $\mu$:
\begin{align}
...
\end{align}
Every constant in the system can be displayed at the tetrahedron of
...
\[
  \Lambda\;:=\;\tfrac1{12}\sum_ig_i^{\vphantom{\mathsf{T}}}g_i\tp
  \;=\;\tfrac1{12}\cdot4I_3\;=\;\tfrac13I_3 ,
\]
using $\sum_ig_i^{\vphantom{\mathsf{T}}}g_i\tp=4I_3$, which is
\eqref{eq:parseval} at $t_i=\tfrac14$.

The multipliers are nonnegative and sum to one.  Each $u_i$ is a unit
vector, and it is the tight eigenvector:
$\langle g_i,u_i\rangle=3/\sqrt3=\sqrt3$, so
$S_{C_i}u_i=4u_i-\sqrt3\,g_i=4u_i-3u_i=u_i$, the eigenvalue being
$\cv=1$.  Condition \eqref{eq:statt} holds with $\mu=1$, since
$g_c\tp\Lambda g_c=\ell_c/3=1$ at every index.  For \eqref{eq:statg}, the
triples containing $c$ are the three with $i\ne c$, and for those
...
\[
  ...
  \;=\;t_c\Lambda g_c .
\]
Pairing that identity with $g_c$ replaces each $u_i$ by
$\langle u_i,g_c\rangle=-1/\sqrt3$ and leaves
$3\cdot\tfrac14\cdot\tfrac13=\tfrac14$, which is $t_c\cv$.  The scalar
$\mu$ of \eqref{eq:statt}, the eigenvalue $\cv$ and $\tr\Lambda$ are all
$1$ here.
\end{example}

By Theorem~\ref{thm:corankone}(ii) every design at $(4,3)$ has $F\ge1$,
and $F=1$ at the tetrahedron, so the datum sits at a minimum.
Theorem~\ref{thm:firstorder} produces one at every local minimum of $F$.

\subsection{The first-order system}\label{sec:kkt}

The objective is a maximum of finitely many least-eigenvalue functions of
matrices depending polynomially on $g$.  It is locally Lipschitz, and
where a least eigenvalue is multiple it is neither differentiable nor
Clarke regular: each $\lmin(\SC)$ is a \emph{concave} function of $\SC$,
smooth at a simple least eigenvalue and nonsmooth, hence not regular, at
a multiple one.  For a maximum of functions that are not regular the
subdifferential is contained in, and in general not equal to, the convex
hull of the subdifferentials of the active pieces
\cite[Prop.~2.3.12]{Clarke1990}.  The inclusion runs in the direction a
necessary condition needs, and nothing below uses more than the
inclusion.
...
\end{equation}
\cite[\S2]{Overton1988},
\cite[Vol.~I, Ch.~VI]{HiriartUrrutyLemarechal1993}.  Composing with a
smooth map contracts the subdifferential into the image of the adjoint
derivative \cite[Thm.~2.3.10]{Clarke1990}.  Applied to
...
Figure~\ref{fig:spectraplex} draws the spectraplex at $k=2$.

\begin{figure}[ht]
  \centering
  \input{figures/spectraplex}
  \caption{The spectraplex \eqref{eq:spectraplex} at $k=2$.  Trace one cuts
  the cone in a horizontal disc of radius $\tfrac12$, whose boundary circle
  meets each extreme ray once and consists of the rank-one projectors.  Since
  $\lmin$ is concave, \eqref{eq:spectraplex} is a superdifferential; it is the
  face of that disc spanned by the unit eigenvectors at $\lmin(A)$.  The face
  at an eigenspace of dimension $r$ has dimension $\tfrac12r(r+1)-1$, so none
  has dimension one, and the multiplier attached to an active subset in
  Theorem~\ref{thm:firstorder} is a positive semidefinite matrix of trace one
  on the eigenspace rather than a scalar.  Neither the spectraplex nor the
  passage of \S\ref{sec:kkt} from a local minimum to
  \eqref{eq:statg}--\eqref{eq:statt} is formal (\S\ref{sec:boundaries}).}
  \label{fig:spectraplex}
\end{figure}
...
$S_{C_i}u_i=\cv u_i$, a symmetric matrix $\Lambda$ and a scalar $\mu$
satisfying \eqref{eq:statg} and \eqref{eq:statt}.
\end{theorem}
...
sum to one, and \eqref{eq:zeta}, with the two summations exchanged so that
the outer one runs over the indices $c$, becomes
...
The maximum rule in \eqref{eq:zeta} is only an inclusion; the sharper
first-order statement available at such a point is the directional one,
namely
...
\cite{HiriartUrrutyYe1995,Danskin1967}.  Nothing below uses more than the
multiplier form.  The infinitesimal rotations \eqref{eq:orbit} impose no
condition: for $\Theta\in\partial\lmin(\SC)$ one has
...
Figure~\ref{fig:kktassembly} assembles the five rules and marks where the
development begins.

\begin{figure}[ht]
  \centering
  \input{figures/kktassembly}
  \caption{The assembly of \eqref{eq:statg}--\eqref{eq:statt}.  Left: the
  passage from a local minimum of $F$ to the system in five steps, each arrow
  labelled by the rules that license it; the last carries no label, being the
  comparison of coefficients.  Right: the rules.  Two give inclusions rather
  than equalities, (2) and (3), for one reason: $\lmin$ is concave and so not
  Clarke regular where the least eigenvalue is multiple.  Both inclusions run
  the direction a necessary condition needs.  The infinitesimal rotations
  \eqref{eq:orbit} impose nothing by \eqref{eq:rotationkill}, so the
  combination in (2) takes place in a space of dimension $mk+m-k^{2}-1$, which
  is $14$ at $(6,3)$ and $18$ at $(7,3)$ and is where the bound
  \eqref{eq:carath} comes from.  Nothing above the rule is formal:
  \S\ref{sec:boundaries} carries the whole passage as one of three steps taken
  on trust, and the ambient library has no Clarke calculus and no
  $\partial\lmin$.}
  \label{fig:kktassembly}
\end{figure}
...
\end{equation}
which says $\cv\ge F(g,t)$.
\end{definition}

The subsets $C_i$ need not be distinct: a tight eigenvalue of
multiplicity $r$ contributes $r$ indices carrying one subset, the form
the spectral decomposition in the proof of Theorem~\ref{thm:firstorder}
delivers.  Nothing below assumes that
$i\mapsto C_i$ is injective, and this is why the active family is defined
as a set of subsets rather than as a set of indices.  Indices with
$\lambda_i=0$ may be discarded: dropping them changes neither
\eqref{eq:statg} nor \eqref{eq:statt}, neither $\Lambda$ nor $A^{+}$, so
one may assume $\{C_i:i\in A\}=A^{+}$.

The tetrahedron datum of Example~\ref{ex:tetrakkt} is admissible: every
triple there has $\lmin(\SC)=1$, so $F=1=\cv$.
...
\eqref{eq:multiplier}.  Finally $A$ is nonempty, since $\sum_i\lambda_i=1$,
and $\cv=u_i\tp S_{C_i}u_i=\sum_{c\in C_i}\langle g_c,u_i\rangle^2\ge0$ for
any $i\in A$; \eqref{eq:multiplier} then exhibits $\Lambda$ as a nonnegative
...
The content of a stationarity datum is exactly \eqref{eq:statg} and
\eqref{eq:statt}.

Symmetry of $\Lambda$ is not part of that content either: nothing in the
proof above uses it, and \eqref{eq:multiplier} produces it, so
Definition~\ref{def:datum} asks for a property it would have granted.  The
formal development drops the hypothesis and derives symmetry with the
rest.  Figure~\ref{fig:quadric} draws the quadric at $(6,3)$.

At the tetrahedron of Example~\ref{ex:tetrakkt} the multiplier was chosen
by hand, and \eqref{eq:multiplier} returns that choice:
$\cv\sum_i\lambda_i\,u_i^{\vphantom{\mathsf{T}}}u_i\tp
=\tfrac14\cdot\tfrac13\sum_ig_i^{\vphantom{\mathsf{T}}}g_i\tp=\tfrac13I_3$.

\begin{figure}[ht]
  \centering
  \input{figures/quadric}
  \caption{The quadric law \eqref{eq:quadric}.  A stationarity datum confines
  every vector of its design to one central quadric of the multiplier, an
  ellipsoid whenever $\Lambda$ is nonsingular.  Drawn at $(6,3)$: the six
  vectors of a design, all on the ellipsoid of
  $\Lambda=\diag(\tfrac12,\tfrac13,\tfrac16)$ at $\cv=1$, with its three
  principal sections and its silhouette.  By \eqref{eq:multiplier} the surface
  is assembled from the tight directions, the semi-axis along $u_i$ having
  length $\sqrt{\cv/\lambda_i}$: it is longest where the multiplier mass is
  least.  Inset: at the tetrahedron of Example~\ref{ex:tetrakkt} the multiplier
  is $\tfrac13I_3$ and the quadric is the sphere $\lVert x\rVert^2=3$.}
  \label{fig:quadric}
\end{figure}
...
over $c$ against $\sum_ct_c=1$ gives $\cv\le m$, the right side being $1$
at each of the $m$ indices.

(iii) Sum \eqref{eq:overlapone} over the $k$ vectors of $C$, exchange the
two summations as in Proposition~\ref{prop:quadric}, and use
\eqref{eq:polarized}: $\sum_i\lambda_i\,u_i\tp\SC u_i=k$.  A convex
combination does not exceed its largest term.

(iv) The sum in \eqref{eq:coverage} runs over $\{i:c\in C_i\}$, and its
value $t_c\cv$ is nonzero because $t_c>0$ at a design and $\cv\ne0$.  Some
term is therefore nonzero.  Membership $c\in C_i$ is the range of the
summation and comes free; the term itself is
$\lambda_i\langle u_i,g_c\rangle^2$, so its nonvanishing forces
$\lambda_i>0$ and $\langle u_i,g_c\rangle\ne0$.
\end{proof}

Part (iii) names a unit vector; it implies $\lmax(\SC)\ge k$ for every
$k$-subset, but the vector is the useful half.  Part (i) says every
vector of a stationary design is heavy in the weak sense; the strict
inequality $\ell_c>1$ would require
excluding equality in Cauchy--Schwarz at every positively weighted index
simultaneously, and is not available.

At the tetrahedron of Example~\ref{ex:tetrakkt} the bounds read
$\ell_c=3\ge1$, $t_c\cv=\tfrac14\le1$ and $\cv=1\le4=m$, and
\eqref{eq:overlapone} restores the index $i=c$ that \eqref{eq:coverage}
omits, for which $\langle u_c,g_c\rangle^2=3$: indeed
$\tfrac14+\tfrac14\cdot3=1$.  Part (iii) is not realized at the tight
index of the subset itself, since $u_i\tp S_{C_i}u_i=\cv=1<3$; it is
realized at any other index, for which
\[
...
Subtracting the two sums leaves the quantity on which the exclusions of
\S\ref{sec:exclusions} act.

\begin{lemma}[Lean]\label{lem:defect}
Let a stationarity datum with $\cv\ne0$ be given.  For every $c$,
\begin{equation}\label{eq:defect}
  \sum_{i\,:\,c\notin C_i}\lambda_i\langle u_i,g_c\rangle^2
  \;=\;1-t_c\,\cv .
\end{equation}
Call the left side the \emph{defect} at $c$.  It vanishes if and only if
$t_c\cv=1$, and then $\langle u_i,g_c\rangle=0$ for every $i$ with
$\lambda_i>0$ and $c\notin C_i$.
\end{lemma}

\begin{proof}
The sum in \eqref{eq:overlapone} runs over all of $A$ and the sum in
\eqref{eq:coverage} over $\{i:c\in C_i\}$, so their difference is the sum
over the complementary indices.  Every term is nonnegative, so the sum
vanishes exactly when each term does.
\end{proof}

Part (ii) of Lemma~\ref{lem:overlap} is the nonnegativity of the defect.
A vanishing defect says $t_c\cv=1$, and $t_c\le1$ then forces $\cv\ge1$;
call this the \emph{defect floor}.

Summing \eqref{eq:defect} over $c$ against $\sum_ct_c=1$ sharpens the
bound $\cv\le m$ of that same part into an identity, the total defect
being
\[
  \sum_c\ \sum_{i\,:\,c\notin C_i}\lambda_i\langle u_i,g_c\rangle^2
  \;=\;m-\cv .
...
Each exclusion below is a statement about the datum alone, and
admissibility is nowhere used.

\begin{proposition}[Lean]\label{prop:e1}
...
\begin{proof}
Taking traces in \eqref{eq:multiplier} gives $\tr\Lambda=\cv$, while
$\tr(a\,ww\tp)=a\lVert w\rVert^2=a$, so $a=\cv$.
...
nonnegative, because Cauchy--Schwarz and
$\lVert u_i\rVert=\lVert w\rVert=1$ force
...
every $c$.  The tight direction enters the value quadratically, so the
sign in $u_i=\pm w$ drops and $u_i\tp S_{C_i}u_i=w\tp S_{C_i}w$.
Therefore, by \eqref{eq:polarized},
\[
...
\begin{corollary}[Lean]\label{cor:e1}
...
\begin{proposition}[Lean]\label{prop:e2}
...
\]
Write $[\,\cdot\,]$ for $1$ when the condition holds and $0$ otherwise.
The left side of \eqref{eq:statg} is the same sum over all of $A$ with
each term carrying the factor $[\,c\in C_i\,]$, so the two sides subtract
term by term and leave the relation
\[
...
$\sum_ct_c=1$ gives $\cv=m$, and then $t_c=1/m$.  The defect floor gives
$\cv\ge1$.
\end{proof}
...
coverage; it does, but only outside the subsets.  Since $t_c\cv=1$ at
every index, every defect vanishes, and Lemma~\ref{lem:defect} then gives
$\langle u_i,g_c\rangle=0$ whenever $\lambda_i>0$ and $c\notin C_i$: each
positively weighted tight direction is orthogonal to every vector its own
subset omits.
...
Lemma~\ref{lem:overlap}(iii), $\sum_i\lambda_iu_i\tp\SC u_i=k$.  An index
with $\lambda_i=0$ may carry a different subset; it is weighted by zero
and drops.  Every remaining $u_i$ is a unit eigenvector of $\SC$ at $\cv$,
so the sum collapses to $\cv\sum_i\lambda_i=\cv$, whence $\cv=k$.  If
$m=2k$ and $A^{+}=\{C,C'\}$ then $C\cup C'$ has $2k$ elements and
$\lvert C\rvert+\lvert C'\rvert=2k$, so inclusion--exclusion leaves
$C\cap C'=\emptyset$.
\end{proof}
...
\begin{corollary}[Lean]\label{cor:e3}
...
Whether the partition branch at $(6,3)$ is inhabited is open.  One
constraint on it is available and does not close it.  If every active
subset either equals a chosen block or is disjoint from it --- which is
what a partition family does --- then that block's multiplier mass equals
its Parseval weight mass,
\[
  \sum_{i\,:\,C_i=B}\lambda_i\;=\;\sum_{c\in B}t_c
  \qquad\text{(\texttt{Gtz.active\allowbreak Weight\_block\allowbreak Mass\_eq\_weight\_block\allowbreak Mass})},
\]
so a two-block datum at $(6,3)$ has its two multipliers equal to the two
block weights.  Every candidate must satisfy this; none is thereby
excluded.
...
on the families in which a single vector lies in every member.
...
\begin{proof}
The hypothesis empties the index set of \eqref{eq:defect}, so the defect
at $c$ vanishes and the defect floor gives $t_c\cv=1$ and $\cv\ge1$.
\end{proof}
...
coordinates.  Take the seven vectors $g_0,\dots,g_6$ to be
$h_0,h_1,h_2,h_3,h_0,h_1,h_2$ and give them the weights
...
$\SC=4I_3-h_d^{\vphantom{\mathsf{T}}}h_d\tp$, as in
...
\]
For \eqref{eq:statg} itself at $c=3$, each of the twelve triples
containing $g_3$ contributes
$\tfrac1{16}\bigl(-\tfrac{1}{\sqrt3}\bigr)\tfrac{h_d}{\sqrt3}
=-\tfrac1{48}h_d$ at its missed direction $h_d$, four triples for each
$d\ne3$, so $\sum_dh_d=0$ turns the total into
$-\tfrac1{12}(h_0+h_1+h_2)=\tfrac1{12}h_3=t_3\Lambda g_3$.
The datum is admissible, and $\lvert A^{+}\rvert=20$.
\end{example}

Example~\ref{ex:splitseven} puts twenty triples in the active family at
the cell to which rank three reduces.  It does not conflict
...
\begin{theorem}[Lean]\label{thm:reach}
...
and nothing derived from the identities \eqref{eq:statg} and
\eqref{eq:statt} alone can establish it.
\end{theorem}
...
\]
The rotation through $135^{\circ}$ sends $g_0,g_1,g_3,g_2$ to
$g_1,-g_3,-g_2,-g_0$, and a sign does not change
$g_c^{\vphantom{\mathsf{T}}}g_c\tp$; the weights and the multipliers are
equal and $\Lambda$ is a multiple of the identity, so the rotation
carries the datum to itself with the pairs cycled
$C_0\to C_2\to C_3\to C_1$.  The three remaining indices follow.  Finally,
the two pairs omitted from the active family are orthogonal:
$S_{\{0,3\}}=S_{\{1,2\}}=2I_2$, so $\lmin=2$ there, $F=2$, and
$S_{\{0,3\}}-I=I\pd0$.
\end{proof}

The datum of Theorem~\ref{thm:reach} is stationary for the maximum taken
over the four chosen pairs, which minimize $\lmin(\SC)$, and not for $F$.

It violates \eqref{eq:argmax}, a disjunctive semialgebraic condition
rather than an equation and therefore beyond the algebra of
\S\ref{sec:laws}.

It also violates the hypothesis of every exclusion: its multiplier is
isotropic rather than of rank one, its four tight directions lie in
$\R^2$ and are dependent, its active family is four distinct pairs,
neither a singleton nor a partition, and no index lies in every pair.
...
Figure~\ref{fig:census} shows the distribution.
...
Against this census the exclusions reach only a corner.  Of the $2136$
classes of families of $3$-subsets of $\{1,\dots,6\}$,
Lemma~\ref{lem:overlap}(iv) removes the $34$ that fail to cover;
Proposition~\ref{prop:saturated} removes $23$ more, those in which some
index lies in every member; and Corollary~\ref{cor:e3} leaves the single
partition class undecided.  Fifty-seven classes are excluded, $2079$ are
not.

Those $23$ are the graphs on five vertices with no isolated vertex.  Fix
the common index: a saturated family is that index together with an edge on
the other five points, and covering says no point of the five is isolated.
They carry between three and ten triples, $1,4,5,5,4,2,1,1$ of them in
order, so no more than five fall in any one column of
Table~\ref{tab:census}.  The last, the ten triples through one index, sits
in the peak column, so the exclusions are not silent where the census is
largest.  Figure~\ref{fig:exclusionreach} sets the two exclusions against
the census.


\begin{figure}[ht]
  \centering
  \input{figures/census}
...
\begin{figure}[ht]
  \centering
  \input{figures/exclusionreach}
  \caption{The reach of the exclusions at $(6,3)$.  Upper: the census of
  Proposition~\ref{prop:census}, $2102$ covering classes with $351$ of them at
  $\lvert A^{+}\rvert=10$.  Lower, on its own scale: the $34$ classes that
  Lemma~\ref{lem:overlap}(iv) removes for failing to cover, and the $23$ that
  Proposition~\ref{prop:saturated} removes for carrying an index in every
  member.  Only the second sits inside the census, the non-covering classes
  being absent from it; it takes $1/3$ of the column at $\lvert A^{+}\rvert=3$
  and $1/351$ at the peak, and neither exclusion reaches above ten.  Both
  excluded families are graphs on five vertices.  The two drawn are the
  extremes of the $23$, a path beside an edge at three triples and the complete
  graph at ten.  The bars are an external
  enumeration: neither Proposition~\ref{prop:census} nor
  Proposition~\ref{prop:carath} carries a declaration, and only the criteria
  the panels apply are kernel-checked.}
  \label{fig:exclusionreach}
\end{figure}
...
corresponding statement at $(4,2)$ without it.  Neither, though, does it
make the conjectures weaker than the cells they decide.  An admissible
datum with $\cv<1$ refutes $\Gtz(m,k)$ outright, since \eqref{eq:argmax}
says $\lmin(\SC)\le\cv<1$ at every $k$-subset and no subset dominates
(\texttt{Gtz.not\_\allowbreak gtz\allowbreak Weighted\_\allowbreak
of\_\allowbreak is\allowbreak Argmax\allowbreak Dominated\_\allowbreak of\_\allowbreak
value\_\allowbreak lt\_one}).
Contrapositively $\Gtz(6,3)$ implies Conjecture~\ref{conj:sixthree}, and
$\Gtz(7,3)$ implies Conjecture~\ref{conj:seventhree}.  The conjectures are
equivalent to their cells given the two nonformal steps of
\S\ref{sec:boundaries}, and the equations of \eqref{eq:statg} and
\eqref{eq:statt} are the whole of the gain.

With admissibility, each conjecture
...
$\lambda_i\ge0$, $\sum_i\lambda_i=1$, \eqref{eq:parseval}, $t_c>0$,
...
produces a minimizing counterexample at that cell in the sense of
...
The minimality hypothesis is all this section takes from
\S\ref{sec:compactness}; Theorem~\ref{thm:variational} supplies it for
the chart objective.

At $(6,3)$ the conjecture is an enumeration.  At $(7,3)$ it is not, and
an argument uniform in the active family is required.  The exclusions of
\S\ref{sec:exclusions} clear away the small and the transversal
configurations, and Example~\ref{ex:splitseven} lives in what remains: a
system realized at the frontier size, at the threshold, by an active
family of twenty triples.
