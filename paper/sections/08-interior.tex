...
\S\ref{sec:compactness} upgrades a failure of $\Gtz(m,k)$ to an extremal
failure: not merely some design below the threshold, but one minimizing
the objective.  What does minimality cost such a design?  The constraint
...
locally Lipschitz but in general nowhere differentiable, so the answer
...
the defect is what keeps a datum below the threshold, and every exclusion
that reaches such a datum reaches it by driving the defect to zero
somewhere.  Summing \eqref{eq:defect} over $c$ against $\sum_ct_c=1$
sharpens the bound $\cv\le m$ of that same part into an identity, the
total defect being
...
not.  The reason is visible in Table~\ref{tab:census}: the exclusions
bite at the two ends of the range, on families too small to cover and on
families large enough to be saturated, while the distribution peaks at
cardinality ten, where they say nothing.  Conjecture~\ref{conj:sixthree}
is not a finite verification away from what is proved here.
