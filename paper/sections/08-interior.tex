...
\begin{equation}\label{eq:designobjective}
  ...
  \qquad \SC=\sum_{c\in C}g_c^{\vphantom{T}}g_c^{T}.
\end{equation}
...
$x^{T}\SC x=\sum_{c\in C}\langle g_c,x\rangle^2>0$, so $\lmin(\SC)>0$.
\end{proof}
...
Fix $m$ and $k$, write $\mathcal{S}^k$ for the space of real symmetric
$k\times k$ matrices, and let
\begin{equation}\label{eq:constraints}
  \Phi(g,t)\;:=\;\sum_{c}t_c\,g_c^{\vphantom{T}}g_c^{T}-I_k
  \;\in\;\mathcal{S}^{k},
  \qquad
...
\begin{equation}\label{eq:conGrad}
  \Phi'(g,t)[\dot g,\dot t]
  =\sum_c\Bigl(\dot t_c\,g_c^{\vphantom{T}}g_c^{T}
      +t_c\bigl(\dot g_c^{\vphantom{T}}g_c^{T}
                +g_c^{\vphantom{T}}\dot g_c^{T}\bigr)\Bigr),
  \qquad
...
$\mathcal{S}^k\times\R$.  Hence the designs form a smooth manifold of
dimension $mk+m-\tfrac12k(k+1)-1$, whose tangent space $T$ at $(g,t)$
consists of the pairs $(\dot g,\dot t)$ annihilated by
\eqref{eq:conGrad}.
\end{lemma}

\begin{proof}
Let $(H,s)\in\mathcal{S}^k\times\R$ be given.  Choose $\dot t$ with
$\sum_c\dot t_c=s$, put
$H':=H-\sum_c\dot t_c\,g_c^{\vphantom{T}}g_c^{T}$, and take
$\dot g_c:=\tfrac12H'g_c$.  Then by \eqref{eq:parseval}
\[
  \sum_c t_c\bigl(\dot g_c^{\vphantom{T}}g_c^{T}
                 +g_c^{\vphantom{T}}\dot g_c^{T}\bigr)
  =\tfrac12H'\sum_c t_c\,g_c^{\vphantom{T}}g_c^{T}
   +\tfrac12\Bigl(\sum_c t_c\,g_c^{\vphantom{T}}g_c^{T}\Bigr)H'
  =H' ,
\]
so $\Phi'[\dot g,\dot t]=H$ and $\sigma'[\dot t]=s$.  The dimension count
and the description of $T$ are the submersion theorem.
\end{proof}

One family of tangent directions will be needed twice.  For an
antisymmetric $X$ the infinitesimal rotation
\begin{equation}\label{eq:orbit}
  \dot g_c\;=\;Xg_c\quad(1\le c\le m),\qquad \dot t=0
\end{equation}
lies in $T$, since
$\sum_c t_c\bigl(Xg_c^{\vphantom{T}}g_c^{T}
+g_c^{\vphantom{T}}g_c^{T}X^{T}\bigr)=X-X=0$ by \eqref{eq:parseval}; and
$X\mapsto(Xg_c)_c$ is injective, because the $g_c$ span $\R^k$ and a
matrix killing a spanning set is zero.  Write $\mathcal{O}\subseteq T$
...
Two facts are classical.  First, $\lmin$ is concave on $\mathcal{S}^k$
and its superdifferential at $A$ is the \emph{spectraplex}
\begin{equation}\label{eq:spectraplex}
\begin{aligned}
  \Delta(A)\;&:=\;\bigl\{\,\Omega\psd0:\ \tr\Omega=1,\
    \operatorname{ran}\Omega\subseteq\ker\bigl(A-\lmin(A)I\bigr)\,\bigr\}\\
  &\;=\;\operatorname{conv}\bigl\{uu^{T}:\ Au=\lmin(A)u,\
    \lVert u\rVert=1\bigr\}
\end{aligned}
\end{equation}
\cite[\S2]{Overton1988},
\cite[Vol.~I, Ch.~VI]{HiriartUrrutyLemarechal1993}.  Second, the Clarke
subdifferential of a locally Lipschitz function composed with a smooth
map is contained in the image of the subdifferential under the adjoint of
the derivative \cite[Thm.~2.3.10]{Clarke1990}.  Applied to
$\lmin\circ\SC$, and using that $\Omega$ is symmetric so that
$\langle\Omega,\dot g_c^{\vphantom{T}}g_c^{T}
+g_c^{\vphantom{T}}\dot g_c^{T}\rangle
=2\langle\Omega g_c,\dot g_c\rangle$, these give
\begin{equation}\label{eq:pieceSub}
  \partial\bigl(\lmin\circ \SC\bigr)(g,t)
  \;\subseteq\;
  \Bigl\{\,\phi_{C,\Omega}:(\dot g,\dot t)\mapsto
    2\sum_{c\in C}\langle\Omega g_c,\dot g_c\rangle
    \ :\ \Omega\in\Delta(\SC)\,\Bigr\}.
\end{equation}
The functionals $\phi_{C,\Omega}$ do not involve $\dot t$.

\begin{theorem}\label{thm:firstorder}
Let $(g,t)$ be a design at which $F$ attains a local minimum over the
designs.  Put $v:=F(g,t)$ and
$\mathcal{A}:=\{C:\lvert C\rvert=k,\ \lmin(\SC)=v\}$.  Then there are a
finite index set $A$, subsets $C_i\in\mathcal{A}$, reals $\lambda_i\ge0$
with $\sum_{i\in A}\lambda_i=1$, unit vectors $u_i$ with
$S_{C_i}u_i=vu_i$, a symmetric matrix $\Lambda$ and a scalar $\mu$ such
that
\begin{align}
  \sum_{i\,:\,c\in C_i}\lambda_i\langle u_i,g_c\rangle\,u_i
    &\;=\;t_c\,\Lambda g_c &&(1\le c\le m), \label{eq:statg}\\
  g_c^{T}\Lambda\,g_c &\;=\;\mu &&(1\le c\le m). \label{eq:statt}
\end{align}
\end{theorem}
...
\cite[Thm.~2.3.10]{Clarke1990} there is $\zeta\in\partial F(g,t)$
vanishing on $T$.  Inactive subsets remain inactive near $(g,t)$, so the
maximum rule \cite[Prop.~2.3.12]{Clarke1990} together with
\eqref{eq:pieceSub} exhibits $\zeta$ as a convex combination
\begin{equation}\label{eq:zeta}
  \zeta\;=\;\sum_{C\in\mathcal{A}}\lambda_C\,\phi_{C,\Omega_C},
  \qquad \lambda_C\ge0,\quad\sum_C\lambda_C=1,\quad
  \Omega_C\in\Delta(\SC).
\end{equation}
Decompose each $\Omega_C$ spectrally,
$\Omega_C=\sum_jw_{C,j}u_{C,j}^{\vphantom{T}}u_{C,j}^{T}$ with
$w_{C,j}\ge0$, $\sum_jw_{C,j}=1$ and each $u_{C,j}$ a unit eigenvector of
$\SC$ at $v$; this is possible by \eqref{eq:spectraplex}.  Index by the
...
kernel $T$, and $\zeta$ vanishes on $T$; hence $\zeta$ factors through
$(\Phi',\sigma')$.  That is, there are $\Lambda\in\mathcal{S}^k$ and
$\mu\in\R$ with
...
$\langle\Lambda,\dot g_c^{\vphantom{T}}g_c^{T}
+g_c^{\vphantom{T}}\dot g_c^{T}\rangle=2\langle\Lambda g_c,\dot g_c\rangle$
as before,
\[
  ...
  \;+\;\sum_c\dot t_c\bigl(g_c^{T}\Lambda g_c-\mu\bigr).
\]
...
Two features of the derivation are used later.  The maximum rule in
\eqref{eq:zeta} is an inclusion and not an equality, because the pieces
...
$\max_{C\in\mathcal{A}}\lmin\bigl(Q_C^{T}\dot S_CQ_C\bigr)\ge0$ for every
$(\dot g,\dot t)\in T$, where $Q_C$ is an orthonormal basis of the
...
multiplier form.  Second, the infinitesimal rotations \eqref{eq:orbit}
impose no condition: for $\Omega\in\Delta(\SC)$ one has
$\Omega\SC=v\Omega=\SC\Omega$, so
\begin{equation}\label{eq:rotationkill}
  \phi_{C,\Omega}\bigl[(Xg_c)_c,0\bigr]
  =2\sum_{c\in C}\langle\Omega g_c,Xg_c\rangle
  =2\tr\bigl(\Omega X\SC\bigr)=2v\,\tr(\Omega X)=0,
\end{equation}
because $\Omega$ is symmetric and $X$ antisymmetric.
...
$v\in\R$, consists of a finite index set $A$, $k$-subsets $C_i$
($i\in A$), reals $\lambda_i\ge0$ with $\sum_i\lambda_i=1$, vectors $u_i$
with $\lVert u_i\rVert=1$ and $S_{C_i}u_i=vu_i$, and a symmetric matrix
...
\begin{equation}\label{eq:argmax}
  v\;\ge\;\lmin(\SC)\qquad\text{for every }k\text{-subset }C ,
\end{equation}
equivalently if $v=F(g,t)$.
\end{definition}
...
Definition~\ref{def:datum} asks only that $v$ be \emph{an} eigenvalue of
each $S_{C_i}$.  It does not ask that $v=\lmin(S_{C_i})$, and it does not
...
\begin{equation}\label{eq:contract}
  ...
  \;=\;t_c\,\bigl(g_c^{T}\Lambda g_c\bigr) .
\end{equation}
This single contraction carries both laws.  Throughout we use the
polarized form of the subset sum,
\begin{equation}\label{eq:polarized}
  y^{T}\SC x\;=\;\sum_{c\in C}\langle g_c,y\rangle\langle g_c,x\rangle ,
\end{equation}
immediate from \eqref{eq:dominate}.

\begin{proposition}[\LV]\label{prop:quadric}
Let a stationarity datum be given.  Then $\mu=v$, so that every vector
lies on one central quadric of the multiplier,
\begin{equation}\label{eq:quadric}
  g_c^{T}\Lambda\,g_c\;=\;v\qquad(1\le c\le m);
\end{equation}
for every $c$ the coverage law
\begin{equation}\label{eq:coverage}
  \sum_{i\,:\,c\in C_i}\lambda_i\langle u_i,g_c\rangle^2\;=\;t_c\,v
\end{equation}
holds; the multiplier is determined by the remaining data,
\begin{equation}\label{eq:multiplier}
  \Lambda\;=\;v\sum_{i\in A}\lambda_i\,u_i^{\vphantom{T}}u_i^{T};
\end{equation}
and consequently $v\ge0$, $\Lambda\psd0$ and $\tr\Lambda=v$.
\end{proposition}

\begin{proof}
Sum \eqref{eq:contract} over $c$.  On the right, \eqref{eq:statt} makes
$g_c^{T}\Lambda g_c$ independent of $c$, so the right side is
...
\eqref{eq:polarized} and $S_{C_i}u_i=vu_i$,
\[
  \sum_{c\in C_i}\langle u_i,g_c\rangle^2
  \;=\;u_i^{T}S_{C_i}u_i\;=\;v\lVert u_i\rVert^2\;=\;v .
\]
Hence the left side is $v\sum_i\lambda_i=v$, so $\mu=v$.  Substituting
$\mu=v$ into \eqref{eq:statt} gives \eqref{eq:quadric}, and into
...
\begin{align*}
  ...
  &=\sum_{i\in A}\lambda_i
   ...
   =\sum_{i\in A}\lambda_i\bigl(u_i^{T}S_{C_i}x\bigr)u_i
  &&\text{by \eqref{eq:polarized}}\\
  &=v\sum_{i\in A}\lambda_i\langle u_i,x\rangle\,u_i ,
\end{align*}
the last step by symmetry of $S_{C_i}$ together with
$S_{C_i}u_i=vu_i$.  A matrix is determined by its action, which is
\eqref{eq:multiplier}.  Finally
$v=u_i^{T}S_{C_i}u_i=\sum_{c\in C_i}\langle g_c,u_i\rangle^2\ge0$ for any
...
$\Lambda\psd0$ and $\tr\Lambda=v\sum_i\lambda_i=v$.
\end{proof}
...
\begin{lemma}[\LV]\label{lem:overlap}
Let a stationarity datum with $v\ne0$ be given.  Then
...
\item[\rm(ii)] $t_cv\le1$ for every $c$, and hence $v\le m$;
\item[\rm(iii)] for every $k$-subset $C$, active or not, there is an
  index $i\in A$ with $u_i^{T}\SC u_i\ge k$;
...
$g_c^{T}\Lambda g_c=v\sum_i\lambda_i\langle u_i,g_c\rangle^2$; comparison
with \eqref{eq:quadric} and cancellation of $v\ne0$ give
\eqref{eq:overlapone}.
...
\eqref{eq:overlapone} with nonnegative terms, so $t_cv\le1$.  Summing
over $c$ against $\sum_ct_c=1$ gives $v\le m$.

(iii) Sum \eqref{eq:overlapone} over the $k$ vectors of $C$ and use
\eqref{eq:polarized}: $\sum_i\lambda_i\,u_i^{T}\SC u_i=k$.  A convex
combination does not exceed its largest term.

(iv) By \eqref{eq:coverage} the sum at $c$ equals $t_cv\ne0$, so one of
its terms is nonzero, and that forces $\lambda_i>0$, $c\in C_i$ and
$\langle u_i,g_c\rangle\ne0$ for the corresponding $i$.
\end{proof}

Part (iii) names a unit vector, and that is what the system delivers; it
implies $\lmax(\SC)\ge k$ for every $k$-subset, but the vector is the
...
$\SC=4I_3-g_{c_0}^{\vphantom{T}}g_{c_0}^{T}$, with spectrum $\{1,4,4\}$.
Take
\[
  A=\{0,1,2,3\},\qquad C_i=\{0,1,2,3\}\setminus\{i\},\qquad
  \lambda_i=\tfrac14,\qquad
  u_i=\tfrac{1}{\sqrt3}\,g_i,\qquad v=1 .
\]
...
$v=1$.  The multiplier predicted by \eqref{eq:multiplier} is
\[
  \Lambda\;=\;1\cdot\sum_{i}\tfrac14\cdot\tfrac13\,
    g_i^{\vphantom{T}}g_i^{T}
  \;=\;\tfrac1{12}\sum_ig_i^{\vphantom{T}}g_i^{T}
  \;=\;\tfrac1{12}\cdot4I_3\;=\;\tfrac13I_3 ,
\]
using $\sum_ig_i^{\vphantom{T}}g_i^{T}=4I_3$, which is
\eqref{eq:parseval} at $t_i=\tfrac14$.  Then $\tr\Lambda=1=v$, as
Proposition~\ref{prop:quadric} requires, and the quadric law
\eqref{eq:quadric} reads $g_c^{T}\Lambda g_c=\ell_c/3=1=v$.
...
\[
  \sum_{i\,:\,c\in C_i}\lambda_i\langle u_i,g_c\rangle^2
  \;=\;3\cdot\tfrac14\cdot\tfrac13\;=\;\tfrac14\;=\;t_cv .
\]
...
\[
  ...
  \;=\;t_c\Lambda g_c .
\]
The datum is admissible: every triple has $\lmin=1$, so $F=1=v$.  The
bounds of Lemma~\ref{lem:overlap} read $\ell_c=3\ge1$,
$t_cv=\tfrac14\le1$ and $v=1\le4=m$.  Part (iii) of that lemma is not
realized at the tight index of the subset itself, since
$u_i^{T}S_{C_i}u_i=v=1<3$; it is realized at any other index, for which
\[
  u_j^{T}S_{C_i}u_j=4-\langle g_i,u_j\rangle^2
...
Three structural configurations are excluded outright.  Each exclusion is
a statement about the datum alone; admissibility is nowhere used.

\begin{proposition}\label{prop:e1}
Let a stationarity datum with $v\ne0$ have $\Lambda=\rho\,ww^{T}$ for
some unit vector $w$ and some $\rho\in\R$.  Then $v=k$.  Since
$\Lambda\psd0$, this covers every datum with $\rk\Lambda\le1$.
\end{proposition}

\begin{proof}
Taking traces in \eqref{eq:multiplier} gives $\rho=\tr\Lambda=v$.
Evaluating \eqref{eq:multiplier} as a quadratic form at an arbitrary $x$
gives $v\langle w,x\rangle^2=v\sum_i\lambda_i\langle u_i,x\rangle^2$, so
after cancelling $v\ne0$,
...
\]
so $u_i=\pm w$.  Now read \eqref{eq:quadric} through $\Lambda=v\,ww^{T}$:
it says $v\langle w,g_c\rangle^2=v$, so $\langle w,g_c\rangle^2=1$ for
every $c$.  Therefore, by \eqref{eq:polarized},
\[
  v\;=\;u_i^{T}S_{C_i}u_i\;=\;w^{T}S_{C_i}w
...
No stationarity datum with $0<v<1$ has a multiplier of rank at most one.
\end{corollary}

\begin{proof}
Otherwise $v=k$ by Proposition~\ref{prop:e1}, and $v\ne0$ forces
$k\ge1$, whence $v\ge1$.
\end{proof}
...
$\sum_{i\in A}\lambda_i\gamma_i\,u_i=0$ with $\gamma:A\to\R$ has
$\lambda_i\gamma_i=0$ for every $i$; linear independence of the family
...
Then $t_cv=1$ for every $c$; hence $v=m$ and $t_c=1/m$ for every $c$.
In particular no stationarity datum with $0<v<1$ has independent tight
...
\[
  ...
  \bigl(\,[\,c\in C_i\,]-t_cv\,\bigr)\;=\;0\qquad(i\in A).
\]
Fix $c$.  By Lemma~\ref{lem:overlap}(iv) there is $i$ with
$\lambda_i>0$, $c\in C_i$ and $\langle u_i,g_c\rangle\ne0$; for that $i$
the last factor is $1-t_cv$, so $t_cv=1$.  Summing over $c$ against
$\sum_ct_c=1$ gives $v=m$, and then $t_c=1/m$.  A design has at least one
vector, so $v=m\ge1$.
\end{proof}

The conclusion is an identification and not a contradiction.  One might
expect independence to force $\langle u_i,g_c\rangle=0$, contradicting
coverage; the coefficient in \eqref{eq:vanishing} can vanish through its
last factor instead, and it does.

The third exclusion is counting.  By Lemma~\ref{lem:overlap}(iv) the
active family covers $\{1,\dots,m\}$, and each of its members has $k$
elements.

\begin{proposition}\label{prop:count}
For a stationarity datum with $v\ne0$,
\begin{equation}\label{eq:count}
  m\;\le\;k\,\lvert A^{+}\rvert .
\end{equation}
If $k<m$ then $\lvert A^{+}\rvert\ge2$.  If $\lvert A^{+}\rvert=1$ then
$m=k$ and $v=k$.  If $m=2k$ and $\lvert A^{+}\rvert=2$, the two members
...
Lemma~\ref{lem:overlap}(iii), $\sum_i\lambda_iu_i^{T}\SC u_i=k$, while
each such $u_i$ is a unit eigenvector of $\SC$ at $v$, whence $v=k$.  If
$m=2k$ and $A^{+}=\{C,C'\}$ then $C\cup C'$ has $2k$ elements and
$\lvert C\rvert+\lvert C'\rvert=2k$, so $C\cap C'=\emptyset$.
\end{proof}

\begin{corollary}[\LV]\label{cor:singleactive}
A stationarity datum with $v\ne0$ whose index set $A$ is a singleton has
$v=k$.  In particular no such datum has $0<v<1$.
\end{corollary}

\begin{proof}
A singleton $A=\{i_0\}$ has $\lambda_{i_0}=1>0$, so $A^{+}=\{C_{i_0}\}$
and Proposition~\ref{prop:count} gives $v=k$; and $v\ne0$ forces $k\ge1$,
whence $v\ge1$.
\end{proof}

\begin{corollary}\label{cor:e3}
Let a stationarity datum at $(6,3)$ have $0<v<1$.  Then either
...
support at $v<1$.  At $(7,3)$, \eqref{eq:count} reads
$7\le3\lvert A^{+}\rvert$.
\end{proof}
...
\begin{proposition}[\LV]\label{prop:saturated}
Let a stationarity datum with $v\ne0$ be given, and suppose some index
$c$ lies in $C_i$ for every $i\in A$.  Then $t_cv=1$, and hence
$v\ge1$.
\end{proposition}
...
index sets coincide, so the inequality $t_cv\le1$ of
Lemma~\ref{lem:overlap}(ii) is an equality: $t_cv=1$.  Since
$t_c\le1$ this gives $v=1/t_c\ge1$.
\end{proof}

\begin{corollary}\label{cor:saturated}
At a datum with $0<v<1$, no index lies in every member of $A$; in
particular, for each index some active subset omits it.
\end{corollary}

Propositions~\ref{prop:e1}, \ref{prop:e2}, \ref{prop:count} and
\ref{prop:saturated} are the exclusions available at $0<v<1$.  Their
...
$\sum_dh_d^{\vphantom{T}}h_d^{T}=4I_3$ and $\langle h_d,h_e\rangle=-1$
...
$\SC=4I_3-h_d^{\vphantom{T}}h_d^{T}$, exactly as in
Example~\ref{ex:tetrakkt}, so $u_C:=h_d/\sqrt3$ is a unit eigenvector at
$v=1$.  Give multiplier $\tfrac1{32}$ to the eight triples omitting $h_3$
...
\[
  \Lambda\;=\;1\cdot\sum_{d}\tfrac14\cdot\tfrac13\,
    h_d^{\vphantom{T}}h_d^{T}\;=\;\tfrac1{12}\cdot4I_3
    \;=\;\tfrac13I_3 ,
\]
whence $\tr\Lambda=1=v$ and $g_c^{T}\Lambda g_c=3/3=1=v$, which is
...
the cell which carries the whole of rank three.  It does not conflict
...
The next statement is limitative, and it fixes the form the conjectures
of \S\ref{sec:census} must take.

\begin{theorem}\label{thm:reach}
There is a design at $(4,2)$ carrying a stationarity datum with
$v=2-\sqrt2<1$.  That design satisfies $\Gtz(4,2)$ with margin: some
$2$-subset $C$ has $\SC-I=I\pd0$, so $F=2$.  Consequently the
implication ``a stationarity datum with $v\ne0$ has $v\ge1$'' is false,
...
$C_1$, $C_2$, $C_3$ by symmetry.  Put $v:=2-\sqrt2$.  Unit eigenvectors
at $v$ are the normalizations $u_i:=r_i/\lVert r_i\rVert$ of
\[
...
$g_c^{T}\Lambda g_c=\tfrac{2-\sqrt2}{2}\cdot2=2-\sqrt2=v$, which is
...
over the four chosen pairs, which are the \emph{minimizers} of
$\lmin(\SC)$, and not for $F$.  What it violates is \eqref{eq:argmax}, a
disjunctive semialgebraic condition rather than an equation, and
therefore not available to the algebra of \S\ref{sec:laws}.  It violates
the hypothesis of each exclusion as well, and in the expected way: its
multiplier is isotropic rather than of rank one; its four tight
directions lie in $\R^2$ and are dependent; and its active family
consists of four distinct pairs, neither a singleton nor a partition.

\subsection{The residue}\label{sec:census}

Two finite bounds delimit what is left.  The lower one is
\eqref{eq:count}.  The upper one is Carath\'eodory's theorem applied to
the convex combination \eqref{eq:zeta}.
...
$\zeta=\sum_{i\in A}\lambda_i\,\phi_{C_i,u_iu_i^{T}}$, a convex
combination of the rank-one functionals of \eqref{eq:pieceSub}; and
$\zeta$ vanishes on $T$.  So the origin lies in the convex hull of the
restrictions to $T$ of those functionals.  By \eqref{eq:rotationkill}
each restriction annihilates the subspace $\mathcal{O}$ of infinitesimal
rotations, which by \eqref{eq:orbit} lies in $T$ and has dimension
$\tfrac12k(k-1)$.  The combination therefore takes place in the
annihilator of $\mathcal{O}$ inside the dual of $T$, a space of dimension
\[
  \dim T-\dim\mathcal{O}
...
The count that matters is not the size of the census but the part of it
the exclusions reach.  Of the $2136$ classes of families of $3$-subsets
...
\[
  \frac1{n!}\sum_{\pi\in S_n}\ \sum_{R}(-1)^{r-\lvert R\rvert}
    \bigl[x^{j}\bigr]\!\!\prod_{\gamma\,\subseteq\, T_R}
      \bigl(1+x^{\lvert\gamma\rvert}\bigr),
\]
where $\pi$ has $r$ cycles on $\{1,\dots,n\}$, the index $R$ runs over
sets of those cycles, $T_R$ is their union, and $\gamma$ runs over the
cycles of the permutation induced by $\pi$ on the $3$-subsets contained
in $T_R$.  A family fixed by $\pi$ is a union of such cycles, and its
union of points is $\pi$-invariant, so the alternating sum is
...
\begin{figure}[ht]
  ...
  Corollary~\ref{cor:e3} removes the single class of cardinality two
  except in its partition branch, and \eqref{eq:carath} removes the
...
and the value $v$; the conditions are \eqref{eq:statg},
\eqref{eq:statt}, $\lVert u_i\rVert=1$, $S_{C_i}u_i=vu_i$,
$\lambda_i\ge0$, $\sum_i\lambda_i=1$, \eqref{eq:parseval}, $t\pd0$,
$\sum_ct_c=1$, $0<v<1$, and \eqref{eq:argmax} --- the last a disjunction
of sign conditions on the leading principal minors of $\SC-vI$, one
disjunction for each $k$-subset $C$.

\begin{theorem}[\CD]\label{thm:rankthree}
...
local minimum over the designs at $(6,3)$ at a design of value $v<1$,
and $v>0$ by Lemma~\ref{lem:fpos}.  Theorem~\ref{thm:firstorder}
produces at that design a stationarity datum with value $v$, and it is
admissible because $v=F$ there.  This contradicts
Conjecture~\ref{conj:sixthree}.  The same argument at $(7,3)$ with
Conjecture~\ref{conj:seventhree} gives $\Gtz(7,3)$.
\end{proof}

The extremality hypothesis is the only thing this section takes from
\S\ref{sec:compactness}, and it is what Theorem~\ref{thm:variational}
supplies for the chart objective.

At $(6,3)$ the conjecture is an enumeration; at $(7,3)$ it is not, and an
argument uniform in the active family is required.  What the exclusions
of \S\ref{sec:exclusions} remove are the small and the transversal
configurations, and Example~\ref{ex:splitseven} shows where the residue
