...
two together yield a finite system of algebraic identities, and from
there on everything is algebra.
...
$\lmin(\SC)\ge1$, so $\Gtz(m,k)$ asserts $F\ge1$ on every design, and a failure of $\Gtz(m,k)$ is a design with $F<1$.
...
inside the open weight simplex.  It is not automatic that the infimum of $F$ over the
...
dominates.  Theorem~\ref{thm:variational} settles that question
...
configurations; what is left is where Example~\ref{ex:splitseven} lives,
a system realized at the frontier size, at the threshold, by an active
