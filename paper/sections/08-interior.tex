...
\eqref{eq:statg} and \eqref{eq:statt}.  Symmetry of $\Lambda$ is not part
of that content either: nothing in the proof above uses it, and
\eqref{eq:multiplier} produces it, so Definition~\ref{def:datum} asks for a
property it would have granted.  The formal development drops the
hypothesis and derives symmetry with the rest.  Figure~\ref{fig:quadric}
draws the quadric at $(6,3)$.
...
Two of the four exclusions below drive the defect \eqref{eq:defect} to
zero, one at a single index and one at every index; the other two run on
the multiplier \eqref{eq:multiplier} and on a count of the active family.
Each is a statement about the datum alone, and admissibility is nowhere
used.
...
at $c$ vanishes and $t_c\cv=1$.  Since $t_c\le1$ this gives
$\cv=1/t_c\ge1$.
\end{proof}
...
are dependent, its active family is four distinct pairs, neither a
singleton nor a partition, and no index lies in every pair.
...
largest; they are thin everywhere.
...
$\Gtz(7,3)$ implies Conjecture~\ref{conj:seventhree}.  What the interior
route buys is therefore the first-order system, not a smaller problem: the
conjectures are equivalent to their cells given the two nonformal steps of
