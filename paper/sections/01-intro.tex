...
$x$, $y$, $u$, $A$, $B$, $H$, $Q$, and the like --- are proof-local bound
...
\toprule
$m,k$ & size and rank & $P$ & the chart projection $VV^{*}$\\
$c,d$ & indices of atoms & $D$ & the diagonal $\diag(t)$\\
$C$ & a $k$-subset & $W$ & the gap $P-D$\\
$\dsn$ & a design & $\Gram_C$ & the Gram matrix of $C$\\
$g_c$ & the $c$-th atom & $M_C$ & the matrix of rows $g_c^{*}$, $c\in C$\\
$t_c$ & its weight & $\SC$ & the subset sum $\sum_{c\in C}g_cg_c^{*}$\\
...
$V$ & the isometry of rows $v_c^{*}$ & $\lambda_i$ & its scalar multipliers\\
...
\medskip
\noindent
The superscript $\mathsf{T}$ is transpose and $^{*}$ is the adjoint, upright
and starred respectively so that neither can be read as a variable; over
$\R$ they agree, and the paper writes $\mathsf{T}$ in the sections that are
real throughout and $^{*}$ where the field is $\FF$.  The star has no other
use.
