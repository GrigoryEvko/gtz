...
and duality converts corank one and corank two into rank one and rank two
(Theorem~\ref{thm:duality}).  Rank two itself is thus the last small case
outstanding, and nothing so far bounds the size.  One kind of
...
different currencies.  Deleting a vector of length at most one
(\S\ref{sec:light}) costs one from the size but moves every surviving
vector by a congruence; the transport back is an identity between
quadratic forms, and it works only because the deleted vector was short.
Carathéodory reduction (\S\ref{sec:crystal}) leaves the vectors where
they are and cuts the size to $k(k+1)/2+1$, and there the transport back
is immediate, since $\SC$ in \eqref{eq:dominate} never sees the weights.
Deletion stalls when every vector is long; at rank two the stalled case
can be handled directly (\S\ref{sec:ranktwo}).
...
$\lmin(\SC)=1$.  The reductions consume the interior of the problem and
leave the extremal locus of \S\ref{sec:ties} standing, and they do so
again at every later stage.
...
unrelated to the complex problem of \S\ref{sec:complex}), and let
...
invertible with $R\tp\Omega R=I_2$; inverting both sides gives
...
inequality of Lemma~\ref{lem:rowsum} fails, as it must at a design with
$K\equiv0$.
...
\eqref{eq:doubleangle} and the light/heavy split are theirs.  The passage to arbitrary weights is new; it costs the reweighting of
...
$\GtzC(k+2,k)$ is false for every $k\ge2$; the two are the same
statement, read through Corollary~\ref{cor:dualcell}.
...
support-reduction step of that theorem, carried out by hand because the
representation must use a \emph{subfamily of the given vectors} rather
than merely few matrices: only then is the dominating subset produced
downstairs a subset of the original index set.  This step uses the weight-freeness of \eqref{eq:dominate}, and it
