...
and corank two into rank one and rank two.  Two things are missing: rank
two itself, and any control on the size.  This section supplies both, and
the two supplies are instances of one device --- an operation producing
from a design a smaller design whose dominating subsets transport back.

Two such operations exist, and they differ in exactly one respect.
...
because $\SC$ in \eqref{eq:dominate} does not involve the weights.  The
first operation stalls exactly when every vector is long, and rank two is
the rank at which the stalled case can be settled outright
(\S\ref{sec:ranktwo}).
...
\begin{theorem}[\LV]\label{thm:light}
Let $m\ge2$ and let $D$ be a design at $(m,k)$ possessing a light vector.
If $\Gtz(m-1,k)$ holds, then $D$ admits a dominating $k$-subset.
\end{theorem}
...
$P:=I_k-t_d\,g_d^{\vphantom{T}}g_d^{T}$, which by \eqref{eq:parseval} is
$\sum_{c\ne d}t_c\,g_c^{\vphantom{T}}g_c^{T}$.  For $u\ne0$,
\[
  u^{T}Pu=\lVert u\rVert^2-t_d\langle g_d,u\rangle^2
  \;\ge\;(1-t_d\ell_d)\lVert u\rVert^2\;>\;0 ,
\]
using Cauchy--Schwarz and $t_d\ell_d\le t_d<1$.  Write $P=LL^{T}$ with $L$
invertible and set $R:=L^{-T}$, so that
\begin{equation}\label{eq:whiten}
  R^{T}PR=I_k ,\qquad \det R\ne0 .
\end{equation}

\emph{Step 2: the deflated design.}  On the index set
$E:=\{1,\dots,m\}\setminus\{d\}$ put
\[
  g'_c:=\sqrt{1-t_d}\;R^{T}g_c ,
  \qquad
  t'_c:=\frac{t_c}{1-t_d}
  \qquad (c\in E).
\]
...
$(R^{T}g)(R^{T}g)^{T}=R^{T}(g^{\vphantom{T}}g^{T})R$ gives
\[
  \sum_{c\in E}t'_c\,g'^{\vphantom{T}}_cg'^{T}_c
  \;=\;\sum_{c\in E}t_c\,R^{T}g_c^{\vphantom{T}}g_c^{T}R
  \;=\;R^{T}PR\;=\;I_k
\]
...
$\sum_{c\in C}g'^{\vphantom{T}}_cg'^{T}_c=(1-t_d)R^{T}\SC R$, so
domination reads
\[
  R^{T}\bigl((1-t_d)\SC-P\bigr)R\;\psd\;0 ,
\]
and congruence by the invertible $R$ gives $(1-t_d)\SC-P\psd0$.  The
identity
\begin{equation}\label{eq:reassemble}
  \SC-I_k
  \;=\;\frac{1}{1-t_d}\Bigl[\bigl((1-t_d)\SC-P\bigr)
        \;+\;t_d\bigl(I_k-g_d^{\vphantom{T}}g_d^{T}\bigr)\Bigr]
\end{equation}
is verified by expanding $P$: the two occurrences of
$t_d\,g_d^{\vphantom{T}}g_d^{T}$ cancel and the bracket equals
...
the scalar is positive.  Hence $C$ dominates in $D$.
\end{proof}

Lightness is used exactly once, in the second summand of
\eqref{eq:reassemble}.  Everything else is bookkeeping around a
congruence.  Iterating the theorem removes light vectors one at a time:

\begin{corollary}[\LV]\label{cor:heavy}
...
$g_c=\sqrt2\,u_c$, $t_c=\tfrac13$.  From $\sum_cu_c^{\vphantom{T}}u_c^{T}
=\tfrac32I_2$ one gets $\sum_ct_c\,g_c^{\vphantom{T}}g_c^{T}=I_2$, so this
is a design at $(3,2)$, with $\ell_c=2$ and $\nu_c=0$.  It is the member at
...
$C=\{c,d\}$ has
\[
  \SC\;=\;3I_2-2\,u_e^{\vphantom{T}}u_e^{T},
  \qquad
...
The whole of rank two rests on one scalar quantity attached to a pair.

\begin{definition}\label{def:obstruction}
For indices $c,d$ of a design at $(m,2)$ put
\begin{equation}\label{eq:obstruction}
  K_{cd}\;:=\;\langle \eta_c,\eta_d\rangle-\nu_c\nu_d+2 .
\end{equation}
\end{definition}
...
$X:=S_{\{c,d\}}-I_2=MM^{T}-I_2$.  For a $2\times2$ matrix $A$ one has
$\det(A-I)=\det A-\tr A+1$; with $A=MM^{T}$, $\det A=(\det M)^2=b_{cd}^2$
...
\eqref{eq:obstruction} and using $\nu=\ell-2$,
\[
  K_{cd}
...
$X_{00}\,x^{T}Xx=(X_{00}x_0+X_{01}x_1)^2+(\det X)x_1^2\ge0$; the converse
is the $2\times2$ minor.  Hence $X\psd0$ if and only if $K_{cd}\le0$.
\end{proof}

At the rank-two trine of Example~\ref{ex:trine2} one has $\nu_c=0$ and $\langle
...
Identity \eqref{eq:core} is where the balance identities
\eqref{eq:balance2} are spent, and it is sharp: the correction term
...
\[
  \Omega:=\sum_c t_c\,\eta_c^{\vphantom{T}}\eta_c^{T},
  \qquad
  \zeta:=\sum_c t_c\,\nu_c\eta_c,
  \qquad
  \sigma:=\sum_c t_c\,\nu_c^2 ,
\]
so that $\Omega$ is a symmetric $2\times2$ matrix with $r^{T}\Omega
...
Put
\[
  \Psi\;:=\;\Omega^2-\zeta\zeta^{T}-2\Omega .
\]

\emph{Some direction is nonnegative for $\Psi$.}  Let $\xi:=\Omega^{-1}
\zeta$.  Expanding the square and using
$\sum_ct_c\langle \eta_c,\xi\rangle^2=\xi^{T}\Omega\xi=\zeta^{T}
\Omega^{-1}\zeta$ and $\sum_ct_c\nu_c\langle \eta_c,\xi\rangle=\langle
\zeta,\xi\rangle=\zeta^{T}\Omega^{-1}\zeta$,
\begin{equation}\label{eq:residual}
  \sum_c t_c\bigl(\langle \eta_c,\xi\rangle-\nu_c\bigr)^2
  \;=\;\sigma-\zeta^{T}\Omega^{-1}\zeta ,
\end{equation}
so $\zeta^{T}\Omega^{-1}\zeta\le\sigma$: the residual of the least-squares
fit of $\nu$ by a linear functional of $\eta$ is nonnegative.  Choose $R$
invertible with $R^{T}\Omega R=I_2$, that is $RR^{T}=\Omega^{-1}$, and let
$r_1,r_2$ be its columns.  Then $\tr(R^{T}\Omega^2R)
=\tr(\Omega^2RR^{T})=\tr\Omega$ and $\tr(R^{T}\zeta\zeta^{T}R)
=\zeta^{T}\Omega^{-1}\zeta$, so by \eqref{eq:tromega} and
\eqref{eq:residual}
\[
  r_1^{T}\Psi r_1+r_2^{T}\Psi r_2
  =\tr(R^{T}\Psi R)
  =\tr\Omega-\zeta^{T}\Omega^{-1}\zeta-4
  =\sigma-\zeta^{T}\Omega^{-1}\zeta
  \;\ge\;0 .
\]
Hence $r^{T}\Psi r\ge0$ for $r:=r_1$ or $r:=r_2$, and $r\ne0$ because $R$
is invertible.
...
\[
  ...
  =\Bigl\langle \eta_c,\sum_d t_d\langle \eta_d,r\rangle \eta_d\Bigr\rangle
   -\nu_c\sum_d t_d\nu_d\langle \eta_d,r\rangle
   +2\sum_d t_d\langle \eta_d,r\rangle
  =\langle \eta_c,\Omega r\rangle-\nu_c\langle\zeta,r\rangle ,
\]
...
\[
  ...
  =r^{T}\bigl(\Omega^2-\zeta\zeta^{T}\bigr)r ,
  \qquad
  \sum_c t_cu_c^2=r^{T}\Omega r .
\]
Lemma~\ref{lem:rowsum} now reads $r^{T}(\Omega^2-\zeta\zeta^{T})r<2\,
r^{T}\Omega r$, that is $r^{T}\Psi r<0$, contradicting the choice of $r$.
\end{proof}

\begin{theorem}[\LV]\label{thm:ranktwo}
...
No eigenvalue is computed and no spectral
theorem is used: Lemma~\ref{lem:pairobs} replaces the least eigenvalue of
a $2\times2$ matrix by a determinant, and everything after that is
quadratic algebra in the moments of the squared picture.  The argument is
uniform in $m$ and in the weights, which is what the induction of
Corollary~\ref{cor:heavy} requires.  It is exactly sharp at the rank-two
...
\begin{theorem}[\LV]\label{thm:coranktwo}
...
$\mathcal D$ be a design at $(k+2,k)$, and put $m:=k+2$.  Its kernel plane
carries an orthonormal basis, arranged as the columns of an $m\times2$
matrix $Z$ with rows $z_c^{*}$, and the co-weight rescaling
$w_c=z_c/\sqrt{1-t_c}\in\R^2$ satisfies
\[
  \sum_c(1-t_c)\,w_c^{\vphantom{T}}w_c^{T}\;=\;I_2 .
\]
...
\begin{equation}\label{eq:pairdom}
  w_a^{\vphantom{T}}w_a^{T}+w_b^{\vphantom{T}}w_b^{T}
  \;\psd\;T\;=\;\sum_c t_c\,w_c^{\vphantom{T}}w_c^{T} .
\end{equation}
With $R^{T}TR=I_2$ and $h_c:=R^{T}w_c$, condition \eqref{eq:pairdom} is
$h_a^{\vphantom{T}}h_a^{T}+h_b^{\vphantom{T}}h_b^{T}\psd I_2$, that is
...
The normalization in \eqref{eq:pairdom} is the whole content of the step.
...
comparing against $I_2$ instead of against $T$ in \eqref{eq:pairdom},
...
\begin{corollary}[\LV]\label{cor:corank}
...
\begin{lemma}[\LV]\label{lem:null}
Let $(g_c)_{c\le m}$ be vectors in $\R^k$ with $m>M(k)$.  There is
$\delta\in\R^m$, $\delta\ne0$, with
\begin{equation}\label{eq:moment}
  \sum_c \delta_c\,g_c^{\vphantom{T}}g_c^{T}=0
  \qquad\text{and}\qquad
  \sum_c \delta_c=0 .
\end{equation}
\end{lemma}
...
$\bigl(\sum_c\delta_c\,g_c^{\vphantom{T}}g_c^{T},\sum_c\delta_c\bigr)$ is
linear, and its target has dimension $k(k+1)/2+1=M(k)<m$.  A linear map
between real vector spaces of dimensions $m$ and $M(k)<m$ has nonzero
kernel.
\end{proof}

\begin{theorem}[\LV]\label{thm:reduce}
Let $D$ be a design at $(m,k)$ with $m>M(k)$.  There is a proper subset
$S$ of its index set and a design $D'$ at $(\lvert S\rvert,k)$ whose
...
\[
  s\;:=\;\min\Bigl\{\frac{t_c}{\delta_c}\;:\;\delta_c>0\Bigr\}\;>\;0 ,
\]
attained at $c^{*}$, and put $t'_c:=t_c-s\,\delta_c$.  These numbers are
nonnegative: if $\delta_c\le0$ then $t'_c\ge t_c>0$, and if $\delta_c>0$
then $s\le t_c/\delta_c$ gives $s\delta_c\le t_c$.  They sum to $1$ and
satisfy $\sum_ct'_c\,g_c^{\vphantom{T}}g_c^{T}=I_k$, by the two identities
\eqref{eq:moment}.  Finally $t'_{c^{*}}=0$.
...
\begin{theorem}[\LV]\label{thm:window}
Fix $k$.  If $\Gtz(m,k)$ holds for every $m\le k(k+1)/2+1$, then
$\Gtz(m,k)$ holds for every $m$.
\end{theorem}

\begin{proof}
Strong induction on $m$; the case $m\le M(k)$ is the hypothesis.  Let
$m>M(k)$ and let $D$ be a design at $(m,k)$.  Take $S$ and $D'$ as in
Theorem~\ref{thm:reduce} and put $m':=\lvert S\rvert<m$.  By the induction
hypothesis some $C\subseteq S$ with $\lvert C\rvert=k$ dominates in $D'$.
The subset sum
$\SC$ of \eqref{eq:dominate} does not involve the weights and the vectors
of $D'$ are those of $D$, so the subset sum of $C$ in $D$ is the subset
sum of $C$ in $D'$, and $C$ dominates in $D$.
\end{proof}
...
$g_c^{\vphantom{T}}g_c^{T}$ of the $k(k+1)/2$-dimensional space
...
subset of the original index set.  This is the point at which the
weight-freeness of \eqref{eq:dominate} does the work, and it is what
distinguishes this reduction from that of Theorem~\ref{thm:light}, where
the vectors move and \eqref{eq:reassemble} is needed to move the
conclusion back.

Nothing about domination enters Lemma~\ref{lem:null}, so nothing suggests
that $M(k)$ is the true threshold; see Problem~\ref{prob:higher}.
...
\begin{lemma}[\LV]\label{lem:repeat}
...
\[
  p^{T}(\SC-I_k)p=\sum_{c\in C}\langle g_c,p\rangle^2-\lVert p\rVert^2
  =-\lVert p\rVert^2<0 . \qedhere
\]
\end{proof}

\begin{proposition}[\LV]\label{prop:splitting}
$\Gtz(m+1,k)\Rightarrow\Gtz(m,k)$.
\end{proposition}

\begin{proof}
For $k=0$ there is nothing to prove.  Let $D$ be a design at $(m,k)$, so
$m\ge k$.  Define a design $D^{+}$ at $(m+1,k)$ by keeping the vectors
$g_1,\dots,g_m$ and appending $g_{m+1}:=g_1$, with weights
$t^{+}_1=t^{+}_{m+1}=t_1/2$ and $t^{+}_c=t_c$ for $2\le c\le m$.  Both
$\sum_ct^{+}_c$ and $\sum_ct^{+}_c\,g_c^{\vphantom{T}}g_c^{T}$ are
unchanged, so $D^{+}$ is a design.  By hypothesis some $T$ with
$\lvert T\rvert=k$ dominates in $D^{+}$.  By Lemma~\ref{lem:repeat}, $T$
does not contain both
$1$ and $m+1$, so the map fixing $1,\dots,m$ and sending $m+1$ to $1$ is
injective on $T$; its image $C$ has $k$ elements and the same subset sum,
hence dominates in $D$.
\end{proof}

\begin{theorem}[\LV]\label{thm:topcell}
$\Gtz\bigl(M(k),k\bigr)\Rightarrow\Gtz(m,k)$ for every $m$.  Hence the
...
$m\le M(k)$; Theorem~\ref{thm:window} then gives every $m$.
\end{proof}
...
\begin{proposition}[\LV]\label{prop:spike}
Let $1\le k\le m$.  Then $\Gtz(m+1,k+1)\Rightarrow\Gtz(m,k)$.
\end{proposition}

\begin{proof}
Let $D$ be a design at $(m,k)$ in which no $k$-subset dominates.  For each
...
block.  By hypothesis some $T$ with $\lvert T\rvert=k+1$ dominates there.
If $m+1\notin T$,
every selected vector has last coordinate zero and the last standard basis
vector gives $1\le0$.  So $m+1\in T$ and $C:=T\setminus\{m+1\}$ has $k$
...
\]
by \eqref{eq:unifgap}, a contradiction.  Hence some $k$-subset of $D$
dominates after all.
\end{proof}
