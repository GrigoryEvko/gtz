...
The counting of \S\ref{sec:first} decides rank one and the square $m=k$,
the rank-one criterion of \S\ref{sec:chart} decides corank one
(Theorem~\ref{thm:corankone}), and duality converts corank two into rank
two (Theorem~\ref{thm:duality}).  Rank two itself is thus the last small
case outstanding, and nothing so far bounds the size.  Both are supplied
by reductions: operations that produce from a design a smaller design
whose dominating subsets transport back.

They pay for the reduction in different currencies.  Deleting a vector of
length at most one costs one from the size but moves every surviving
vector by a congruence; the transport back is an identity between
quadratic forms, and it works only
because the deleted vector was short.  Carath\'eodory reduction leaves the
vectors where they are and cuts the size to $k(k+1)/2+1$, and there the
transport back is immediate, since $\SC$ in \eqref{eq:dominate} never
sees the weights.  Deletion stalls when every vector is long; at rank two
the stalled case can be handled directly.
...
light one is the dichotomy exploited here.
...
\]
using Cauchy--Schwarz and $t_d\ell_d\le t_d<1$.  By Lemma~\ref{lem:whiten}
applied to $\Sigma_d$ there is an $R$ with
...
$\sum_{c\in C}(g'_c)(g'_c)\tp=(1-t_d)R\tp\SC R$, and $I_k=R\tp\Sigma_dR$ by
\eqref{eq:whiten}, so domination reads
\[
  R\tp\bigl((1-t_d)\SC-\Sigma_d\bigr)R\;\psd\;0 ,
\]
and congruence by the invertible $R$ gives $(1-t_d)\SC-\Sigma_d\psd0$,
since $u\tp Au=(R^{-1}u)\tp(R\tp AR)(R^{-1}u)$ for every $u$ and every
symmetric $A$.  The identity
...
\begin{figure}[ht]
  \centering
  \input{figures/lightdelete}
  \caption{Theorem~\ref{thm:light} at $(3,2)$, with $g_3$ the light vector.
  Left, upstairs: the unit circle, which $g_3$ lies inside, and the ellipse
  $u\tp\Sigma_3u=1$, bounded because $t_3\ell_3=\tfrac{369}{625}<1$ and
  stretched by $\tfrac{25}{16}$ along $g_3$ alone.  Left, downstairs: the
  deflated design, both of whose vectors are heavy.  Right: the three steps
  of the proof, ending in \eqref{eq:reassemble} as a sum $X+Y$ of two
  positive semidefinite matrices.  The hypothesis $\ell_3\le1$ is used once,
  at the smaller eigenvalue $t_3(1-\ell_3)=\tfrac{31}{625}$ of $Y$; the other
  two pairs of this design fail, so the pair the theorem produces is the
  complement of the light index.}
  \label{fig:lightdelete}
\end{figure}

Lightness enters \eqref{eq:reassemble} at one place only, the second
summand.  That summand has eigenvalue $t_d(1-\ell_d)$ along $g_d$ and $t_d$
with multiplicity $k-1$ across it, so one of the $k$ can go negative, and
$\ell_d\le1$ stops it.  Figure~\ref{fig:lightdelete} runs the three
steps on a design at $(3,2)$.  Iterating the theorem removes light
...
design where every dominating triple has $\lmin(\SC)=1$.

The reductions consume the interior of the problem and leave the extremal
locus of \S\ref{sec:ties} standing.

\subsection{Rank two}\label{sec:ranktwo}

Fix a design at $(m,2)$ and write $g_c=(x_c,y_c)$.  Identify $\R^2$ with
$\C$ by $(x,y)\mapsto x+iy$ (a device for the computation below,
unrelated to the complex problem of \S\ref{sec:complex}), so that the
inner product of two vectors is the real part of the first complex number
times the conjugate of the second, and let $\eta_c\in\R^2$ correspond to
the square of the complex number attached to $g_c$:
\begin{equation}\label{eq:doubleangle}
  \eta_c\;:=\;\bigl(x_c^2-y_c^2,\;2x_cy_c\bigr),
  \qquad
  \hat\ell_c\;:=\;\ell_c-2 .
\end{equation}
The square is the quotient by $g_c\mapsto-g_c$, a sign that
$g_c^{\vphantom{\mathsf{T}}}g_c\tp$ does not see.  Squaring doubles
arguments and squares moduli, so $\lVert
...
\end{equation}
the second identity being Lagrange's.  Which sign the imaginary part
carries depends on the order of the pair, and it never matters: $b_{cd}$
occurs in \eqref{eq:wdot}, and everywhere below, squared.  Domination of
$\{c,d\}$ turns on $\ell_c$, $\ell_d$ and $b_{cd}^2$ alone, by
\eqref{eq:pairdet} below, and \eqref{eq:wdot} makes the last of the three
a function of the squared picture.
...
These three identities are the only use made of \eqref{eq:parseval} in
this subsection.
...
\]
where $e$ is the third index.  Every pair therefore dominates, and none
strictly.  We call this design the rank-two trine.
\end{example}
...
\end{equation}
On the other hand, eliminate $a_{cd}^2$ from \eqref{eq:wdot} in
\eqref{eq:obstruction} and use $\hat\ell=\ell-2$.  The two products
$\ell_c\ell_d$ cancel, leaving an expression affine in the leverages,
which \eqref{eq:pairdet} identifies:
\[
  K_{cd}
  ...
  =-2\det X .
\]
Now $\tr X=\ell_c+\ell_d-2>0$, and a symmetric $2\times2$ matrix of
positive trace is positive semidefinite exactly when its determinant is
nonnegative.  In one direction, $\det X\ge0$ gives
$X_{00}X_{11}\ge X_{01}^2\ge0$, so the two diagonal entries have the same
sign and the positive trace makes both nonnegative.  If $X_{00}=0$ then
$X_{01}=0$ as well and $X=\diag(0,X_{11})\psd0$.  If $X_{00}>0$, complete
the square:
\[
  X_{00}\,x\tp Xx\;=\;(X_{00}x_0+X_{01}x_1)^2+(\det X)\,x_1^2\;\ge\;0 ,
\]
and divide by $X_{00}$.  Conversely a positive semidefinite matrix has
nonnegative principal minors, and $\det X$ is one of them.  Hence
$X\psd0$ if and only if $K_{cd}\le0$.
\end{proof}
...
The inequality $K_{cd}\le0$ is therefore tight at a genuine design, and no
argument below may improve it to a strict inequality.
Figure~\ref{fig:obstruction} draws the arc of positions of $\eta_d$ that
Lemma~\ref{lem:pairobs} admits, and the ten pairs of a design at $(5,2)$,
four of which dominate.

\begin{figure}[ht]
  \centering
  \input{figures/obstruction}
  \caption{The pair obstruction \eqref{eq:obstruction} at $(5,2)$.  Left: the
  squared vectors of an all-heavy design, at distance $\ell_c$ from the
  origin.  The two circles drawn are those of the pair $\{1,3\}$, and the arc
  on the circle of radius $\ell_3=2$ carries the partners $\eta_d$ with
  $K_{1d}\le0$.  Every point of that arc makes an angle of at
  least $\psi^{*}$ with $\eta_1$, where $\cos\psi^{*}=\kappa(\ell_c,\ell_d):=
  \bigl[(\ell_c-2)(\ell_d-2)-2\bigr]/(\ell_c\ell_d)$; both
  $\kappa+1=2(\ell_c-1)(\ell_d-1)/(\ell_c\ell_d)$ and
  $1-\kappa=2(\ell_c+\ell_d-1)/(\ell_c\ell_d)$ are positive at a heavy pair,
  so the arc is proper and nonempty there and the two leverages by themselves
  decide no pair.  Centre: the ten pairs, four of them dominating.  Right: the
  trine of Example~\ref{ex:trine2}, where the cosine the design realizes and
  the threshold $\kappa(2,2)$ are both $-\tfrac12$, so each pair sits at an
  endpoint of the arc and $K_{cd}=0$.}
...
\begin{proof}
Summing \eqref{eq:obstruction} against $t_d$ and using the first and
third identities of \eqref{eq:balance2} together with $\sum_dt_d=1$,
\[
  \sum_d t_dK_{cd}
  ...
  =0-0+2 .
\]
Symmetry of $K$ is symmetry of the inner product.  Those two facts carry
\eqref{eq:core} on their own, for any symmetric array with row sums $2$;
\eqref{eq:allfail} enters only at the last clause of the statement, to
give the correction term a sign.  Expand $(u_c-u_d)^2$ and use the row sum
twice:
\[
...
$4\sum_ct_cu_c^2$ minus twice the left side; halve that and subtract it
from $2\sum_ct_cu_c^2$, and the left side comes back, which is
\eqref{eq:core}.  The diagonal terms of the correction vanish, so the
undetermined $K_{cc}$ do no harm; \eqref{eq:allfail} constrains $c\ne d$
only.  If $u$ is nonconstant there are $c\ne d$ with $u_c\ne u_d$, and
\eqref{eq:allfail} makes the corresponding summand strictly positive.
\end{proof}

Identity \eqref{eq:core} is sharp: the correction term vanishes
identically when $K$ vanishes off the diagonal, as at the rank-two trine.
It remains to produce a nonconstant $u$ for which the left side of
\eqref{eq:core} is at least $2\sum_ct_cu_c^2$.  The family $(\eta_c)$
either lies on a line through the origin or spans the plane, and the two
lemmas below share one ingredient, the barycentre identity
$\sum_ct_c\eta_c=0$ tested against a direction.  In the spanning case that
direction is the $r$ carrying the test vector $u_c=\langle
\eta_c,r\rangle$, and the identity makes $u$ nonconstant.  In the
collinear case it is the line itself, the identity forces a sign change
among the $\langle \eta_c,\cdot\rangle$, and \eqref{eq:obstruction}
settles the resulting antipodal pair without \eqref{eq:core}.
...
\eta_c\rVert\lVert r\rVert=\ell_c\lVert r\rVert\ne0$, the first equality
because $\eta_c$ is parallel to $r^{\perp}$ and $\lVert
r^{\perp}\rVert=\lVert r\rVert$, the nonvanishing because all-heavy makes
$\ell_c>1$.  By
...
\[
  K_{pq}=-\ell_p\ell_q-(\ell_p-2)(\ell_q-2)+2
        =-2(\ell_p-1)(\ell_q-1)<0 .
\]
Here the two products $\ell_p\ell_q$ add instead of cancelling, and what
survives factors into the two heaviness margins, both positive in an
all-heavy design.  By Lemma~\ref{lem:pairobs} the pair $\{p,q\}$
dominates.
\end{proof}
...
\eta_c,r\rangle$ does, so the spanning hypothesis gives $\Omega\pd0$.  Each
$\tr(\eta_c^{\vphantom{\mathsf{T}}}\eta_c\tp)$ is $\lVert
\eta_c\rVert^2=\ell_c^2$, so $\tr\Omega=\sum_ct_c\ell_c^2$, and the second
identity of \eqref{eq:balance2} with $\sum_ct_c=1$ gives
...
\]
At $u_c=\langle \eta_c,r\rangle$ the two sides of \eqref{eq:core} are
$r\tp(\Omega^2-\zeta\zeta\tp)r$ and $2\,r\tp\Omega r$, so $\Psi$ is their
difference, and under \eqref{eq:allfail} Lemma~\ref{lem:rowsum} forbids a
direction with $r\tp\Psi r\ge0$.  The proof produces one.
...
fit of $\hat\ell$ by a linear functional of $\eta$ is nonnegative.  By
Lemma~\ref{lem:whiten} choose $R$ invertible with $R\tp\Omega R=I_2$;
inverting both sides gives
$RR\tp=\Omega^{-1}$.  Let $r_1,r_2$ be its columns.  Then $\tr(R\tp\Omega^2R)
=\tr(\Omega^2RR\tp)=\tr\Omega$ and $\tr(R\tp\zeta\zeta\tp R)
=\zeta\tp\Omega^{-1}\zeta$, while the third term of $\Psi$ contributes
$-2\tr(R\tp\Omega R)=-2\tr I_2=-4$.  So by \eqref{eq:tromega} and
\eqref{eq:residual}
\[
...
is invertible.  $\Psi$ need not be positive semidefinite; one nonnegative
direction suffices, and the trace supplies it.

\emph{The test vector.}  Suppose no pair dominates, so
\eqref{eq:allfail} holds, and put $u_c:=\langle \eta_c,r\rangle$.  By
\eqref{eq:balance2}, $\sum_ct_cu_c=0$, so a constant $u$ would be
identically zero; then $r$ would be orthogonal to every $\eta_c$, hence
zero by the spanning hypothesis, which it is not.  So
Lemma~\ref{lem:rowsum} applies to $u$.  Compute both sides.  The first sum
below is $\Omega r$, which is $\Omega$ read as an operator rather than as
a moment:
\[
  \sum_d t_du_dK_{cd}
...
\]
the last term vanishing by \eqref{eq:balance2}.  Multiply by
$t_cu_c=t_c\langle \eta_c,r\rangle$ and sum: the first term becomes
$r\tp\Omega^2r$, which is $\langle\Omega r,\Omega r\rangle$ because
$\Omega$ is symmetric, and the second collapses by the definition of
$\zeta$,
\[
  \sum_{c,d}t_ct_dK_{cd}u_cu_d
...
No eigenvalue is computed anywhere in the proof.  Lemma~\ref{lem:pairobs}
trades the least eigenvalue of a $2\times2$ matrix for a determinant,
and what follows is quadratic algebra in the moments of the squared
picture, uniform in $m$ and in the weights as the induction of
Corollary~\ref{cor:heavy} demands.  The rank-two trine of
Example~\ref{ex:trine2} shows the argument has nothing to spare: there
$\Omega=2I_2$, $\zeta=0$ and $\sigma=0$, so $\Psi=\Omega^2-2\Omega=0$,
every direction is admissible in Lemma~\ref{lem:spanning}, and the strict
inequality of Lemma~\ref{lem:rowsum} fails, as it must where $K$ vanishes
off the diagonal.  Figure~\ref{fig:regression} draws the fit.

\begin{figure}[ht]
  \centering
  \input{figures/regression}
  \caption{Lemma~\ref{lem:spanning} as a fit.  By \eqref{eq:residual} the
  slack $\sigma-\zeta\tp\Omega^{-1}\zeta$ is the $t$-weighted squared
  residual of $\hat\ell$ against the linear functional
  $\langle\eta,\xi\rangle$, $\xi:=\Omega^{-1}\zeta$; disc area is the
  weight, and an atom under the fitted plane carries a dashed segment.
  Left: an all-heavy design at $(5,2)$, where $\det\Psi<0$ and $\Psi$ is
  indefinite.
  Right: the trine of Example~\ref{ex:trine2}, where $\hat\ell$ vanishes
  identically and the fit is exact.}
...
\eqref{eq:doubleangle} and the light/heavy split are theirs.  As a
statement the theorem is not more than theirs: the uniform result at every
size gives the weighted one by the transfer of \S\ref{sec:intro} --- split
each atom of a rational-weight design into equal copies, apply the uniform
result to the larger design, and pass to the limit, exactly as
\S\ref{sec:cladder} does over $\C$ through Lemma~\ref{lem:split}.  What is
new is the proof at arbitrary weights, which costs the reweighting of
Theorem~\ref{thm:light} in the light branch and replaces the
Perron--Frobenius step of \cite{SenguptaPautov2026} by
Lemma~\ref{lem:rowsum} and the trace computation of
Lemma~\ref{lem:spanning} in the heavy branch.  It is also what the
formalization carries: the transfer argument is not mechanized and the
direct one is.

At uniform weights the designs whose $K$ vanishes off the diagonal are
classified in \cite{Nesterenko2026real}: the squared vectors fall into
three clusters of equal vectors, of sizes $p,q,r$ with $p+q+r=m$, the
cluster of size $p$ having length
...
matrix $Z$ with rows $z_c\tp$, and the co-weight rescaling
$w_c=z_c/\sqrt{1-t_c}\in\R^2$ satisfies
\[
  \sum_c(1-t_c)\,w_c^{\vphantom{\mathsf{T}}}w_c\tp
  \;=\;\sum_cz_c^{\vphantom{\mathsf{T}}}z_c\tp\;=\;Z\tp Z\;=\;I_2 ,
\]
the rescaling cancelling the co-weight at each index.
...
domination of the pair $\{a,b\}$ in $(h_c,t_c)$, which is itself a design
at $(k+2,2)$ because $\sum_ct_ch_c^{\vphantom{\mathsf{T}}}h_c\tp
=R\tp\Sdual R=I_2$.
...
two fails at every size above three (Theorem~\ref{thm:complex}), and
accordingly $\GtzC(k+2,k)$ is false for every $k\ge2$, since
Corollary~\ref{cor:dualcell} identifies it with $\GtzC(k+2,2)$.
...
The bound is Carath\'eodory's \cite{Caratheodory1911}: condition
...
$\mathrm{Sym}_k$, and a point of a convex hull in dimension $k(k+1)/2$ is
a convex combination of at most $\win(k)$ of the points
(\cite{Rockafellar1970}, \S17).  The proof of Theorem~\ref{thm:reduce} is
the standard support-reduction step of that theorem, carried out by hand
because the representation must use a \emph{subfamily of the given
vectors} rather than merely few matrices: only then is the dominating
subset produced downstairs a subset of the original index set.  The
transport back uses the weight-freeness of \eqref{eq:dominate}, and it
...
Figure~\ref{fig:psdcone} draws $\mathrm{Sym}_2$.  In the design at $(5,2)$ of
Figure~\ref{fig:crystal} the moment map has rank $4$, so its kernel is a
line, and the walk along that line leaves the simplex at $\theta=\tfrac1{56}$.

\begin{figure}[ht]
  \centering
  \input{figures/psdcone}
  \caption{The cone $\{A\psd0\}$ in $\mathrm{Sym}_2$.  The three coordinate
  directions are Frobenius-orthogonal of equal length, so the cone is circular,
  of half-angle $\pi/4$ about the ray of scalar matrices.  Left: the rank-two
  trine of Example~\ref{ex:trine2}.  Its three atoms lie on the boundary circle
  at height $1$, and \eqref{eq:parseval} exhibits $I_2$ as their centroid.
  Right: the horizontal projection.  The atom of $g_c$ lands at radius
  $\ell_c/2$ and at argument $2\theta_c$, twice that of $g_c$: the dot at the
  midpoint of $\eta_c$.  So $g_c$ and $-g_c$ give the same point, and the
  doubling is \eqref{eq:doubleangle}.  The window $\win(2)$ of
  Theorem~\ref{thm:window} is the Carath\'eodory number of this space.}
  \label{fig:psdcone}
\end{figure}

\begin{figure}[ht]
  \centering
  \input{figures/crystal}
  \caption{Theorem~\ref{thm:reduce} at $(5,2)$, in the coordinates of
  Figure~\ref{fig:psdcone}.  Left: five rank-one atoms on the boundary of the
  cone, at heights $\ell_c/2$, with $I_2$ their $t$-barycentre by
  \eqref{eq:parseval}; disc area is the weight, and the signed integer beside
  an atom is its coordinate of $\delta$.  Since
  $\dim\mathrm{Sym}_2+1=\win(2)=4<5$ the moment map has a kernel, here the
  line spanned by $\delta=(-9,1,3,-3,8)$.  Right: the five weights
  $t_c-\theta\delta_c$ along that line, every one of them a Parseval
  weighting, all nonnegative exactly on
  $-\tfrac1{42}\le\theta\le\tfrac1{56}$.  At the right end the ringed atom
  loses its weight and the design drops to $(4,2)$ with
  $t'=\tfrac18(3,1,3,1,0)$; the atoms themselves never moved, which leaves
  the smaller design on a subfamily of the given vectors.}
  \label{fig:crystal}
\end{figure}
...
sizes.  Splitting one vector of a design into two equal copies of half the
weight pads it to the next size up, and a $k$-subset containing both
copies never dominates.
...
At $k=3$ the bound of Theorem~\ref{thm:window} is $\win(3)=7$ and
Corollary~\ref{cor:corank} disposes of every size up to five.  A
sufficient condition in the first two spectral moments is available at
this rank (Proposition~\ref{prop:twomoment}); \S\ref{app:moments} shows
it decides no triple of the icosahedral design.

\begin{corollary}[Lean]\label{cor:twocells}
...
at $(7,3)$ it consumes $(6,3)$ and $(6,2)$.
...
p_C\rVert^2$; let $\sigma$ be the average of $1$ and the largest of the
finitely many ratios $\sum_{c\in C}\langle g_c,p_C\rangle^2/\lVert
p_C\rVert^2$.  Every ratio is below $1$, hence so is their maximum, and
$\sigma$ lies strictly between the two; in particular $\sigma\in(0,1)$ and
...
Figure~\ref{fig:ladder} collects the implications.

\begin{figure}[ht]
  \centering
  \input{figures/ladder}
  \caption{Rank three.  The box on the left is
  Corollary~\ref{cor:corank}; the filled arrows are
  Proposition~\ref{prop:splitting} and Theorem~\ref{thm:window}; the open
  arrows are the boundary analysis of \S\ref{sec:compactness}, which at a
  cell consumes the two cells one size below.  Dashed arrows carry the
  two unproved inputs of \S\ref{sec:interior}.}
  \label{fig:ladder}
\end{figure}
