...
Statement (a) is unavailable: it does not follow from real-rootedness of $q$
alone (Proposition~\ref{prop:rootedness}), and the interlacing method of
\cite{MSS2015} requires the whole convex family to be real-rooted, a property
of volume sampling that has not been verified.  Statement (b) is worse than
unavailable.  It is false, at both residual cells of rank three, with exact
witnesses at which $\Gtz$ itself holds.  At $(6,3)$ take the six directions
of the $D_3$ root system at uniform weight $\tfrac16$ --- every leverage
equal to $3$, so the design is as isotropic as a design of that size can
be.  There $q(1)=\tfrac7{16}>0$ while $q(0)<0$, so a root lies strictly
inside $(0,1)$; it is $0.94007\ldots$
(\texttt{Gtz.not\_mixed\allowbreak Root\allowbreak At\allowbreak Least\allowbreak One\_six\allowbreak Three}).  At $(7,3)$ take the four
tetrahedron directions together with three doubled coordinate axes:
$q(1)=\tfrac{11}{256}>0$ and the least root is $0.99437\ldots$
(\texttt{Gtz.not\_mixed\allowbreak Root\allowbreak At\allowbreak Least\allowbreak One\_seven\allowbreak Three}).  Each cell is refuted on
its own, and both together
(\texttt{Gtz.not\_mixed\allowbreak Root\allowbreak At\allowbreak Least\allowbreak One\_rank\allowbreak Three\allowbreak Residuals}).  What the
witnesses kill is the bound, not the conjecture.

That the margin in (b) is zero anyway was already forced from the other
side, by Proposition~\ref{prop:qone}: at a design where every basis has $1$
in the spectrum of $\SC$ one has $q(1)=0$, and at the tetrahedron
$q=(x-1)(x-4)^2$ (\S\ref{app:tetra}).  The witnesses above push the least
root below $1$, so the mixture is not merely tight but wrong.  Any
derandomization of Theorem~\ref{thm:average} must replace the mixture, not
sharpen it.
...
At general weights the rule fails outright:
Proposition~\ref{prop:volumefails} exhibits a design at $(7,3)$ where the
maximizer of the volume $\pi_C$ of \eqref{eq:dpp} does not dominate and
another triple does.  What breaks there is the weighting by
$\prod_{c\in C}t_c$, not the determinant, so the uniform-weight question is
...
\texttt{Gtz.exists\_\allowbreak designs\_\allowbreak with\_\allowbreak disjoint\_\allowbreak domination\allowbreak Sets}).  Invariant data
...
(\texttt{Gtz.exists\_\allowbreak symmetry\_\allowbreak with\_\allowbreak no\_\allowbreak fixed\_\allowbreak dominating\allowbreak Subset}), so every
...
(\texttt{Gtz.heaviest\_\allowbreak atom\_\allowbreak can\_\allowbreak lie\_\allowbreak outside\_\allowbreak every\_\allowbreak dominating\allowbreak Subset},
\texttt{Gtz.heavy\allowbreak Pivot\allowbreak Design\_\allowbreak all\allowbreak Heavy}).
...
Local search fares no better.  A score that improves within a bounded
exchange radius at every non-dominating subset would prove $\Gtz(m,k)$ by a
finite argmax; \S\ref{app:exchange} refutes the hypothesis for the three
natural scores and states the boundary, which is that no radius survives
with content rather than that every radius is refuted.  Every witness there
satisfies $\Gtz$ at its own cell.
