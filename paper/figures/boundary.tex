...
\node[txt, anchor=north] at (2.3,1.26) {$(P^{\star},t^{\star})$};
\node[txt, anchor=north] at (2.3,-0.06) {$t_z=0$};

\node[txt, anchor=north] at (2.3,-0.52)
  {closed simplex $\Delta_{m-1}$: $t_c\ge0$, $\sum_c t_c=1$;\\[1pt]
...
\node[txt, anchor=north west, text width=74mm, align=left] at (0,5.05)
...
\node[txt, text width=35mm, align=center] (a2) at (1.95,2.52)
  {the column $Pe_z$ vanishes;\\ delete $z$, rescale $t\mapsto t/(1-t_z)$};
\node[bx] (a3) at (1.95,1.40) {$(m-1,k)$};
\node[txt, text width=35mm, align=center] (a4) at (1.95,0.78) {$C$ lifts verbatim};
\node[bx] (a5) at (1.95,0.15) {$|C|=k$};
\draw[ar] (a1) -- (a2); \draw[ar] (a2) -- (a3);
\draw[ar] (a3) -- (a4); \draw[ar] (a4) -- (a5);

% ---- case B : deflation at the pivot --------------------------------
\node[ttl] at (5.75,4.05) {Schur deflation};
\node[bx] (b1) at (5.75,3.62) {$P_{zz}>0$};
\node[txt, text width=35mm, align=center] (b2) at (5.75,2.52)
  {$P'=P-(Pe_z)(Pe_z)^{T}/P_{zz}$,\\ $\tr P'=k-1$, $P'e_z=0$; delete $z$};
\node[bx] (b3) at (5.75,1.40) {$(m-1,k-1)$};
\node[txt, text width=35mm, align=center] (b4) at (5.75,0.78) {$C\mapsto C\cup\{z\}$};
\node[bx] (b5) at (5.75,0.15) {$|C\cup\{z\}|=k$};
\draw[ar] (b1) -- (b2); \draw[ar] (b2) -- (b3);
\draw[ar] (b3) -- (b4); \draw[ar] (b4) -- (b5);

% ---- the identity that pays for the second case ---------------------
\node[txt, anchor=north] at (3.85,-0.45)
  {$x^{T}Wx=x^{T}(P'-\diag t)\,x+\dfrac{(Pe_z\cdot x)^{2}}{P_{zz}}$,\\[3pt]
