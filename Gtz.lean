...
-- The gauge wall cuts the spanning tree list from sixteen to nine.  The
-- triangle principal block of the complement matrix is a positive multiple of
-- the rank-one matrix generated by the sign pattern (1,-1,1), so the complement
-- form vanishes at an explicit nonzero vector supported on any TWO triangle
-- labels.  A selection whose omitted set carries two triangle labels is
-- therefore never positive definite, at every omitted cardinality.  Spent on
