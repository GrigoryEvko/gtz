...
/--
CURRENT STATUS: the open formula is `Gtz.KFourKnifeBandRefinedPathKernelOrStarWeakToStrict`. It is restricted to spanning trees and lies outside Layer A, the exchange star, and the seventeen-cell all-tree unsigned minor atlas. Every one of the sixteen spanning trees carries a moduli-only cell. Atlas blindness at its canonical maximal floors gives an explicit nonzero nonnegative adjugate witness, a named diagonally non-dominant row, and one of three division-free polynomial bad-edge budgets for every tree. The formerly opaque weak-tree existential is now classified exactly: a weak path arrives with its concrete nonzero pulled-back gap-kernel direction, while the only alternative is a weak vertex star. All twelve paths are saturated; the four stars are the only tree type whose three fundamental triangles cannot be simultaneously sign-saturated. `Gtz.kFourKnifeBandRefinedPathKernelOrStar_iff` proves exact equivalence with the public refined knife band. The exact rational Gershgorin washout point satisfies a bad row for all sixteen trees while possessing strict trees, so the bad-row or bad-budget system alone is not the remaining theorem. The chart-heavy gate is absent because it is automatic. No inhabitant of the sharpened residual is currently proved. The following long STATUS paragraph is retained as a historical pre-unsigned-cell snapshot and is superseded by this line.
...
axiom obligationKnifeBandRefinedKFour :
    Gtz.KFourKnifeBandRefinedPathKernelOrStarWeakToStrict
...
theorem obligationKFourFamilySelection : Gtz.KFourFamilySelection :=
  Gtz.kFourFamilySelection_iff_pathKernelOrStar.mpr
    obligationKnifeBandRefinedKFour

/-- The path-kernel-or-star A3 axiom reconstructs the former refined knife band
by spending all seventeen cells, the full sixteen-tree obstruction ledger, the
twelve exact path-kernel couplings, the exhaustive path/star classifier, and
the automatic chart-heavy law. -/
theorem obligationKnifeBandRefinedKFour_full :
    Gtz.KFourKnifeBandRefinedWeakToStrict :=
  Gtz.kFourKnifeBandRefinedPathKernelOrStar_iff.mp
    obligationKnifeBandRefinedKFour
