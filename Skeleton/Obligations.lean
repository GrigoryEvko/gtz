...
* Root A, rank three: `forall n, 0 < n -> Gtz.GtzOriginal n 3`.  Rests on FIVE
  ...
  - `obligationKnifeBandRefinedKFour` =
    `Gtz.KFourKnifeBandRefinedAllTreeBlindWeakToStrict`, strictness demanded
    ONLY off Layer A, the exchange star, and the seventeen-cell all-tree minor
    atlas: the prior ten cells plus the seven formerly missing path cells.
    Every one of the sixteen spanning trees now has a moduli-only cell.  Both
...
/--
CURRENT STATUS: the open formula is `Gtz.KFourKnifeBandRefinedAllTreeBlindWeakToStrict`. It is restricted to spanning trees and lies outside Layer A, the exchange star, and the seventeen-cell all-tree unsigned minor atlas. The prior ten cells cover the four stars and five paths (with two certificates at the gauge star); the seven row-atlas producers fill the seven missing paths, so every one of the sixteen spanning trees now carries a moduli-only cell. `Gtz.kFourKnifeBandRefinedAllTreeBlind_iff` proves exact equivalence with the public refined knife band. The chart-heavy gate is absent because it is automatic. `Gtz.pathCell015WitnessPoint_fires` proves a newly removed path region is inhabited; no inhabitant of the sharpened residual is currently proved. The following long STATUS paragraph is retained as a historical pre-unsigned-cell snapshot and is superseded by this line.
...
axiom obligationKnifeBandRefinedKFour :
    Gtz.KFourKnifeBandRefinedAllTreeBlindWeakToStrict

/-- The all-tree-atlas-blind A3 axiom reconstructs the former refined knife
band by spending all seventeen cells and the automatic chart-heavy law. -/
theorem obligationKnifeBandRefinedKFour_full :
    Gtz.KFourKnifeBandRefinedWeakToStrict :=
  Gtz.kFourKnifeBandRefinedAllTreeBlind_iff.mp
    obligationKnifeBandRefinedKFour
...
all-tree minor certificate cells.  The residual quantifiers range only
