...
* Root A, rank three: `forall n, 0 < n -> Gtz.GtzOriginal n 3`.  Rests on FIVE
  ...
  - `obligationBaseTripleTightUThreeSix` =
    `Gtz.BaseTripleTightLineFreeOffConicWeakToStrict` (the design-level
    weak-to-strict upgrade with `PosSemidef` antecedent kept, the weak triple
    pinned at `{0,1,2}` by the relabelling-invariance suite, and the tie's own
    tight direction carried);
...
/--
STATUS: open -- THE EXACT U(3,6) RESIDUAL: `Gtz.BaseTripleTightLineFreeOffConicWeakToStrict`, the design-level weak-to-strict upgrade on the line-free off-conic stratum with the `PosSemidef` antecedent KEPT (load-bearing: the fiber margin infimum over masses and weights is zero at every direction tuple, so the antecedent-free sibling is out of reach), the weak triple PINNED at the base triple {0,1,2}, and the tie's own tight direction CARRIED as an explicit hypothesis. Everything between this Prop and the class statement is a theorem: `Gtz.lineFreeOffConicWeakToStrict_of_baseTripleTight` spends the five-lemma relabelling-invariance suite (which is what lets the triple be pinned) and the pointwise KKT extraction `Gtz.exists_tightDirection_of_dominates_not_posDef` (which is what supplies the tight direction) to reach `Gtz.LineFreeOffConicWeakToStrict`, kept alive below as the theorem `obligationWeakToStrictUThreeSix`, from which `Gtz.stressFreeStratumIsTieFree_lineFree_of_weakToStrict` discharges `obligationTieFreeUThreeSix` verbatim, converting stress-freeness to off-conic through `Gtz.stratumStressHasFullSupport_lineFree` (TrichotomyLedger.lean:524; the single MIXED class -- stress-free here means OFF-CONIC, and that excluded conic sublocus is exactly where the margin degenerates; the (4,3) tetrahedron and (5,3) diamond tie shadows live there).
