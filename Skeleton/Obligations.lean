...
STAGE-7 UPDATE (2026-08-16). This axiom now HAS producers, and the STATUS line above is stale on that point. Two of them are landed: `Gtz.obligationSubThresholdBandHinge_of_excessDominates` (Gtz/Wave/OffsetUpperBound.lean:1446) and the strictly weaker `Gtz.obligationSubThresholdBandHinge_of_primitiveExcessDominates` (Gtz/Wave/ThresholdCellDominance.lean:167). Each asks for an excess-dominated selection at every design of every band cell. NEITHER IS REFUTED, and that is the point worth carrying: the complete-graph witness that kills the same instrument at the THRESHOLD cell sits at `size = rank * (rank + 1) / 2`, which is a different cell family, and this band is empty at rank three. So no refutation in this tree touches these two producers. THE OPEN CHECK, and it is cheap: at rank four the band is sizes eight and nine, and the complete graph on five vertices less two edges carries five vertices, eight edges and cycle rank four, so its graphic design sits inside the band. If the crowdedness obstruction of Gtz/Wave/ThresholdCellDominance.lean bites there, both producers die and this axiom returns to zero live producers, which is what the STATUS line claims. If it does not bite, the primitive producer is a live route and no fork has ever worked it. Either answer redirects the lane, and nobody has run it. CAUTION on the climbing route named under ATTACK: `Gtz.UniformPositionBridge.gtzWeighted_belowWindow_of_predecessor` needs `Gtz.GtzWeightedAll (rank - 1)`, so at rank four it needs rank three. That route is DOWNSTREAM of the five on-path obligations and is not a parallel lane.
