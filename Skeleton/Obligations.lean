...
* Root A, rank three: `forall n, 0 < n -> Gtz.GtzOriginal n 3`.  Rests on FIVE
  ...
  - `obligationKnifeBandRefinedKFour` =
    `Gtz.KFourKnifeBandRefinedAllPathSaturatedWeakToStrict`, strictness demanded
    ...
    one of the sixteen spanning trees.  On each of the twelve path trees, the
    full dual vector is coupled through an exact signed pullback to a nonzero
    kernel direction of the actual weak chart gap.  The four vertex stars are
    the only sign-frustrated tree type left outside this coupling.  The exact
    washout witness shows that the bad-row layer alone cannot decide coverage,
    so the residual deliberately retains the full dual vectors.  Both quantifiers are
    restricted to the sixteen spanning trees.  The chart-heavy gate is gone
    because six positive chart weights sum to one and therefore always contain
    a label of weight at least `1/10`.
...
/--
CURRENT STATUS: the open formula is `Gtz.KFourKnifeBandRefinedPathWindowResidualOrStarWeakToStrict`. It is restricted to spanning trees and lies outside Layer A, the exchange star, and the seventeen-cell all-tree unsigned minor atlas. Every one of the sixteen spanning trees carries a moduli-only cell. Atlas blindness at its canonical maximal floors gives an explicit nonzero nonnegative adjugate witness, a named diagonally non-dominant row, and one of three division-free polynomial bad-edge budgets for every tree. The formerly opaque weak-tree existential is now classified exactly. A weak path arrives with its concrete nonzero pulled-back gap-kernel direction and an outside repair pointer whose every host selection reads strictly positively on that same direction. Adding the pointer produces a positive-semidefinite four-edge window which is strictly positive on the old path kernel. Every positive-definite window with some old-edge pivot below one is now discharged by the exact deletion rung and dependent-triple exclusion. The axiom therefore sees only three alternatives: a weak vertex star; a positive-definite path window whose three deletion pivots are all at least one; or a path window with a second kernel direction not collinear with the old one. All twelve paths are saturated, pointed, windowed, and reduced to those two exact walls; the four stars are the only tree type whose three fundamental triangles cannot be simultaneously sign-saturated. `Gtz.kFourKnifeBandRefinedPathWindowResidualOrStar_iff` proves exact equivalence with the public refined knife band. The exact rational Gershgorin washout point satisfies a bad row for all sixteen trees while possessing strict trees, so the bad-row or bad-budget system alone is not the remaining theorem. The chart-heavy gate is absent because it is automatic. No inhabitant of the sharpened residual is currently proved. The following long STATUS paragraph is retained as a historical pre-unsigned-cell snapshot and is superseded by this line.
...
axiom obligationKnifeBandRefinedKFour :
    Gtz.KFourKnifeBandRefinedPathWindowResidualOrStarWeakToStrict
...
theorem obligationKFourFamilySelection : Gtz.KFourFamilySelection :=
  Gtz.kFourFamilySelection_iff_pathWindowResidualOrStar.mpr
    obligationKnifeBandRefinedKFour

/-- The path-window-or-star A3 axiom reconstructs the former refined knife band
...
path's outside repair pointer, its four-edge window dichotomy, the exact deletion
pivot rung, and the automatic chart-heavy law. -/
theorem obligationKnifeBandRefinedKFour_full :
    Gtz.KFourKnifeBandRefinedWeakToStrict :=
  Gtz.kFourKnifeBandRefinedPathWindowResidualOrStar_iff.mp
    obligationKnifeBandRefinedKFour
