...
* Root A, rank three: `forall n, 0 < n -> Gtz.GtzOriginal n 3`.  Rests on FIVE
  ...
  - `obligationKnifeBandRefinedKFour` =
    `Gtz.KFourKnifeBandRefinedPendantBlindWeakToStrict`, strictness demanded
    ONLY off Layer A, the exchange star, and all ten unsigned minor
    certificate cells: the six gauge/band/vertex cells from the previous
    layers and the four pendant cells around the `{2, 3}` matching.  Both
...
/--
CURRENT STATUS: the open formula is `Gtz.KFourKnifeBandRefinedPendantBlindWeakToStrict`. It is restricted to spanning trees and lies outside Layer A, the exchange star, and all ten cells of the full unsigned minor atlas: the prior gauge-star minor, band-tree, gauge-star trace, and three vertex-star cells, plus the four pendant cells around matching `{2,3}`. `Gtz.kFourKnifeBandRefinedPendantBlind_iff` proves exact equivalence with the public refined knife band. The chart-heavy gate is absent because it is automatic. `Gtz.coverageRefuterTwo_pendantCell123Fires` proves the newly removed pendant region is inhabited; no inhabitant of the sharpened residual is currently proved. The following long STATUS paragraph is retained as a historical pre-unsigned-cell snapshot and is superseded by this line.
...
axiom obligationKnifeBandRefinedKFour :
    Gtz.KFourKnifeBandRefinedPendantBlindWeakToStrict

/-- The full-minor-atlas-blind A3 axiom reconstructs the former refined knife
band by spending all ten unsigned cells and the automatic chart-heavy law. -/
theorem obligationKnifeBandRefinedKFour_full :
    Gtz.KFourKnifeBandRefinedWeakToStrict :=
  Gtz.kFourKnifeBandRefinedPendantBlind_iff.mp
    obligationKnifeBandRefinedKFour

/-- **Discharged from the sharpened axiom.**  Same name, same statement.  Four
covered families are now spent: Layer A, the exchange star, and all ten
unsigned minor certificate cells.  The residual quantifiers range only
