...
/--
STATUS: chart-covered, analytic half open. The covering half is the unconditional `Gtz.parameterizedChartCovers_threeLinesDirection` (Gtz/Design/RigidityBridge.lean:1098) with consumer `Gtz.stressFreeStratumIsTieFree_threeLines_of_chart` (:1135); the stratum is uniformly stress-free (`Gtz.stratumIsStressFree_threeLines`, Gtz/Reduction/TrichotomyLedger.lean:497). What is open is the budget-blind tenth-heavy residual `Gtz.ChartTieFreeThreeLinesFundamentalDomainTenthHeavyBudgetBlind` on HALF the admissible parameter line, the fundamental domain `1 <= |slide|`. Three landed engines have fired before the axiom. First, the chart at `slide` is realized over the chart at `1/slide`, so only the fundamental domain remains. Second, `Gtz.exists_design_of_chartPoint` and the unconditional strict tenth floor remove every all-light point. Third, the allocated Cauchy--Schwarz certificate removes both canonical semialgebraic cells: the vertex triple `{0,1,3}` and the free triple `{2,4,5}`. The live point is therefore tenth-heavy and admits neither `Gtz.ThreeLinesVertexBudgetCellFires` nor `Gtz.ThreeLinesFreeBudgetCellFires`. `Gtz.chartTieFreeThreeLinesFundamentalDomainTenthHeavyBudgetBlind_iff` proves the narrowed formula equivalent to the former A2 statement; it is not a stronger sufficient condition.
CONSUMERS: `obligationStressFreeHingeSixThree` (the split parent, now a theorem), hence the rank-three capstone.
WHY OPEN: the budget inequalities are sufficient cells, not an exhaustive cover. The two exact rational witnesses prove both cells are inhabited, but no theorem says every tenth-heavy weak point enters one. The strict-triple producer `Gtz.directionChartIsTieFree_of_hasStrictTriple` remains false at a degenerate direction, and the reindexing producer only moves a certificate from one slide to another.
ATTACK: cover the complement of the two explicit allocation cells, PARAMETRICALLY in `slide`, or add further allocation charts and spend them through the same budget-blind IFF. The `Z/2` involution is ALL the symmetry available: the pattern automorphism group is `S3` on the triangle {0,1,3}, acting on the Menelaus modulus `J = slide` by 3-cycles trivially and by inversion on transpositions, so halving is the ceiling of the symmetry route and no further region carving is free. The two excluded slides are degenerations onto a collapsed atom or the M(K4) pattern; they are not gaps.
...
axiom obligationChartTieFreeThreeLinesFundamentalDomain :
    Gtz.ChartTieFreeThreeLinesFundamentalDomainTenthHeavyBudgetBlind

/-- The budget-blind A2 axiom reconstructs the former fundamental-domain
statement by spending chart whitening, the strict tenth-light theorem, and
both allocated budget cells. -/
theorem obligationChartTieFreeThreeLinesFundamentalDomain_full :
    Gtz.ChartTieFreeThreeLinesFundamentalDomain :=
  Gtz.chartTieFreeThreeLinesFundamentalDomain_of_budgetBlind
    obligationChartTieFreeThreeLinesFundamentalDomain
