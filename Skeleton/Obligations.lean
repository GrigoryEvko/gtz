...
* Root A, rank three: `forall n, 0 < n -> Gtz.GtzOriginal n 3`.  Rests on FIVE
  ...
  - `obligationKnifeBandRefinedKFour` =
    `Gtz.KFourKnifeBandRefinedTreeStarRefusedAllMaxHeavyWallWeakToStrict`, strictness demanded
...
STAGE-7 UPDATE (2026-08-16). The weight-uniform entry in the NOT-REFUTED field below was met a SECOND time, from the moment side, and it cost the campaign two cycles. Four theorems reached the five on-path obligations from a criterion stated at uniform weight, each paying for the bridge with the hypothesis `forall design : WeightedDesign 6 3, forall label, design.weight label = 1/6`. That asserts every design is uniform, and `Gtz.nonUniformLeverageTieDesign` (Gtz/Ties/NonUniformLeverageTie.lean:67, weights one ninth, one ninth, one ninth, two ninths, one ninth, two ninths) refutes it outright. All four are DELETED, not annotated, because a door that cannot open type-checks and reads as an asset. **RULE, carried from this: an unsatisfiable antecedent is a trap, not a graveyard entry. Delete it and record the reason at the site.** The criteria themselves survive, because each takes the uniform weight as its own antecedent at ONE design, and their per-design producers are untouched. **A HIGH MEASUREMENT ON THE UNIFORM STRATUM IS NOT EVIDENCE ABOUT THIS OBLIGATION**: one of the deleted doors rested on a criterion holding at 800 of 800 exact designs. The obligation quantifies over the whole stratum. NOT ONLY THE FLAT LOCUS: uniform weight and the flat diagonal are INDEPENDENT hypotheses, because `Gtz.projectionOfDesign_diagonal` gives `P_cc = w_c * leverageOf (a_c)` and a design carries no unit-norm constraint. Uniform weight forces `P_cc = 1/2` only when every atom has squared norm three. Any brief that treats them as one condition is wrong.
...
#BOGUS (2026-08-17, verified by read): the last clause of WHY OPEN and the whole first sentence of ATTACK are FALSE. The chart EXISTS and is pinned. `Gtz.oneLineDirection` (Gtz/Design/OneLineChart.lean:66) is the three-parameter chart, `Gtz.parameterizedChartCovers_oneLineDirection` (Gtz/Design/OneLineCovering.lean:400) is its covering with ZERO hypotheses, and `Gtz.stressFreeStratumIsTieFree_oneLine_of_chart` (Gtz/Design/OneLineCovering.lean:460) is its consumer in the same module. All three are pinned in Gtz/Audit.lean at :31109, :31131 and :31132. `Gtz/Wave/WiringLineChartRoads.lean` (commit d24ea8c) records the same correction and adds the second route `Gtz.oneLineOnPath_of_chart` (:154). ONLY THE ANALYTIC HALF IS OPEN: tie-freeness of the chart at every admissible parameter. Do NOT send a fork to build a chart. The admissible range is inhabited (`Gtz.isAdmissibleOneLineParameter_two_three_four`), so the demand is real and not vacuous.
#STALE: the STATUS line above names `Gtz.OneLineTenthHeavyJointBlindLineSparse`, but the axiom below now asserts `Gtz.OneLineSeededLineSparse`. The two are interderivable by `Gtz.oneLineSeededLineSparse_iff`, so no logical drift follows, and the STATUS text still names the previous Prop.
...
WHY OPEN: same producer situation as the one-line class -- no chart exists for this pattern and the plane-pair filter is structurally inapplicable.
#REGISTRY-STALE (2026-08-17): THE CLAUSE "no chart exists for this pattern" IS FALSE, AND THE ATTACK FIELD BELOW REPEATS IT. The chart is `Gtz.twoMeetingLinesDirection` (Gtz/Design/TwoMeetingLinesChart.lean:70), two parameters, admissibility `Gtz.IsAdmissibleTwoMeetingLinesParameter` (:93). The covering half is UNCONDITIONAL and sorry-free: `Gtz.parameterizedChartCovers_twoMeetingLinesDirection` (Gtz/Design/TwoMeetingLinesCovering.lean:283), and it does not even use primitivity. The road to this axiom's own Prop is closed end to end: `Gtz.stressFreeStratumIsTieFree_twoMeetingLines_of_chart` (:336), `Gtz.twoMeetingLinesOnPath_of_chart` (Gtz/Wave/WiringLineChartRoads.lean:164). A sample admissible parameter is in kernel, `Gtz.isAdmissibleTwoMeetingLinesParameter_two_three` (:199). WHAT IS ACTUALLY OPEN is the analytic half only: `forall param, IsAdmissibleTwoMeetingLinesParameter param -> DirectionChartIsTieFree (twoMeetingLinesDirection param)`. Read Gtz/Design/TwoMeetingLinesCovering.lean before you brief anyone on this field.
#UNSPENT (2026-08-17): the class's own corpus is landed and wired to nothing. `Gtz.twoMeetingLinesTransversalStrict_iff_invariants` (Gtz/Design/TwoMeetingLinesNeedle.lean:226) writes this axiom's whole conclusion as twelve polynomial inequalities in `Gtz.tripleLeverageSum`, `Gtz.triplePairAreaSum` and `Gtz.atomBracket`. `Gtz.twoMeetingLinesResidual_of_min_complLeverage_lt` (Gtz/Design/TwoMeetingLinesComplementCap.lean:204) and `Gtz.twoMeetingLinesResidual_of_min_weightedCompl_lt` (Gtz/Design/TwoMeetingLinesParsevalCap.lean:150) each discharge the residual on a named slab while consuming NONE of its hypotheses. `Gtz.twoMeetingLines_angle_identity` (Gtz/Design/TwoMeetingLinesRigidity.lean:145) and the two sine caps run on exactly the two unit normals this axiom hands over. All of these have zero consumers. Non-vacuity is in kernel with an exact-rational witness, `Gtz.twinFailureDesign` (Gtz/Design/OrthogonalConicAndTwinRefutation.lean:670), which fires the pattern, heaviness and a weak dominator, refuses both line complements, and carries the strict mixed triple `{2,4,5}`.
#BOGUS (2026-08-17, verified by read): "no chart exists for this pattern" is FALSE, and so is the chart-building instruction in ATTACK. `Gtz.twoMeetingLinesDirection` is the two-parameter chart, `Gtz.parameterizedChartCovers_twoMeetingLinesDirection` (Gtz/Design/TwoMeetingLinesCovering.lean:283) is its covering with ZERO hypotheses, and `Gtz.stressFreeStratumIsTieFree_twoMeetingLines_of_chart` (:336) is its consumer. `Gtz/Wave/WiringLineChartRoads.lean` records the correction and adds `Gtz.twoMeetingLinesOnPath_of_chart` (:164). ONLY THE ANALYTIC HALF IS OPEN. The diamond-carrying warning in ATTACK stays correct and still governs the boundary leg. Two concurrent lines are the closest residual pattern to the plane-pair boundary: the two line planes cover five of the six atoms, and it is exactly the sixth that escapes the escape law.
...
STAGE-7 UPDATE (2026-08-16). Three designations died and one component became finite. **STRUCK, AND DO NOT REOPEN: `Gtz.KFourMaxBoostHostsStrictTree` IS KERNEL-REFUTED** by `Gtz.KFourMaxBoostHostsStrictTree_refuted` (Gtz/Wave/KFourBoostProductRefuter.lean:206), at masses `(2,10,6,1,5,2)` and weights in thirtieths, where the unique argmax `{0,4,5}` has leading minors 47 and 255 and determinant -5642 while `{0,1,4}` at less than half the boost product dominates. Neither tree is a star, so no star tiebreaker repairs it. A directed hunt of 200000 restarts landed on a counterexample in 83 percent of them. **STRUCK, AND DO NOT REOPEN: `Gtz.KFourMinThresholdHostsStrictTree`** by `Gtz.kFourMinThresholdHostsStrictTree_refuted` (Gtz/Wave/KFourTreeLaplacian.lean:672), which fails at three of the four mandatory points. **The max-bracket designation is VACUOUS, not merely refuted**: the six K4 edge vectors are totally unimodular, so the squared bracket is one at every spanning tree and the rule designates nothing. With these, every determinantal reading at this stratum is now closed. LANDED SINCE: the star Laplacian congruence (`Gtz.posDef_directionChartGap_star_iff`), the matrix-tree law, the permanent coherence of the canonical star, and the signed Laplacian law of Gtz/Wave/KFourSignedTreeLaplacian.lean, under which Kirchhoff reads every selection determinant as the sixteen-term spanning-tree sum. **THE CORANK-TWO GAUGE WALL IS A RATIONAL VARIETY**: three division-free equations cut it out, and a rank obstruction kills seven of the sixteen trees, so `Gtz.kFourGaugeStarCorankWallClosure_of_nineTrees` reduces the registered component to NINE named trees with no matrix hypothesis. **CLOSED DOOR: the isotropic star cell can never fire on that wall**, because `min h <= m0 + m1 + m2` always, and the wall is its exact boundary. METHOD WARNING, second occurrence: a 3000-point sample reported the three non-gauge stars covering the wall at 3000 of 3000, and a directed hunt found exact rational counterexamples. NOT YET FOREVER, measured only: the nine-tree closure looks TRUE at 16000 of 16000 sampled wall points, with six trees that never fire (`{0,3,5}, {0,4,5}, {1,3,4}, {1,4,5}, {2,3,4}, {2,3,5}`) and stars alone missing about two in four thousand. Stars together with paths cover every sampled point. **THE GENERAL GRAPHIC STRATUM IS NOT A SHORTCUT**: whitening the nine-tree residual gives `sum over T of (1/w_c) a_c a_c^T > I` at a tight frame, which is `Gtz.GtzWeighted 6 3` itself restricted to this stratum. Retiring this axiom through the general stratum means proving the main theorem there first. The WALL component stays finite and live, and it is the one piece worth a fork.
...
theorem obligationKFourFamilySelection : Gtz.KFourFamilySelection :=
  Gtz.kFourFamilySelection_iff_treeStarRefusedAllMaxHeavyWall.mpr
    obligationKnifeBandRefinedKFour
...
theorem obligationKnifeBandRefinedKFour_full :
  ...
  Gtz.kFourKnifeBandRefinedTreeStarRefusedAllMaxHeavyWall_iff.mp
    obligationKnifeBandRefinedKFour
...
STAGE-7 UPDATE (2026-08-16). This axiom now HAS producers, and the STATUS line above is stale on that point. Two of them are landed: `Gtz.obligationSubThresholdBandHinge_of_excessDominates` (Gtz/Wave/OffsetUpperBound.lean:1446) and the strictly weaker `Gtz.obligationSubThresholdBandHinge_of_primitiveExcessDominates` (Gtz/Wave/ThresholdCellDominance.lean:167). Each asks for an excess-dominated selection at every design of every band cell. **BOTH ARE REFUTED AT RANK FOUR, so the STATUS line above is right and this axiom has ZERO live producers.** The check was run in exact rationals on 2026-08-16. A connected graph on `V` vertices with `E` edges gives a graphic design of rank `V - 1` and size `E`, with the transfer impedance as the projection entry, all rational. At rank four the band is sizes eight and nine, and the complete graph on five vertices less one or two edges lands exactly there. Excess dominance fires at NONE of them: size nine gives best margin `-1/9`, size eight gives `-1/10` for both the two-adjacent and the two-disjoint deletion, and the threshold cell `K5` itself gives `-1/10`. The calibration reproduced the landed `K4` values exactly, free excess `1/3` at every edge and best margin `-1/6` against `Gtz.kFourProfile_deficit_eq`. So the excess-dominance instrument is dead at this axiom as well as at the threshold cell, and the dominance lane is closed registry-wide. THE SHAPE A SUCCESSOR NEEDS: the band designs do NOT satisfy `Gtz.CompleteGraphProfile`, because their free excess is not constant — it spans `13/45` to `16/45` at size nine and `11/40` to `19/40` at size eight. A weaker profile is necessary, bounded free excess together with a meeting-pair floor. Once that profile exists the crowdedness count closes the lane, because four edges on five vertices with meeting degree at most one would span at least six vertices. CAUTION on the climbing route named under ATTACK: `Gtz.UniformPositionBridge.gtzWeighted_belowWindow_of_predecessor` needs `Gtz.GtzWeightedAll (rank - 1)`, so at rank four it needs rank three. That route is DOWNSTREAM of the five on-path obligations and is not a parallel lane. CAUTION on the climbing route named under ATTACK: `Gtz.UniformPositionBridge.gtzWeighted_belowWindow_of_predecessor` needs `Gtz.GtzWeightedAll (rank - 1)`, so at rank four it needs rank three. That route is DOWNSTREAM of the five on-path obligations and is not a parallel lane.
...
STAGE-7 UPDATE (2026-08-16). The excess-dominance lane is CLOSED here, and the closure is unconditional at rank three. **STRUCK, AND DO NOT REOPEN: the hypothesis of `Gtz.obligationThresholdCellHingeRankFourAndUp_of_excessDominates` and of its primitive repair IS KERNEL-REFUTED at the rank-three member**, by `Gtz.not_forall_excessDominates_sixThree_unconditional` (Gtz/Wave/ThresholdCellDominance.lean:790), which carries no hypothesis because `Gtz.completeGraphProfile_graphicKFour` (:775) inhabits the profile with the landed `Gtz.graphicKFourDesign`. THE STRENGTH GAP, which is why the producers were the wrong object: `ExcessDominatesBlock` refutes `Gtz.IsTie`, so the quantified hypothesis asserts that the cell holds NO tie at all, while this axiom asserts only that a tie carries a parallel pair. The producer proves its implication by refuting its own antecedent. THE MECHANISM, rank-uniform: the complete graph on `rank + 1` vertices has `rank * (rank + 1) / 2` edges and cycle rank `rank`, so its graphic design sits at exactly this cell at every rank. Its free excess falls short of two meeting partners by exactly the reciprocal of the cell size, so dominance would force every selected edge to meet at most one other, and `rank` edges cannot do that. ONE RATIO GOVERNS THE THREE INSTRUMENTS: `profileRatio = 2 - 2 / rank`. Diagonal dominance needs a ratio above two and never fires. The sign-free cell needs a squared ratio above three and first fires at rank eight. Sylvester with coherence needs a ratio above one and holds from rank three, with the coherent determinant factoring as the off-diagonal cubed times the squared ratio deficit times the ratio plus two. At rank three that returns `5/864` and the sign-blind branch returns `-49/864`, both landed K4 values of one rank-uniform polynomial. NOT YET FOREVER, and this is the exact gap: the general-rank crowdedness is proved in kernel ONLY at rank three. Above it the vertex count `3p + 2s <= rank + 1` against `2p + s = rank` is a hypothesis. Landing it, or a `decide` over the 210 four-subsets of `Fin 10` with the K5 adjacency, closes this lane at every rank at once. Everything else it needs is landed. **The sixth on-path door `Gtz.allFiveOnPath_of_primitiveExcessDominates` carries the same instrument to all five class obligations, and the same witness refutes it, so this lane is closed on-path and off-path together.**
